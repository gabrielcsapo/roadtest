...
// In static/CDN deployments (e.g. GitHub Pages), /__roadtest_source__ doesn't exist.
// buildUi() writes roadtest-sources.json alongside the bundle as a fallback.
...
function loadStaticSources(): Promise<Record<string, string>> {
  if (!_staticSourcesPromise) {
    _staticSourcesPromise = fetch("./roadtest-sources.json")
...
export function CodeTab({ suiteName, coverage, testCoverage, suites, onSelectTest }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/__roadtest_source__?path=${encodeURIComponent(selectedPath)}`, {
      headers: { "X-MSW-Intention": "bypass" },
    })
      .then((r) => {
        if (r.ok) return r.text();
        // Fallback: static sources bundle written by `roadtest build` for CDN/static hosts
  ...
  if (!coverage) {
    return (
      <div style={{ padding: "32px 20px", textAlign: "center", color: "#4b4b60", fontSize: 13 }}>
        ...
        Coverage not available — run <code style={{ color: "#a5b4fc" }}>roadtest --ui</code> to
        enable
      </div>
    );
  }
