...
function useGraphData(suites: TestSuite[], coverage: IstanbulCoverage | null) {
  ...
  useEffect(() => {
    // In a static build, graph data is pre-generated as roadtest-graph.json.
    // In dev mode, it's served on-the-fly by the roadtest dev server middleware.
    fetch("./roadtest-graph.json")
      .then((r) => {
      ...
      .then(setRaw)
      .catch(() =>
        fetch("/__roadtest_graph__", { headers: { "X-MSW-Intention": "bypass" } })
          .then((r) => r.json())
          .then(setRaw)
          .catch(() => setError(true)),
      );
      ...
      >
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ fontSize: 13 }}>
            ...
            <code style={{ color: "#a5b4fc" }}>roadtest --ui</code>
          </div>
        </div>
      </div>
    );
