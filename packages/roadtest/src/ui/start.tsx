...
export async function reloadFile(path: string, loader: () => Promise<unknown>) {
  ...
  try {
    ...
    console.error(`[roadtest] Failed to reload ${path}:`, e);
  }
  setCurrentSourceFile(null);
...
export async function startApp(
  ...
  if (window.name === "__vt_sandbox") {
    ...
    (window as unknown as Record<string, unknown>)["__roadtest"] = {
      store,
      runAll,
      runSuite,
      runTest,
  ...
  if (window.name === "__vt_display") {
    ...
    async function showTest(
      ...
      try {
        ...
        if (!(e instanceof Error) || !("__vtDisplayStop" in e)) {
          console.debug("[roadtest display]", e);
        }
      } finally {
    ...
    function scanComponentTree(): ComponentTreeResult {
      // 1. Check the expected container (roadtest's setRenderTarget destination).
