...
export async function writeSnapshotsToServer(suites: TestSuite[]): Promise<void> {
  ...
  await fetch("/__fieldtest_snapshot_write__", {
    method: "POST",
...
export async function compareSnapshotsWithServer(suites: TestSuite[]): Promise<void> {
  ...
  try {
    const res = await fetch("/__fieldtest_snapshots__", {
      headers: { "X-MSW-Intention": "bypass" },
    });
    if (!res.ok) {
      console.log(
        "[fieldtest] snapshot comparison skipped: /__fieldtest_snapshots__ returned",
        res.status,
      );
      return;
    }
    baselines = (await res.json()) as Record<string, string>;
    console.log("[fieldtest] snapshot baselines loaded:", Object.keys(baselines).length, "files");
  } catch (e) {
    console.log("[fieldtest] snapshot comparison skipped: fetch failed", e);
    return;
  }
  ...
  console.log(`[fieldtest] snapshot comparison complete: ${totalPass} passed, ${totalFail} failed`);
}

/**
 * Check whether the FieldTest dev server is running and whether
...
export async function checkDevServer(): Promise<{ devServer: boolean; autoWrite: boolean }> {
  try {
    const res = await fetch("/__fieldtest_update_snapshots__", {
      headers: { "X-MSW-Intention": "bypass" },
    });
