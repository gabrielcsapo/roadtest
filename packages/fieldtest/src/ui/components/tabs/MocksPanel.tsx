 ...
 *
 * Falls back to raw on-disk source (via /__fieldtest_source__ or
 * fieldtest-sources.json) when source map resolution is unavailable.
 */
async function fetchResolved(
  ...
  async function fetchRawSource(): Promise<string | null> {
    try {
      const res = await fetch(`/__fieldtest_source__?url=${encodeURIComponent(vitePath)}`);
      if (res.ok) return res.text();
    } catch {
      /* build mode */
    }
    try {
      const jsonRes = await fetch("./fieldtest-sources.json");
      if (jsonRes.ok) {
