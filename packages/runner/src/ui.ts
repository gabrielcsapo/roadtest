...
async function buildGraphData(root: string): Promise<{ nodes: {id:string;shortPath:string}[]; edges: {from:string;to:string}[] }> {
...
function fieldtestDevPlugin(): Plugin {
  ...
  return {
    ...
    transformIndexHtml(html) {
      return html.replace(/<script\s[^>]*\bsrc=["'][^"']*\/src\/[^"']+["'][^>]*><\/script>\s*/gi, '')
    },
...
export async function startUi() {
  const include =
    process.argv.find((a, i) => i > 1 && !a.startsWith('--') && !process.argv[i - 1]?.includes('--')) ??
    'src/**/*.test.{ts,tsx}'
...
const PREVIEW_CANDIDATES = ['.fieldtest/preview.tsx', '.fieldtest/preview.ts', '.fieldtest/preview.jsx', '.fieldtest/preview.js']
const SETUP_CANDIDATES   = ['.fieldtest/setup.ts',    '.fieldtest/setup.tsx',   '.fieldtest/setup.js',   '.fieldtest/setup.jsx']
...
export async function buildUi() {
  ...
  for (let i = 2; i < process.argv.length; i++) {
    if (process.argv[i].startsWith('--')) {
      ...
      if (!process.argv[i].includes('=') && i + 1 < process.argv.length && !process.argv[i + 1].startsWith('--')) i++
    } else {
  ...
  const patchedHtml = originalHtml
    ...
    .replace('</body>', `  <script type="module" src="/__fieldtest_entry__.ts"></script>\n  </body>`)
