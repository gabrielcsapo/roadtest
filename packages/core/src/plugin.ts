...
const NODE_BUILTINS = new Set([
  ...
  'dgram', 'diagnostics_channel', 'dns', 'domain', 'events', 'fs',
...
async function extractStaticImports(code: string, id: string): Promise<string[]> {
  ...
  let ast: any
  try { ast = await parseAstAsync(jsCode) } catch { return [] }
...
async function hasNodeBuiltinDep(
  ...
  for (const specifier of specifiers) {
    ...
    let src: string
    try { src = readFileSync(resolvedPath, 'utf-8') } catch { continue }
...
interface NodeAssertion { label: string; status: 'pass' | 'fail'; error?: string }
interface NodeConsoleEntry { level: string; args: string[]; timestamp: number }

interface NodeTestCase {
  id: string; name: string; suiteId: string; suiteName: string
  status: 'pending' | 'running' | 'pass' | 'fail' | 'skipped'
  error?: string
  assertions: NodeAssertion[]
  consoleLogs: NodeConsoleEntry[]
  snapshots: never[]; networkEntries: never[]; mockEntries: never[]
  testCoverage: null
  duration?: number
...
interface NodeTestSuite {
  id: string; name: string; tests: NodeTestCase[]
  status: 'pending' | 'running' | 'pass' | 'fail'
  sourceFile: string; runtime: 'node'; duration?: number
}
...
function createNodeContext(sourceFile: string) {
  ...
  function makeId() { return String(_id++) }
  ...
  function createMatchers(received: unknown, negated = false): any {
    ...
    return {
      ...
      toBe: (e: unknown) => assert(Object.is(received, e), `Expected ${stringify(e)} but received ${stringify(received)}`, `toBe(${stringify(e)})`),
      toEqual: (e: unknown) => assert(JSON.stringify(received) === JSON.stringify(e), `Expected ${stringify(e)} but received ${stringify(received)}`, `toEqual(${stringify(e)})`),
      toBeTruthy: () => assert(Boolean(received), `Expected truthy but received ${stringify(received)}`, 'toBeTruthy()'),
      toBeFalsy: () => assert(!received, `Expected falsy but received ${stringify(received)}`, 'toBeFalsy()'),
      toBeNull: () => assert(received === null, `Expected null but received ${stringify(received)}`, 'toBeNull()'),
      toBeUndefined: () => assert(received === undefined, `Expected undefined but received ${stringify(received)}`, 'toBeUndefined()'),
      toBeDefined: () => assert(received !== undefined, 'Expected a defined value', 'toBeDefined()'),
      toContain: (item: unknown) => {
        const ok = Array.isArray(received) ? received.includes(item) : typeof received === 'string' && received.includes(String(item))
      ...
      toBeGreaterThan: (n: number) => assert((received as number) > n, `Expected ${received} > ${n}`, `toBeGreaterThan(${n})`),
      toBeLessThan: (n: number) => assert((received as number) < n, `Expected ${received} < ${n}`, `toBeLessThan(${n})`),
    }
  }

  function describe(name: string, fn: () => void) {
    const suite: NodeTestSuite = { id: makeId(), name, tests: [], status: 'pending', sourceFile, runtime: 'node' }
  ...
  function it(name: string, fn: () => void | Promise<void>) {
    const suite = currentSuite ?? (() => {
      const s: NodeTestSuite = { id: makeId(), name: '(root)', tests: [], status: 'pending', sourceFile, runtime: 'node' }
      suites.push(s)
      currentSuite = s
      return s
    })()
    suite.tests.push({
      id: makeId(), name, suiteId: suite.id, suiteName: suite.name,
      status: 'pending', assertions: [], consoleLogs: [],
      snapshots: [], networkEntries: [], mockEntries: [], testCoverage: null,
      fn,
    })
  }
    ...
    for (const suite of suites) {
      ...
      for (const test of suite.tests) {
        ...
        for (const lvl of LEVELS) {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          ;(console as any)[lvl] = (...args: unknown[]) => logs.push({
            level: lvl,
            args: args.map(a => typeof a === 'string' ? a : (() => { try { return JSON.stringify(a) } catch { return String(a) } })()),
            timestamp: Date.now(),
          })
        }
        try {
...
async function runNodeTestFile(server: ViteDevServer, filePath: string, root: string): Promise<NodeTestSuite[]> {
...
export function fieldtest(options: FieldtestOptions = {}): Plugin {
  ...
  return {
    ...
    async transform(code, id) {
      ...
      if (needsNode) {
        return { code: `import { __vtRegisterNodeTest } from '@viewtest/core'\n__vtRegisterNodeTest(${JSON.stringify(id)})` }
      }
