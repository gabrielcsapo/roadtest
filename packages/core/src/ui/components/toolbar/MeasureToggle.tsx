...
function BoxOverlay({ info }: { info: BoxInfo }) {
  const { rect, margin: m, padding: p, border: b } = info;
  ...
  const layer = (x: number, y: number, w: number, h: number, bg: string, label?: string) => (
    <div
      style={{
        position: "fixed",
        left: x,
  ...
  useEffect(() => {
    ...
    function onOver(e: MouseEvent) {
      ...
      setInfo({
        ...
        containerRect,
        margin: {
          top: parsePx(s.marginTop),
          right: parsePx(s.marginRight),
          bottom: parsePx(s.marginBottom),
          left: parsePx(s.marginLeft),
        },
        padding: {
        ...
        border: {
          top: parsePx(s.borderTopWidth),
          right: parsePx(s.borderRightWidth),
          bottom: parsePx(s.borderBottomWidth),
          left: parsePx(s.borderLeftWidth),
        },
      });
    }
    function onOut() {
