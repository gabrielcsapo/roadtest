...
function renderMarkdown(md: string, highlighter: Highlighter | null): string {
  ...
  html = html.replace(/^\|(.+)\|\s*\n\|[-| :]+\|\s*\n((?:\|.+\|\s*\n?)+)/gm, (_match, header, body) => {
    const cols = header.split('|').map((h: string) => h.trim()).filter(Boolean)
  ...
  return result.map(l => (l.startsWith('<p>') && !l.endsWith('</p>') ? l + '</p>' : l)).join('\n')
}
...
function highlight(text: string, query: string): string {
  ...
  return text.replace(new RegExp(`(${escaped})`, 'gi'), '<mark class="bg-ft-gold/30 text-ft-gold-hi rounded px-0.5">$1</mark>')
}

// ── Shared highlighter — loaded once, shared across all sections ──────────────
let _highlighter: Highlighter | null = null
getHighlighter().then(h => { _highlighter = h })
...
function DocSection({ doc, query }: { doc: DocEntry; query: string }) {
  ...
  useEffect(() => {
    if (_highlighter) { setHighlighter(_highlighter); return }
    getHighlighter().then(h => { _highlighter = h; setHighlighter(h) })
  }, [])
...
export default function Docs() {
  ...
  function scrollTo(id: string) {
    ...
    setTimeout(() => { scrollPendingRef.current = false }, 800)
  }
        ...
        <main ref={mainRef} className="min-w-0 flex-1 px-8 py-8 lg:px-12">
          ...
          {filtered.length === 0 ? (
            <div className="py-20 text-center">
              ...
              <p className="text-ft-mid">No docs matched <strong className="text-ft-text">"{query}"</strong></p>
              <button
                onClick={() => setQuery('')}
            ...
            filtered.map(doc => (
              <DocSection key={doc.id} doc={doc} query={query} />
            ))
          )}
        </main>
      </div>
