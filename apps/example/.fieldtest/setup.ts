...
type PendingEntry = { t0: number; method: string; url: string; requestBody?: string; test: TestCase }
...
if (isBrowser && isSandboxFrame) {
  ...
  try {
    ...
    await w.start({ serviceWorker: { url: './mockServiceWorker.js' }, onUnhandledRequest: 'bypass', quiet: true })
  } catch (e) {
  ...
  w.events.on('*', async (raw) => {
    ...
    if (type === 'request:start' && request) {
      ...
      pending.set(requestId, { t0: Date.now(), method: request.method, url: request.url, test: currentTest })
      try {
  ...
  registerBeforeDisplayHook(() => { displayHandlers.length = 0 })

  _mswInstance = {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    use(...handlers: any[]) { displayHandlers.push(...handlers) },
    resetHandlers() { displayHandlers.length = 0 },
  }
} else if (!isBrowser) {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const worker: { use(...h: any[]): void; resetHandlers(): void } =
  _mswInstance ?? { use() {}, resetHandlers() {} }
