...
describe("validation", () => {
  ...
  describe("validateField", () => {
    ...
    it("returns error for non-alphanumeric", () => {
      expect(validateField("hello!", [{ type: "alphanumeric" }])).toBe(
    ...
    it("returns error for pattern mismatch with custom message", () => {
      expect(
        validateField("abc", [{ type: "pattern", regex: /^\d+$/, message: "Digits only" }]),
      ).toBe("Digits only");
    });
...
describe("validateField - comprehensive rule combinations", () => {
  const emailRuleCases = [
    ...
    },
    {
      ...
      rules: [{ type: "email" as const }, { type: "maxLength" as const, max: 50 }],
      expected: null,
    },
    {
      value: "hi",
      rules: [{ type: "minLength" as const, min: 5 }],
      expected: "Minimum length is 5",
    },
    { value: "hello", rules: [{ type: "minLength" as const, min: 5 }], expected: null },
    {
      value: "hello!",
      rules: [{ type: "alphanumeric" as const }],
      expected: "Only letters and numbers are allowed",
    },
    ...
    { value: "test", rules: [{ type: "maxLength" as const, max: 10 }], expected: null },
    {
      ...
      rules: [{ type: "maxLength" as const, max: 5 }],
      expected: "Maximum length is 5",
    },
    { value: "", rules: [], expected: null },
    { value: "abc", rules: [{ type: "pattern" as const, regex: /^[a-z]+$/ }], expected: null },
    {
      value: "ABC",
      rules: [{ type: "pattern" as const, regex: /^[a-z]+$/ }],
      expected: "Invalid format",
    },
    { value: "   ", rules: [{ type: "required" as const }], expected: "This field is required" },
    {
      value: "x".repeat(100),
      rules: [{ type: "maxLength" as const, max: 99 }],
      expected: "Maximum length is 99",
    },
    ...
    {
      value: "x".repeat(7),
      rules: [{ type: "minLength" as const, min: 8 }],
      expected: "Minimum length is 8",
    },
