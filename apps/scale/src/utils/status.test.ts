...
describe('status', () => {
  ...
  const allComplianceStatuses: ComplianceStatus[] = ['compliant', 'non-compliant', 'in-progress', 'not-applicable']
  ...
  describe('complianceStatusToColor additional', () => {
    ...
    it('compliant and non-compliant have different colors', () => {
      expect(complianceStatusToColor('compliant')).not.toBe(complianceStatusToColor('non-compliant'))
    })
  })
})

describe('canTransitionTo - comprehensive matrix', () => {
  const allStatuses: Array<'active' | 'inactive' | 'pending' | 'archived'> = ['active', 'inactive', 'pending', 'archived']
  const validMatrix: Array<{ from: 'active' | 'inactive' | 'pending' | 'archived'; to: 'active' | 'inactive' | 'pending' | 'archived'; valid: boolean }> = [
