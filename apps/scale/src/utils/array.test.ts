...
describe('array', () => {
  describe('groupBy', () => {
    ...
    it('groups by numeric field (converted to string)', () => {
      const arr = [{ score: 90, name: 'A' }, { score: 90, name: 'B' }, { score: 80, name: 'C' }]
  ...
  describe('chunk', () => {
    const cases = [
      ...
      { arr: [1, 2, 3, 4], size: 2, expected: [[1, 2], [3, 4]] },
      { arr: [1], size: 1, expected: [[1]] },
      { arr: [], size: 2, expected: [] },
      { arr: [1, 2, 3], size: 10, expected: [[1, 2, 3]] },
      { arr: [1, 2, 3, 4, 5, 6], size: 3, expected: [[1, 2, 3], [4, 5, 6]] },
    ]
    for (const c of cases) {
  ...
  describe('groupBy - comprehensive', () => {
    ...
    it('groupBy preserves insertion order of items within group', () => {
      const arr = [{ grp: 'a', val: 1 }, { grp: 'a', val: 2 }]
  ...
  describe('unique - comprehensive', () => {
    for (let n = 1; n <= 10; n++) {
      it(`unique removes ${n} duplicates from 2*n array`, () => {
        const arr = [...Array.from({ length: n }, (_, i) => i), ...Array.from({ length: n }, (_, i) => i)]
