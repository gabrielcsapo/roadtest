...
describe('StatusBadge', () => {
  ...
  for (const status of statuses) {
    it(`custom class for status=${status}`, async () => {
      const { getByTestId } = await render(<StatusBadge status={status} className={`badge-${status}`} />)
      expect(getByTestId('status-badge')).toBeDefined()
    })
  }
  ...
  for (let i = 0; i < 4; i++) {
    it(`className test ${i + 1}`, async () => {
      const { getByTestId } = await render(<StatusBadge status={statuses[i % 4]} className={`cls-${i}`} />)
      expect(getByTestId('status-badge')).toBeDefined()
    })
  }
