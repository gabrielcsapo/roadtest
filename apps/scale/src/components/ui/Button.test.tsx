...
describe('Button', () => {
  ...
  for (const variant of variants) {
    for (const size of sizes) {
      it(`renders variant=${variant} size=${size}`, async () => {
        const { getByTestId } = await render(<Button variant={variant} size={size}>Click</Button>)
  ...
  for (const variant of variants) {
    it(`disabled state for variant=${variant}`, async () => {
      const { getByTestId } = await render(<Button variant={variant} disabled>Click</Button>)
      const btn = getByTestId('button')
      expect(btn).toBeDefined()
    })
  }
  ...
  for (const variant of variants) {
    it(`loading state for variant=${variant}`, async () => {
      const { getByTestId } = await render(<Button variant={variant} loading>Click</Button>)
  ...
  it('calls onClick when clicked', async () => {
    let clicked = false
    const { getByTestId } = await render(<Button onClick={() => { clicked = true }}>Click</Button>)
  ...
  it('does not call onClick when disabled', async () => {
    let clicked = false
    const { getByTestId } = await render(<Button disabled onClick={() => { clicked = true }}>Click</Button>)
    await fireEvent.click(getByTestId('button'))
  ...
  it('does not call onClick when loading', async () => {
    let clicked = false
    const { getByTestId } = await render(<Button loading onClick={() => { clicked = true }}>Click</Button>)
    await fireEvent.click(getByTestId('button'))
    expect(clicked).toBe(false)
  })
  ...
  it('does not show icon when loading', async () => {
    const { queryByTestId } = await render(<Button loading icon={<span>★</span>}>Loading</Button>)
  ...
  it('fires multiple click events', async () => {
    let count = 0
    const { getByTestId } = await render(<Button onClick={() => { count++ }}>Click</Button>)
    await fireEvent.click(getByTestId('button'))
    await fireEvent.click(getByTestId('button'))
    await fireEvent.click(getByTestId('button'))
    expect(count).toBe(3)
  ...
  for (const size of sizes) {
    it(`disabled at size=${size}`, async () => {
      const { getByTestId } = await render(<Button size={size} disabled>Disabled</Button>)
      expect(getByTestId('button')).toBeDefined()
    })
  }
  ...
  for (const size of sizes) {
    it(`loading at size=${size}`, async () => {
      const { getByTestId } = await render(<Button size={size} loading>Loading</Button>)
      expect(getByTestId('button-spinner')).toBeDefined()
    })
  }
  ...
  for (const variant of variants) {
    it(`fullWidth with variant=${variant}`, async () => {
      const { getByTestId } = await render(<Button variant={variant} fullWidth>Full</Button>)
      expect(getByTestId('button')).toBeDefined()
    })
  }
  ...
  for (const variant of variants) {
    it(`icon with variant=${variant}`, async () => {
      const { getByTestId } = await render(<Button variant={variant} icon={<span>+</span>}>Add</Button>)
      expect(getByTestId('button-icon')).toBeDefined()
    })
  }
  ...
  it('fires onClick with event object', async () => {
    let event: any = null
    const { getByTestId } = await render(<Button onClick={(e) => { event = e }}>Click</Button>)
    await fireEvent.click(getByTestId('button'))
