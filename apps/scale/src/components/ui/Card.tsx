...
export const Card: React.FC<CardProps> = ({
  ...
  const headerStyle: React.CSSProperties = {
    padding:
      padding === "none" ? "16px 20px 0" : `${paddingValues[padding]} ${paddingValues[padding]} 0`,
  ...
  const footerStyle: React.CSSProperties = {
    padding:
      padding === "none" ? "16px 20px" : `0 ${paddingValues[padding]} ${paddingValues[padding]}`,
    ...
    >
      ...
      {(title || subtitle) && (
        <div style={headerStyle} data-testid={`${testId}-header`}>
          ...
          {subtitle && (
            <p style={subtitleStyle} data-testid={`${testId}-subtitle`}>
              {subtitle}
            </p>
          )}
        </div>
