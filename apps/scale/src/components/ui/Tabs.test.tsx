...
describe("Tabs", () => {
    ...
    for (const variant of variants) {
      it(`renders ${count} tabs with variant=${variant}`, async () => {
        ...
        const { getByTestId } = await render(
          <Tabs tabs={tabs} active="tab0" onChange={() => {}} variant={variant} />,
        );
  ...
  for (let i = 0; i < 11; i++) {
    it(`disabled tab test ${i + 1}`, async () => {
      ...
      const { getByTestId } = await render(
        <Tabs tabs={tabs} active="enabled" onChange={() => {}} />,
      );
  ...
  it("custom className", async () => {
    const tabs = makeTabs(3);
    const { getByTestId } = await render(
      <Tabs tabs={tabs} active="tab0" onChange={() => {}} className="custom" />,
    );
    expect(getByTestId("tabs-wrapper")).toBeDefined();
  });
  ...
  it("custom testId", async () => {
    const tabs = makeTabs(3);
    const { getByTestId } = await render(
      <Tabs tabs={tabs} active="tab0" onChange={() => {}} data-testid="my-tabs" />,
    );
  ...
  it("data-variant attribute set", async () => {
    const tabs = makeTabs(3);
    const { getByTestId } = await render(
      <Tabs tabs={tabs} active="tab0" onChange={() => {}} variant="pill" />,
    );
    expect(getByTestId("tabs")).toBeDefined();
  });
