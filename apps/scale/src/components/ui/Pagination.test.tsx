...
describe('Pagination', () => {
  ...
  it('first button navigates to page 1', async () => {
    let page = 5
    const { getByTestId } = await render(
      <Pagination page={page} total={100} pageSize={10} onPageChange={(p) => { page = p }} />
    )
  ...
  it('page number click works', async () => {
    let page = 1
    const { getByTestId } = await render(
      <Pagination page={page} total={50} pageSize={10} onPageChange={(p) => { page = p }} />
    )
  ...
  it('shows page size selector when onPageSizeChange provided', async () => {
    const { getByTestId } = await render(
      <Pagination page={1} total={100} pageSize={10} onPageChange={() => {}} onPageSizeChange={() => {}} />
    )
  ...
  it('calls onPageSizeChange when changed', async () => {
    let size = 10
    const { getByTestId } = await render(
      <Pagination page={1} total={100} pageSize={size} onPageChange={() => {}} onPageSizeChange={(s) => { size = s }} />
    )
  ...
  it('pageSizeOptions are rendered in select', async () => {
    const { getByTestId } = await render(
      <Pagination page={1} total={100} pageSize={10} onPageChange={() => {}} onPageSizeChange={() => {}} pageSizeOptions={[5, 10, 20]} />
    )
    expect(getByTestId('pagination-page-size')).toBeDefined()
  })
  ...
  it('custom pageSizeOptions', async () => {
    const { getByTestId } = await render(
      <Pagination page={1} total={100} pageSize={5} onPageChange={() => {}} onPageSizeChange={() => {}} pageSizeOptions={[5, 15, 30, 60]} />
    )
    expect(getByTestId('pagination-page-size')).toBeDefined()
  })
  ...
  it('prev disabled on page 1 does not fire', async () => {
    let fired = false
    const { getByTestId } = await render(
      <Pagination page={1} total={100} pageSize={10} onPageChange={() => { fired = true }} />
    )
    await fireEvent.click(getByTestId('pagination-prev'))
  ...
  it('next disabled on last page does not fire', async () => {
    let fired = false
    const { getByTestId } = await render(
      <Pagination page={10} total={100} pageSize={10} onPageChange={() => { fired = true }} />
    )
    await fireEvent.click(getByTestId('pagination-next'))
    expect(fired).toBe(false)
  })
  ...
  it('snapshot: with page size', async () => {
    await render(<Pagination page={1} total={100} pageSize={10} onPageChange={() => {}} onPageSizeChange={() => {}} />)
  ...
  for (let i = 0; i < 10; i++) {
    it(`callback test ${i + 1}`, async () => {
      let page = 1
      const { getByTestId } = await render(
        <Pagination page={1} total={50} pageSize={10} onPageChange={(p) => { page = p }} />
      )
  ...
  it('custom testId', async () => {
    const { getByTestId } = await render(
      <Pagination page={1} total={50} pageSize={10} onPageChange={() => {}} data-testid="my-pager" />
    )
