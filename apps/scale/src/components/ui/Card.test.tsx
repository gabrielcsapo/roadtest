...
describe('Card', () => {
  ...
  for (const padding of paddings) {
    for (const shadow of shadows) {
      it(`renders padding=${padding} shadow=${shadow}`, async () => {
        const { getByTestId } = await render(<Card padding={padding} shadow={shadow}>Content</Card>)
  ...
  it('renders subtitle', async () => {
    const { getByTestId } = await render(<Card title="Title" subtitle="Sub">Content</Card>)
  ...
  for (let i = 0; i < 8; i++) {
    it(`footer test ${i + 1}`, async () => {
      const { getByTestId } = await render(<Card footer={<span data-testid={`footer-${i}`}>Footer {i}</span>}>Content</Card>)
  ...
  it('calls onClick when clicked', async () => {
    let clicked = false
    const { getByTestId } = await render(<Card onClick={() => { clicked = true }}>Click me</Card>)
  ...
  for (let i = 0; i < 7; i++) {
    it(`click callback test ${i + 1}`, async () => {
      let count = 0
      const { getByTestId } = await render(<Card onClick={() => { count++ }}>Card {i}</Card>)
      await fireEvent.click(getByTestId('card'))
      expect(count).toBe(1)
    })
  }
  ...
  for (let i = 0; i < 7; i++) {
    it(`loading state test ${i + 1}`, async () => {
      const { getByTestId } = await render(<Card loading title={`Card ${i}`}>Content</Card>)
      expect(getByTestId('card-spinner')).toBeDefined()
    })
  }
  ...
  for (let i = 0; i < 11; i++) {
    it(`content variation ${i + 6}`, async () => {
      const { getByTestId } = await render(<Card title={`Title ${i}`} subtitle={`Sub ${i}`}>Content {i}</Card>)
      expect(getByTestId('card-title')).toBeDefined()
    })
  }
  ...
  it('snapshot: default', async () => {
    await render(<Card title="Security" subtitle="Overview">Content</Card>)
    await snapshot('card-default')
  })

  it('snapshot: padding=lg', async () => {
    await render(<Card padding="lg" shadow="lg">Content</Card>)
    await snapshot('card-lg')
  })

  it('snapshot: loading', async () => {
    await render(<Card loading title="Loading">Content</Card>)
  ...
  it('snapshot: no border', async () => {
    await render(<Card border={false} shadow="md">Content</Card>)
