...
describe('Tooltip', () => {
  ...
  for (const placement of placements) {
    it(`renders with placement=${placement}`, async () => {
      const { getByTestId } = await render(
        <Tooltip content="Info" placement={placement}><button>Hover me</button></Tooltip>
      )
  ...
  for (const placement of placements) {
    it(`shows tooltip on hover for placement=${placement}`, async () => {
      const { getByTestId } = await render(
        <Tooltip content="Tooltip text" placement={placement}><button>Hover</button></Tooltip>
      )
  ...
  for (const placement of placements) {
    it(`hides tooltip on mouse leave for placement=${placement}`, async () => {
      const { getByTestId, queryByTestId } = await render(
        <Tooltip content="Tooltip" placement={placement}><span>Hover</span></Tooltip>
      )
      await fireEvent.mouseEnter(getByTestId('tooltip-wrapper'))
  ...
  for (const placement of placements) {
    it(`data-placement=${placement} is set`, async () => {
      const { getByTestId } = await render(
        <Tooltip content="Test" placement={placement}><span>Hover</span></Tooltip>
      )
      await fireEvent.mouseEnter(getByTestId('tooltip-wrapper'))
      expect(getByTestId('tooltip')).toBeDefined()
    })
  ...
  for (const placement of placements) {
    it(`snapshot: placement=${placement}`, async () => {
      const { getByTestId } = await render(
        <Tooltip content="Tooltip" placement={placement}><button>Btn</button></Tooltip>
      )
      await fireEvent.mouseEnter(getByTestId('tooltip-wrapper'))
  ...
  for (const content of contents) {
    it(`renders content: "${content.substring(0, 30)}"`, async () => {
      const { getByTestId } = await render(
        <Tooltip content={content}><span>Hover</span></Tooltip>
      )
      await fireEvent.mouseEnter(getByTestId('tooltip-wrapper'))
      expect(getByTestId('tooltip')).toBeDefined()
    })
  ...
  it('does not show when disabled', async () => {
    const { getByTestId, queryByTestId } = await render(
      <Tooltip content="Hidden" disabled><button>Hover</button></Tooltip>
    )
    await fireEvent.mouseEnter(getByTestId('tooltip-wrapper'))
    expect(queryByTestId('tooltip')).toBeNull()
  })
  ...
  for (const placement of placements) {
    it(`disabled with placement=${placement}`, async () => {
      const { getByTestId, queryByTestId } = await render(
        <Tooltip content="Hidden" disabled placement={placement}><span>Hover</span></Tooltip>
      )
      await fireEvent.mouseEnter(getByTestId('tooltip-wrapper'))
      expect(queryByTestId('tooltip')).toBeNull()
    })
  ...
  it('disabled=false shows tooltip', async () => {
    const { getByTestId } = await render(
      <Tooltip content="Visible" disabled={false}><span>Hover</span></Tooltip>
    )
    await fireEvent.mouseEnter(getByTestId('tooltip-wrapper'))
    expect(getByTestId('tooltip')).toBeDefined()
  })
  ...
  it('wrapper still renders when disabled', async () => {
    const { getByTestId } = await render(
      <Tooltip content="Hidden" disabled><span>Text</span></Tooltip>
    )
    expect(getByTestId('tooltip-wrapper')).toBeDefined()
  })
  ...
  it('shows on focus', async () => {
    const { getByTestId } = await render(
      <Tooltip content="Focus tooltip"><button>Focus me</button></Tooltip>
    )
  ...
  it('hides on blur', async () => {
    const { getByTestId, queryByTestId } = await render(
      <Tooltip content="Focus tooltip"><button>Focus</button></Tooltip>
    )
    await fireEvent.focus(getByTestId('tooltip-wrapper'))
  ...
  it('tooltip not shown by default', async () => {
    const { queryByTestId } = await render(
      <Tooltip content="Hidden"><button>Btn</button></Tooltip>
    )
    expect(queryByTestId('tooltip')).toBeNull()
  })
  ...
  it('trigger always shown', async () => {
    const { getByTestId } = await render(
      <Tooltip content="Test"><button>Trigger</button></Tooltip>
    )
    expect(getByTestId('tooltip-trigger')).toBeDefined()
  })
  ...
  for (const delay of [0, 100, 200, 300, 500]) {
    it(`renders with delay=${delay}`, async () => {
      const { getByTestId } = await render(
        <Tooltip content="Delayed" delay={delay}><button>Hover</button></Tooltip>
      )
      expect(getByTestId('tooltip-wrapper')).toBeDefined()
    })
  }
  ...
  for (const delay of [0, 100, 200, 300, 500]) {
    it(`wrapper renders with delay=${delay}`, async () => {
      const { getByTestId } = await render(
        <Tooltip content="Delayed" delay={delay}><span>Text</span></Tooltip>
      )
      expect(getByTestId('tooltip-trigger')).toBeDefined()
    })
  }
  ...
  it('custom className', async () => {
    const { getByTestId } = await render(
      <Tooltip content="Test" className="custom"><span>Hover</span></Tooltip>
    )
    expect(getByTestId('tooltip-wrapper')).toBeDefined()
  })
  ...
  it('custom testId', async () => {
    const { getByTestId } = await render(
      <Tooltip content="Test" data-testid="my-tip"><span>Hover</span></Tooltip>
    )
  ...
  for (let i = 0; i < 12; i++) {
    it(`accessibility test ${i + 1}`, async () => {
      const { getByTestId } = await render(
        <Tooltip content={`Info ${i}`} placement={placements[i % 4]}><span>Hover {i}</span></Tooltip>
      )
      await fireEvent.mouseEnter(getByTestId('tooltip-wrapper'))
      expect(getByTestId('tooltip')).toBeDefined()
    })
