...
describe("SearchInput", () => {
  ...
  for (const val of testValues) {
    it(`renders with value="${val.substring(0, 20)}"`, async () => {
      const { getByTestId } = await render(<SearchInput value={val} onChange={() => {}} />);
  ...
  it("no clear button when onClear not provided", async () => {
    const { queryByTestId } = await render(<SearchInput value="text" onChange={() => {}} />);
    expect(queryByTestId("search-input-clear")).toBeNull();
  });
  ...
  it("shows loading indicator when loading", async () => {
    const { getByTestId } = await render(<SearchInput value="" onChange={() => {}} loading />);
    expect(getByTestId("search-input-loading")).toBeDefined();
  });

  it("no loading indicator when not loading", async () => {
    const { queryByTestId } = await render(<SearchInput value="" onChange={() => {}} />);
  ...
  it("custom className", async () => {
    const { getByTestId } = await render(
      <SearchInput value="" onChange={() => {}} className="custom" />,
    );
    expect(getByTestId("search-input-wrapper")).toBeDefined();
  });
  ...
  it("custom testId", async () => {
    const { getByTestId } = await render(
      <SearchInput value="" onChange={() => {}} data-testid="my-search" />,
    );
