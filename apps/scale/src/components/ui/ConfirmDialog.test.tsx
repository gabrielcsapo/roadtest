...
describe('ConfirmDialog', () => {
  ...
  for (const variant of variants) {
    it(`renders variant=${variant} loading=true`, async () => {
      const { getByTestId } = await render(<ConfirmDialog {...defaultProps} variant={variant} loading />)
  ...
  for (const variant of variants) {
    it(`closed dialog: variant=${variant}`, async () => {
      const { queryByTestId } = await render(<ConfirmDialog {...defaultProps} open={false} variant={variant} />)
  ...
  it('calls onConfirm when confirm clicked', async () => {
    ...
    const { getByTestId } = await render(
      <ConfirmDialog {...defaultProps} onConfirm={() => { confirmed = true }} />
    )
  ...
  it('calls onCancel when cancel clicked', async () => {
    let cancelled = false
    const { getByTestId } = await render(
      <ConfirmDialog {...defaultProps} onCancel={() => { cancelled = true }} />
    )
  ...
  it('does not call onConfirm when loading', async () => {
    let confirmed = false
    const { getByTestId } = await render(
      <ConfirmDialog {...defaultProps} loading onConfirm={() => { confirmed = true }} />
    )
    await fireEvent.click(getByTestId('confirm-dialog-confirm'))
  ...
  for (const variant of variants) {
    it(`confirm fires for variant=${variant}`, async () => {
      let called = false
      const { getByTestId } = await render(
        <ConfirmDialog {...defaultProps} variant={variant} onConfirm={() => { called = true }} />
      )
      await fireEvent.click(getByTestId('confirm-dialog-confirm'))
  ...
  for (const variant of variants) {
    it(`cancel fires for variant=${variant}`, async () => {
      let called = false
      const { getByTestId } = await render(
        <ConfirmDialog {...defaultProps} variant={variant} onCancel={() => { called = true }} />
      )
      await fireEvent.click(getByTestId('confirm-dialog-cancel'))
      expect(called).toBe(true)
    })
  ...
  const confirmLabels = ['Delete', 'Remove', 'Archive', 'Proceed', 'Yes, continue', 'Confirm delete', 'Apply', 'Accept', 'Submit', 'Execute']
  for (const label of confirmLabels) {
    it(`confirmLabel="${label}"`, async () => {
      const { getByTestId } = await render(<ConfirmDialog {...defaultProps} confirmLabel={label} />)
      expect(getByTestId('confirm-dialog-confirm')).toBeDefined()
    })
  }
  ...
  for (const msg of messages) {
    it(`message="${msg.substring(0, 40)}"`, async () => {
      const { getByTestId } = await render(
        <ConfirmDialog {...defaultProps} message={msg} />
      )
  ...
  it('confirm button disabled when loading', async () => {
    let count = 0
    const { getByTestId } = await render(
      <ConfirmDialog {...defaultProps} loading onConfirm={() => { count++ }} />
    )
    await fireEvent.click(getByTestId('confirm-dialog-confirm'))
    await fireEvent.click(getByTestId('confirm-dialog-confirm'))
    await fireEvent.click(getByTestId('confirm-dialog-confirm'))
  ...
  for (const variant of variants) {
    it(`loading for variant=${variant} prevents confirm`, async () => {
      let called = false
      const { getByTestId } = await render(
        <ConfirmDialog {...defaultProps} variant={variant} loading onConfirm={() => { called = true }} />
      )
      await fireEvent.click(getByTestId('confirm-dialog-confirm'))
  ...
  const titles = [
    'Delete Vendor', 'Archive Policy', 'Remove User', 'Revoke Access',
    'Reset Password', 'Clear Data', 'Disable Integration', 'Cancel Subscription',
    'Delete Framework', 'Remove Control', 'Archive Issue', 'Resolve Incident',
    'Bulk Delete', 'Confirm Import', 'Reset Settings', 'Terminate Session',
    'Suspend Account', 'Restore Backup', 'Purge Logs', 'Disconnect System',
  ]

  for (const title of titles) {
    it(`title="${title}"`, async () => {
      const { getByTestId } = await render(
        <ConfirmDialog {...defaultProps} title={title} />
      )
      expect(getByTestId('confirm-dialog-title')).toBeDefined()
    })
  }
