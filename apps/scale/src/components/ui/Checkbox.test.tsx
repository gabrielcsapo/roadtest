...
describe('Checkbox', () => {
  ...
  it('renders indeterminate state', async () => {
    const { getByTestId } = await render(<Checkbox checked={false} indeterminate onChange={() => {}} />)
    expect(getByTestId('checkbox')).toBeDefined()
  })

  for (const size of sizes) {
    it(`checked at size=${size}`, async () => {
      const { getByTestId } = await render(<Checkbox checked={true} size={size} onChange={() => {}} />)
      expect(getByTestId('checkbox')).toBeDefined()
    })
  }
  ...
  for (const size of sizes) {
    it(`unchecked at size=${size}`, async () => {
      const { getByTestId } = await render(<Checkbox checked={false} size={size} onChange={() => {}} />)
      expect(getByTestId('checkbox')).toBeDefined()
    })
  }
  ...
  for (const size of sizes) {
    it(`indeterminate at size=${size}`, async () => {
      const { getByTestId } = await render(<Checkbox checked={false} indeterminate size={size} onChange={() => {}} />)
      expect(getByTestId('checkbox')).toBeDefined()
    })
  }

  it('checked + indeterminate', async () => {
    const { getByTestId } = await render(<Checkbox checked={true} indeterminate onChange={() => {}} />)
    expect(getByTestId('checkbox')).toBeDefined()
  })
  ...
  it('does not fire onChange when disabled', async () => {
    let called = false
    const { getByTestId } = await render(<Checkbox checked={false} disabled onChange={() => { called = true }} />)
  ...
  for (const size of sizes) {
    it(`disabled at size=${size}`, async () => {
      const { getByTestId } = await render(<Checkbox checked={false} disabled size={size} onChange={() => {}} />)
      expect(getByTestId('checkbox')).toBeDefined()
    })
  }

  it('disabled + label', async () => {
    const { getByTestId } = await render(<Checkbox checked={false} disabled label="Disabled option" onChange={() => {}} />)
    expect(getByTestId('checkbox-label')).toBeDefined()
  })

  it('disabled + error', async () => {
    const { getByTestId } = await render(<Checkbox checked={false} disabled error="Error" onChange={() => {}} />)
    expect(getByTestId('checkbox-error')).toBeDefined()
  })

  it('disabled + indeterminate', async () => {
    const { getByTestId } = await render(<Checkbox checked={false} disabled indeterminate onChange={() => {}} />)
    expect(getByTestId('checkbox')).toBeDefined()
  })

  it('disabled + checked + label', async () => {
    const { getByTestId } = await render(<Checkbox checked={true} disabled label="Locked" onChange={() => {}} />)
    expect(getByTestId('checkbox-label')).toBeDefined()
  })

  // label variations = 15 tests
  it('renders label when provided', async () => {
    const { getByTestId } = await render(<Checkbox checked={false} label="Accept terms" onChange={() => {}} />)
    expect(getByTestId('checkbox-label')).toBeDefined()
  })
  ...
  for (const label of labelTexts) {
    it(`label="${label.substring(0, 30)}"`, async () => {
      const { getByTestId } = await render(<Checkbox checked={false} label={label} onChange={() => {}} />)
      expect(getByTestId('checkbox-label')).toBeDefined()
    })
  }
  ...
  for (const size of sizes) {
    it(`size=${size} with label`, async () => {
      const { getByTestId } = await render(<Checkbox checked={false} size={size} label="Label" onChange={() => {}} />)
      expect(getByTestId('checkbox-label')).toBeDefined()
    })
  }
  ...
  it('calls onChange with true when checked', async () => {
    let val = false
    const { getByTestId } = await render(<Checkbox checked={false} onChange={(v) => { val = v }} />)
    await fireEvent.click(getByTestId('checkbox'))
  ...
  it('calls onChange when unchecked', async () => {
    let val = true
    const { getByTestId } = await render(<Checkbox checked={true} onChange={(v) => { val = v }} />)
    await fireEvent.click(getByTestId('checkbox'))
  ...
  for (let i = 0; i < 18; i++) {
    it(`onChange test ${i + 1}`, async () => {
      let called = false
      const { getByTestId } = await render(<Checkbox checked={false} onChange={() => { called = true }} />)
  ...
  it('renders error message', async () => {
    const { getByTestId } = await render(<Checkbox checked={false} error="Required" onChange={() => {}} />)
    expect(getByTestId('checkbox-error')).toBeDefined()
  })
  ...
  const errorMsgs = ['This field is required', 'You must accept', 'Please check', 'Invalid selection', 'Must be checked', 'Agreement required', 'Confirmation needed', 'Review required']
  for (const msg of errorMsgs) {
    it(`error="${msg}"`, async () => {
      const { getByTestId } = await render(<Checkbox checked={false} error={msg} onChange={() => {}} />)
      expect(getByTestId('checkbox-error')).toBeDefined()
    })
  }
  ...
  it('label is linked to input via htmlFor', async () => {
    const { getByLabelText } = await render(<Checkbox checked={false} label="Accept" id="accept-cb" onChange={() => {}} />)
    expect(getByLabelText('Accept')).toBeDefined()
  })

  for (let i = 0; i < 11; i++) {
    it(`accessibility test ${i + 1}`, async () => {
      const { getByTestId } = await render(<Checkbox checked={i % 2 === 0} label={`Option ${i}`} onChange={() => {}} />)
      expect(getByTestId('checkbox')).toBeDefined()
    })
  }

  it('custom testId', async () => {
    const { getByTestId } = await render(<Checkbox checked={false} data-testid="my-cb" onChange={() => {}} />)
