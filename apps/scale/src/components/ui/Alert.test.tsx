...
describe("Alert", () => {
  ...
  it("renders custom icon", async () => {
    const { getByTestId } = await render(
      <Alert icon={<span data-testid="custom-icon">★</span>}>Content</Alert>,
    );
  ...
  it("custom icon overrides default", async () => {
    const { getByTestId } = await render(
      <Alert icon={<span data-testid="my-icon">🔒</span>}>Content</Alert>,
    );
  ...
  it("renders icon + title + body + dismiss", async () => {
    const { getByTestId } = await render(
      <Alert variant="danger" title="Error" dismissible icon={<span>!</span>}>
        Something failed
      </Alert>,
    );
    expect(getByTestId("alert-icon")).toBeDefined();
  ...
  for (let i = 0; i < 12; i++) {
    it(`compound content test ${i + 1}`, async () => {
      ...
      const { getByTestId } = await render(
        <Alert variant={variant} title={`Title ${i}`} dismissible>
          Content {i}
        </Alert>,
      );
      expect(getByTestId("alert")).toBeDefined();
  ...
  for (let i = 0; i < 14; i++) {
    it(`accessibility and variant test ${i + 1}`, async () => {
      ...
      const { getByRole } = await render(
        <Alert variant={variant} title={`Alert ${i}`}>
          Message
        </Alert>,
      );
      expect(getByRole("alert")).toBeDefined();
