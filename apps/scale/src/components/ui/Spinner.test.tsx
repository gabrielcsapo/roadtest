...
describe('Spinner', () => {
  ...
  const colors = [
    '#6366f1', '#ef4444', '#f97316', '#fbbf24', '#4ade80',
    '#60a5fa', '#a78bfa', '#f472b6', '#34d399', '#fb7185',
    '#38bdf8', '#818cf8', '#c084fc', '#94a3b8', '#10b981',
    '#e11d48', '#d946ef', '#0ea5e9', '#84cc16', '#f59e0b',
    '#06b6d4', '#8b5cf6', '#ec4899', '#14b8a6', '#64748b',
    '#dc2626', '#7c3aed', '#0369a1', '#15803d', '#b45309',
  ]
  ...
  const labels = [
    'Loading...', 'Please wait', 'Fetching data', 'Processing',
    'Saving changes', 'Running audit', 'Checking compliance',
    'Analyzing risk', 'Generating report', 'Syncing',
    'Uploading', 'Validating', 'Verifying', 'Authenticating',
    'Connecting', 'Updating', 'Refreshing', 'Scanning',
  ]
  ...
  for (let i = 0; i < 9; i++) {
    it(`accessibility test ${i + 1}`, async () => {
      const { getByRole } = await render(<Spinner size={sizes[i % 5]} label={labels[i % labels.length]} />)
      expect(getByRole('status')).toBeDefined()
    })
  }
})
