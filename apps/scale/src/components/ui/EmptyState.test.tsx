...
describe('EmptyState', () => {
  ...
  const actionLabels = [
    'Add vendor', 'Create policy', 'Invite user', 'Upload file',
    'Get started', 'Learn more', 'View docs', 'Configure now',
  ]
  ...
  it('calls action onClick when clicked', async () => {
    let clicked = false
    const { getByTestId } = await render(
      <EmptyState title="Empty" action={{ label: 'Click me', onClick: () => { clicked = true } }} />
    )
  ...
  for (let i = 0; i < 9; i++) {
    it(`action click test ${i + 1}`, async () => {
      let called = false
      const { getByTestId } = await render(
        <EmptyState title={`Empty ${i}`} action={{ label: `Action ${i}`, onClick: () => { called = true } }} />
      )
      await fireEvent.click(getByTestId('empty-state-action'))
  ...
  it('snapshot: with action', async () => {
    await render(<EmptyState title="No policies" action={{ label: 'Create policy', onClick: () => {} }} />)
  ...
  it('title + description + action', async () => {
    const { getByTestId } = await render(
      <EmptyState title="Empty" description="No results" action={{ label: 'Add', onClick: () => {} }} />
    )
    expect(getByTestId('empty-state-title')).toBeDefined()
    expect(getByTestId('empty-state-description')).toBeDefined()
    expect(getByTestId('empty-state-action')).toBeDefined()
  ...
  it('title + icon + action', async () => {
    const { getByTestId } = await render(
      <EmptyState title="Empty" icon={<span>⭐</span>} action={{ label: 'Go', onClick: () => {} }} />
    )
    expect(getByTestId('empty-state-icon')).toBeDefined()
    expect(getByTestId('empty-state-action')).toBeDefined()
  })
