...
describe("Table", () => {
  ...
  it("shows custom empty node", async () => {
    const { getByTestId } = await render(
      <Table
        data={[]}
        columns={columns}
        empty={<span data-testid="custom-empty">No vendors</span>}
      />,
    );
  ...
  for (const msg of emptyMessages) {
    it(`custom empty: "${msg}"`, async () => {
      const { getByTestId } = await render(
        <Table data={[]} columns={columns} empty={<span data-testid="custom-empty">{msg}</span>} />,
      );
      expect(getByTestId("custom-empty")).toBeDefined();
    });
  }
  ...
  it("shows checkboxes when onRowSelect provided", async () => {
    const { getByTestId } = await render(
      <Table data={data} columns={columns} selectedRows={[]} onRowSelect={() => {}} />,
    );
  ...
  it("row is marked selected", async () => {
    const { getByTestId } = await render(
      <Table data={data} columns={columns} selectedRows={["2"]} onRowSelect={() => {}} />,
    );
    ...
    const cols = [
      {
        key: "status",
        label: "Status",
        render: (val: string) => <span data-testid={`status-${val}`}>{val}</span>,
      },
    ];
    ...
    const cols = [
      {
        key: "name",
        label: "Name",
        render: (_: any, row: any) => <span data-testid={`row-${row.id}`}>{row.name}</span>,
      },
    ];
    const { getByTestId } = await render(<Table data={data} columns={cols} />);
      ...
      const cols = [
        {
          key: "name",
          label: "Name",
          render: (val: string) => <b data-testid={`bold-${i}`}>{val}</b>,
        },
      ];
      const { getByTestId } = await render(<Table data={data} columns={cols} />);
  ...
  it("snapshot: with selection", async () => {
    await render(
      <Table data={data} columns={columns} selectedRows={["1"]} onRowSelect={() => {}} />,
    );
