...
describe('Input', () => {
  ...
  const labelTexts = ['Username', 'Email', 'Password', 'Search', 'Phone', 'Company', 'Address', 'Notes']
  ...
  const errorMessages = ['Required field', 'Invalid email', 'Too short', 'Too long', 'Must be a number', 'Invalid format', 'Already taken', 'Not allowed']
  ...
  it('calls onChange on input change', async () => {
    let value = ''
    const { getByTestId } = await render(<Input onChange={(e) => { value = e.target.value }} />)
    await fireEvent.change(getByTestId('input'), { target: { value: 'hello' } })
    expect(value).toBe('hello')
  })

  const inputValues = ['test@example.com', 'password123', 'search term', '42', 'John Doe', 'Company Inc', 'http://example.com', 'some text value', 'another value']
  for (const val of inputValues) {
    it(`handles value="${val}"`, async () => {
      let changed = ''
      const { getByTestId } = await render(<Input onChange={(e) => { changed = e.target.value }} />)
  ...
  it('prefix and suffix together', async () => {
    const { getByTestId } = await render(<Input prefix={<span>@</span>} suffix={<span>.com</span>} />)
    expect(getByTestId('input-prefix')).toBeDefined()
    expect(getByTestId('input-suffix')).toBeDefined()
  })
  ...
  const placeholders = ['Search vendors...', 'Enter email', 'Type password', 'Enter number', 'Find controls']
