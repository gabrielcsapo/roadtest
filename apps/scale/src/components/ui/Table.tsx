...
export function Table<T = any>({
    ...
    return (
      <div style={wrapperStyle} className={className} data-testid={`${testId}-loading`}>
        <div style={{ padding: '40px', textAlign: 'center', color: '#6b6b8a' }}>
          <div data-testid={`${testId}-spinner`} style={{ display: 'inline-block', width: '24px', height: '24px', border: '3px solid #3a3a4a', borderTopColor: '#6366f1', borderRadius: '50%' }} />
          <p>Loading...</p>
        </div>
      </div>
    )
  ...
  return (
    <div style={wrapperStyle} className={className} data-testid={testId}>
      {data.length === 0 ? (
        <div data-testid={`${testId}-empty`} style={{ padding: '40px', textAlign: 'center', color: '#6b6b8a' }}>
            ...
            <tr>
              {onRowSelect && (
                <th style={{ ...thStyle, width: '40px' }}>
                  <input
                    type="checkbox"
                    checked={data.length > 0 && data.every(row => selectedRows.includes(rowKey(row)))}
              ...
              {columns.map(col => (
                ...
                >
                  {col.label}
                  {col.sortable && (
                    <span style={{ marginLeft: '4px', opacity: sortKey === col.key ? 1 : 0.4 }} data-testid={`${testId}-sort-${col.key}`}>
            ...
            {data.map((row: any) => {
              ...
              return (
                ...
                >
                  ...
                  {columns.map(col => (
                    <td key={col.key} style={tdStyle} data-testid={`${testId}-cell-${id}-${col.key}`}>
