...
describe('Select', () => {
  ...
  it('selects option on click', async () => {
    let selected = ''
    const { getByTestId } = await render(<Select options={baseOptions} onChange={(v) => { selected = v as string }} />)
    await fireEvent.click(getByTestId('select-trigger'))
  ...
  it('closes dropdown after selection in single mode', async () => {
    const { getByTestId, queryByTestId } = await render(<Select options={baseOptions} onChange={() => {}} />)
    await fireEvent.click(getByTestId('select-trigger'))
  ...
  it('shows selected value label', async () => {
    const { getByTestId } = await render(<Select options={baseOptions} value="b" onChange={() => {}} />)
    const val = getByTestId('select-value')
    expect(val).toBeDefined()
  })
  ...
  it('does not fire onChange for disabled option', async () => {
    let selected = ''
    const { getByTestId } = await render(<Select options={optionsWithDisabled} onChange={(v) => { selected = v as string }} />)
    await fireEvent.click(getByTestId('select-trigger'))
  ...
  it('shows multiple selected count', async () => {
    const { getByTestId } = await render(<Select options={baseOptions} multiple value={['a', 'b']} onChange={() => {}} />)
    expect(getByTestId('select-value')).toBeDefined()
  })

  it('multiple mode keeps dropdown open after selection', async () => {
    const { getByTestId } = await render(<Select options={baseOptions} multiple onChange={() => {}} />)
    await fireEvent.click(getByTestId('select-trigger'))
    await fireEvent.click(getByTestId('select-option-a'))
    expect(getByTestId('select-dropdown')).toBeDefined()
  })
  ...
  const errorMessages = ['Required', 'Invalid selection', 'Please choose', 'Not available', 'Select at least one', 'Maximum reached', 'No longer valid', 'Option removed', 'Restricted']
  ...
  it('filters options when searching', async () => {
    const { getByTestId, queryByTestId } = await render(<Select options={baseOptions} searchable />)
    await fireEvent.click(getByTestId('select-trigger'))
  ...
  it('multiple: onChange returns array', async () => {
    let result: string[] = []
    const { getByTestId } = await render(<Select options={baseOptions} multiple onChange={(v) => { result = v as string[] }} />)
    await fireEvent.click(getByTestId('select-trigger'))
    await fireEvent.click(getByTestId('select-option-a'))
  ...
  it('multiple: can deselect', async () => {
    let result: string[] = ['a']
    const { getByTestId } = await render(<Select options={baseOptions} multiple value={['a']} onChange={(v) => { result = v as string[] }} />)
    await fireEvent.click(getByTestId('select-trigger'))
    await fireEvent.click(getByTestId('select-option-a'))
  ...
  it('multiple: empty value shows placeholder', async () => {
    const { getByTestId } = await render(<Select options={baseOptions} multiple value={[]} onChange={() => {}} />)
    expect(getByTestId('select-value')).toBeDefined()
  })

  it('multiple: two items shows count', async () => {
    const { getByTestId } = await render(<Select options={baseOptions} multiple value={['a', 'c']} onChange={() => {}} />)
    expect(getByTestId('select-value')).toBeDefined()
  })

  // placeholder variations = 5 tests
  const placeholders = ['Select vendor', 'Choose status', 'Pick framework', 'Assign user', 'Filter by risk']
