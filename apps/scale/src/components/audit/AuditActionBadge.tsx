...
function getActionCategory(action: string): { color: string; label: string } {
  ...
  if (['updated', 'modified', 'changed', 'rotated', 'renewed', 'role_changed', 'edited'].includes(verb)) {
...
export function AuditActionBadge({ action, size = 'md', 'data-testid': testId }: AuditActionBadgeProps) {
