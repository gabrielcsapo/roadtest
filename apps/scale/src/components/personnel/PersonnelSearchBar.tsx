...
export function PersonnelSearchBar({
  ...
  const hasActiveFilters = search.length > 0 || statusFilter.length > 0 || bgCheckFilter.length > 0;
  ...
  return (
    ...
    >
      <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
        ...
        />
        {hasActiveFilters && (
          <Button data-testid="clear-filters-btn" variant="ghost" size="sm" onClick={clearAll}>
            Clear
          </Button>
        )}
      </div>
      ...
      <div data-testid="results-count" style={{ fontSize: "12px", color: "#9ca3af" }}>
        Showing <span data-testid="filtered-count">{filtered}</span> of{" "}
        <span data-testid="total-count">{total}</span> personnel
      </div>
    </div>
  );
}
