...
describe('PersonnelBulkActions', () => {
  ...
  it('renders nothing when selectedCount=0', async () => {
    const { queryByTestId } = await render(
      <PersonnelBulkActions selectedCount={0} />
    )
  ...
  it('renders bar when selectedCount=1', async () => {
    const { getByTestId } = await render(
      <PersonnelBulkActions selectedCount={1} />
    )
  ...
  it('calls onOffboard when offboard button clicked', async () => {
    let called = false
    const { getByTestId } = await render(
      <PersonnelBulkActions selectedCount={3} onOffboard={() => { called = true }} />
    )
  ...
  it('calls onExport when export button clicked', async () => {
    let called = false
    const { getByTestId } = await render(
      <PersonnelBulkActions selectedCount={3} onExport={() => { called = true }} />
    )
  ...
  it('calls onSendReminder when reminder button clicked', async () => {
    let called = false
    const { getByTestId } = await render(
      <PersonnelBulkActions selectedCount={3} onSendReminder={() => { called = true }} />
    )
  ...
  it('does not call handler when disabled', async () => {
    let called = false
    const { getByTestId } = await render(
      <PersonnelBulkActions selectedCount={3} onExport={() => { called = true }} disabled={true} />
    )
    await fireEvent.click(getByTestId('bulk-export-btn'))
  ...
  it('snapshot: all actions available', async () => {
    const { container } = await render(
      <PersonnelBulkActions selectedCount={5} onOffboard={() => {}} onExport={() => {}} onSendReminder={() => {}} />
    )
  ...
  it('snapshot: loading state', async () => {
    const { container } = await render(
      <PersonnelBulkActions selectedCount={5} loading={true} onOffboard={() => {}} onExport={() => {}} />
    )
  ...
  it('snapshot: disabled state', async () => {
    const { container } = await render(
      <PersonnelBulkActions selectedCount={5} disabled={true} onOffboard={() => {}} onExport={() => {}} />
    )
  ...
  it('extra render check 1 - component renders correctly', async () => {
    const { getByTestId } = await render(
      <PersonnelBulkActions selectedCount={5} />
    )
    expect(getByTestId('bulk-actions-bar')).toBeDefined()
  })
