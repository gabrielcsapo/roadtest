...
describe('PersonnelDepartmentFilter', () => {
  ...
  it('renders filter container', async () => {
    const { getByTestId } = await render(
      <PersonnelDepartmentFilter departments={sampleDepartments} value={emptyValue} onChange={() => {}} />
    )
  ...
  for (const dept of sampleDepartments) {
    ...
    it(`department checkbox for ${dept} is checked when in value`, async () => {
      ...
      const { getByTestId } = await render(
        <PersonnelDepartmentFilter departments={sampleDepartments} value={value} onChange={() => {}} />
      )
  ...
  for (const dept of sampleDepartments) {
    ...
    it(`calls onChange removing ${dept} when already selected`, async () => {
      ...
      await fireEvent.change(getByTestId(`dept-checkbox-${deptId}`), { target: { checked: false } })
  ...
  for (const status of statuses) {
    ...
    it(`calls onChange when ${status} toggled`, async () => {
      ...
      await fireEvent.change(getByTestId(`status-checkbox-${status}`), { target: { checked: true } })
  ...
  it('renders with single department', async () => {
    const { getByTestId } = await render(
      <PersonnelDepartmentFilter departments={['Engineering']} value={emptyValue} onChange={() => {}} />
    )
  ...
  it('snapshot: default state', async () => {
    const { container } = await render(
      <PersonnelDepartmentFilter departments={sampleDepartments} value={emptyValue} onChange={() => {}} counts={sampleCounts} />
    )
  ...
  it('snapshot: with selections', async () => {
    ...
    const { container } = await render(
      <PersonnelDepartmentFilter departments={sampleDepartments} value={value} onChange={() => {}} counts={sampleCounts} />
    )
  ...
  it('removes status from selection when untoggled', async () => {
    ...
    const value = { ...emptyValue, status: ['active', 'offboarding'] }
    const { getByTestId } = await render(
      <PersonnelDepartmentFilter departments={sampleDepartments} value={value} onChange={(v) => { changed = v }} />
    )
  ...
  for (const dept of sampleDepartments) {
    ...
    it(`selecting ${dept} passes dept in onChange.departments`, async () => {
      ...
      const { getByTestId } = await render(
        <PersonnelDepartmentFilter departments={sampleDepartments} value={emptyValue} onChange={(v) => { updated = v }} />
      )
      await fireEvent.change(getByTestId(`dept-checkbox-${deptId}`), { target: { checked: true } })
  ...
  for (const status of statuses) {
    ...
    it(`deselecting ${status} when active passes updated status array`, async () => {
      ...
      const value = { ...emptyValue, status: [status] }
      const { getByTestId } = await render(
        <PersonnelDepartmentFilter departments={sampleDepartments} value={value} onChange={(v) => { updated = v }} />
      )
      await fireEvent.change(getByTestId(`status-checkbox-${status}`), { target: { checked: false } })
