...
describe('PersonnelDetails', () => {
  ...
  it('shows loading spinner when loading=true', async () => {
    const { getByTestId } = await render(<PersonnelDetails person={mockPersonnel} loading={true} />)
    expect(getByTestId('personnel-details-loading')).toBeDefined()
  })

  it('hides main content when loading', async () => {
    const { queryByTestId } = await render(<PersonnelDetails person={mockPersonnel} loading={true} />)
  ...
  it('loading=false shows details', async () => {
    const { getByTestId } = await render(<PersonnelDetails person={mockPersonnel} loading={false} />)
    expect(getByTestId('personnel-details')).toBeDefined()
  })
  ...
  for (const status of statuses) {
    ...
    it(`shows status badge for status=${status}`, async () => {
      ...
      expect(container.querySelectorAll('[data-testid="personnel-status-badge"]').length).toBeGreaterThan(0)
    })
  ...
  for (const bgStatus of bgStatuses) {
    it(`renders background check badge for ${bgStatus}`, async () => {
      ...
      const { container } = await render(<PersonnelDetails person={person} />)
      expect(container.querySelectorAll('[data-testid="background-check-badge"]').length).toBeGreaterThan(0)
    })
  ...
  it('shows edit button when onEdit provided', async () => {
    const { getByTestId } = await render(<PersonnelDetails person={mockPersonnel} onEdit={() => {}} />)
  ...
  it('shows offboard button when onOffboard provided and status is active', async () => {
    const { getByTestId } = await render(<PersonnelDetails person={mockPersonnel} onOffboard={() => {}} />)
    expect(getByTestId('details-offboard-btn')).toBeDefined()
  })
  ...
  it('calls onEdit when edit button clicked', async () => {
    ...
    const { getByTestId } = await render(
      <PersonnelDetails person={mockPersonnel} onEdit={(p) => { edited = p }} />
    )
  ...
  it('calls onOffboard when offboard button clicked', async () => {
    ...
    const { getByTestId } = await render(
      <PersonnelDetails person={mockPersonnel} onOffboard={(p) => { offboarded = p }} />
    )
  ...
  it('extra render check 1 - component renders correctly', async () => {
    const { getByTestId } = await render(
      <PersonnelDetails person={mockPersonnel} />
    )
    expect(getByTestId('personnel-details')).toBeDefined()
  })
