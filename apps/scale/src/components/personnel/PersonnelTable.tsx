...
export function PersonnelTable({
  ...
  if (loading) {
    return (
      <div data-testid="personnel-table-loading" style={{ display: 'flex', justifyContent: 'center', padding: '48px' }}>
        <Spinner />
      </div>
    )
  }
        ...
        <thead>
          <tr style={{ borderBottom: '2px solid #e5e7eb', background: '#f9fafb' }}>
            ...
            <th
              ...
              style={{ padding: '12px 16px', textAlign: 'left', cursor: 'pointer', userSelect: 'none', fontSize: '13px', fontWeight: 600, color: '#374151' }}
          ...
          {people.map((person) => {
            ...
            return (
              ...
              >
                ...
                <td style={{ padding: '12px 16px' }}>
                  <div data-testid={`personnel-name-${person.id}`} style={{ fontWeight: 500, color: '#111827' }}>
                    {person.name}
                  </div>
                  <div style={{ fontSize: '12px', color: '#6b7280' }}>{person.email}</div>
                </td>
                <td data-testid={`personnel-dept-${person.id}`} style={{ padding: '12px 16px', color: '#374151' }}>
                  {person.department}
                </td>
