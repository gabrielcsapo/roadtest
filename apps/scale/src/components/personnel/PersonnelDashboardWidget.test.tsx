...
describe('PersonnelDashboardWidget', () => {
  ...
  it('shows main widget when not loading', async () => {
    const { getByTestId } = await render(
      <PersonnelDashboardWidget people={personnelList} />
    )
  ...
  it('total=3 shows 3', async () => {
    const { getByTestId } = await render(<PersonnelDashboardWidget people={personnelList.slice(0, 3)} />)
  ...
  it('calls onClick with "all" when total stat clicked', async () => {
    let view = ''
    const { getByTestId } = await render(
      <PersonnelDashboardWidget people={personnelList} onClick={(v) => { view = v }} />
    )
  ...
  it('snapshot: loading', async () => {
    const { container } = await render(<PersonnelDashboardWidget people={personnelList} loading={true} />)
    await snapshot('personnel-widget-loading')
  })

  it('snapshot: all active no issues', async () => {
    const people = personnelList.filter((p) => p.status === 'active' && p.backgroundCheckStatus === 'passed')
