...
describe('PersonnelSearchBar', () => {
  ...
  it('shows total count', async () => {
    const { getByTestId } = await render(<PersonnelSearchBar {...defaultProps} total={42} filtered={42} />)
    expect(getByTestId('total-count').textContent).toBe('42')
  })

  it('shows filtered count', async () => {
    const { getByTestId } = await render(<PersonnelSearchBar {...defaultProps} total={100} filtered={25} />)
  ...
  it('calls onSearch when input changes', async () => {
    ...
    const { getByTestId } = await render(
      <PersonnelSearchBar {...defaultProps} onSearch={(v) => { searched = v }} />
    )
  ...
  it('calls onSearch with empty string when cleared', async () => {
    ...
    const { getByTestId } = await render(
      <PersonnelSearchBar {...defaultProps} search="Bob" onSearch={(v) => { searched = v }} />
    )
  ...
  it('search input has correct placeholder', async () => {
    const { getByTestId } = await render(<PersonnelSearchBar {...defaultProps} />)
    expect((getByTestId('search-input') as HTMLInputElement).placeholder).toBe('Search personnel...')
  })
  ...
  for (const status of statuses) {
    ...
    it(`clicking ${status} filter calls onStatusFilter with ${status}`, async () => {
      let filtered: string[] = []
      const { getByTestId } = await render(
        <PersonnelSearchBar {...defaultProps} onStatusFilter={(s) => { filtered = s }} />
      )
  ...
  for (const bgStatus of bgStatuses) {
    ...
    it(`clicking ${bgStatus} bgcheck filter calls onBgCheckFilter`, async () => {
      let filtered: string[] = []
      const { getByTestId } = await render(
        <PersonnelSearchBar {...defaultProps} onBgCheckFilter={(s) => { filtered = s }} />
      )
  ...
  it('shows clear button when status filter active', async () => {
    const { getByTestId } = await render(<PersonnelSearchBar {...defaultProps} statusFilter={['active']} />)
    expect(getByTestId('clear-filters-btn')).toBeDefined()
  })

  it('shows clear button when bgcheck filter active', async () => {
    const { getByTestId } = await render(<PersonnelSearchBar {...defaultProps} bgCheckFilter={['passed']} />)
    expect(getByTestId('clear-filters-btn')).toBeDefined()
  })
  ...
  it('shows correct total when total=0', async () => {
    const { getByTestId } = await render(<PersonnelSearchBar {...defaultProps} total={0} filtered={0} />)
    expect(getByTestId('total-count').textContent).toBe('0')
  })

  it('shows correct filtered count=0', async () => {
    const { getByTestId } = await render(<PersonnelSearchBar {...defaultProps} total={50} filtered={0} />)
    expect(getByTestId('filtered-count').textContent).toBe('0')
  })

  it('shows correct total=1000', async () => {
    const { getByTestId } = await render(<PersonnelSearchBar {...defaultProps} total={1000} filtered={999} />)
    expect(getByTestId('total-count').textContent).toBe('1000')
  })

  it('results count text contains "of"', async () => {
    const { getByTestId } = await render(<PersonnelSearchBar {...defaultProps} total={50} filtered={10} />)
  ...
  const filterCombos = [
    ...
    { status: ['active', 'offboarding', 'offboarded'], bg: ['pending', 'passed', 'failed', 'not-required'] },
  ]
  ...
  const countPairs = [[0,0],[1,1],[10,5],[100,50],[1000,999],[500,0],[25,25],[7,3],[99,88],[42,42]]
  ...
  it('extra render check 1 - component renders correctly', async () => {
    const { getByTestId } = await render(
      <PersonnelSearchBar {...defaultProps} />
    )
    expect(getByTestId('personnel-search-bar')).toBeDefined()
  })
