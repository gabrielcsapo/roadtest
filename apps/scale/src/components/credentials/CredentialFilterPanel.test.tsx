...
describe('CredentialFilterPanel', () => {
  ...
  for (const type of types) {
    ...
    it(`toggling ${type} calls onChange with type added`, async () => {
      ...
      const { getByTestId } = await render(
        <CredentialFilterPanel value={emptyValue} onChange={(v) => { changed = v }} services={[]} />
      )
    ...
    it(`toggling active ${type} calls onChange with type removed`, async () => {
      ...
      const value = { ...emptyValue, types: [type] }
      const { getByTestId } = await render(
        <CredentialFilterPanel value={value} onChange={(v) => { changed = v }} services={[]} />
      )
  ...
  for (const service of sampleServices) {
    ...
    it(`toggling ${service} calls onChange with service added`, async () => {
      let changed: any = null
      const { getByTestId } = await render(
        <CredentialFilterPanel value={emptyValue} onChange={(v) => { changed = v }} services={sampleServices} />
      )
      await fireEvent.change(getByTestId(`service-checkbox-${svcId}`), { target: { checked: true } })
  ...
  it('clear-all resets all filters', async () => {
    ...
    const { getByTestId } = await render(
      <CredentialFilterPanel value={value} onChange={(v) => { changed = v }} services={sampleServices} />
    )
  ...
  for (const type of types) {
    it(`shows type count for ${type} when counts provided`, async () => {
      const { getByTestId } = await render(
        <CredentialFilterPanel value={emptyValue} onChange={() => {}} services={[]} counts={sampleCounts} />
      )
  ...
  it('snapshot: empty filters', async () => {
    const { container } = await render(
      <CredentialFilterPanel value={emptyValue} onChange={() => {}} services={sampleServices} counts={sampleCounts} />
    )
  ...
  it('snapshot: with selections', async () => {
    const value = { types: ['api-key'], statuses: ['valid'], services: ['AWS'] }
    const { container } = await render(
      <CredentialFilterPanel value={value} onChange={() => {}} services={sampleServices} counts={sampleCounts} />
    )
  ...
  for (const service of sampleServices) {
    ...
    it(`toggling active ${service} removes it`, async () => {
      ...
      await fireEvent.change(getByTestId(`service-checkbox-${svcId}`), { target: { checked: false } })
