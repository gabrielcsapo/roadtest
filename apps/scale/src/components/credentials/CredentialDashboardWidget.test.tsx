...
describe("CredentialDashboardWidget", () => {
  ...
  it("shows loading when loading=true", async () => {
    const { getByTestId } = await render(
      <CredentialDashboardWidget credentials={credentialList} loading={true} />,
    );
  ...
  it("loading=false shows widget", async () => {
    const { getByTestId } = await render(
      <CredentialDashboardWidget credentials={credentialList} loading={false} />,
    );
    expect(getByTestId("credential-dashboard-widget")).toBeDefined();
  });
  ...
  it("total=3 shows 3", async () => {
    const { getByTestId } = await render(
      <CredentialDashboardWidget credentials={credentialList.slice(0, 3)} />,
    );
