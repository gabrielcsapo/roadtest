...
const credentialList: Credential[] = [
  ...
  { ...mockCred, id: 'c2', type: 'certificate', status: 'expiring-soon', expiresAt: '2024-04-15T00:00:00Z', name: 'SSL Certificate', service: 'CloudFlare' },
  { ...mockCred, id: 'c3', type: 'password', status: 'expired', expiresAt: '2024-01-01T00:00:00Z', name: 'DB Password', service: 'PostgreSQL' },
  { ...mockCred, id: 'c4', type: 'oauth-token', status: 'valid', expiresAt: null, name: 'GitHub OAuth' },
  { ...mockCred, id: 'c5', type: 'certificate', status: 'expiring-soon', expiresAt: '2024-05-01T00:00:00Z', name: 'Client Cert', service: 'Internal' },
]
...
const fiveExpiring = [
  { ...mockCred, id: 'e1', status: 'expiring-soon' as const, name: 'Cred 1', service: 'S1', expiresAt: '2024-04-10T00:00:00Z' },
  { ...mockCred, id: 'e2', status: 'expiring-soon' as const, name: 'Cred 2', service: 'S2', expiresAt: '2024-04-11T00:00:00Z' },
  { ...mockCred, id: 'e3', status: 'expiring-soon' as const, name: 'Cred 3', service: 'S3', expiresAt: '2024-04-12T00:00:00Z' },
  { ...mockCred, id: 'e4', status: 'expiring-soon' as const, name: 'Cred 4', service: 'S4', expiresAt: '2024-04-13T00:00:00Z' },
  { ...mockCred, id: 'e5', status: 'expiring-soon' as const, name: 'Cred 5', service: 'S5', expiresAt: '2024-04-14T00:00:00Z' },
]
...
describe('CredentialExpiryWarning', () => {
  ...
  it('calls onRotate with credential id when rotate clicked for expiring', async () => {
    let rotatedId = ''
    const { getByTestId } = await render(
      <CredentialExpiryWarning credentials={oneExpiring} onRotate={(id) => { rotatedId = id }} />
    )
  ...
  it('calls onRotate with credential id for expired', async () => {
    let rotatedId = ''
    const { getByTestId } = await render(
      <CredentialExpiryWarning credentials={oneExpired} onRotate={(id) => { rotatedId = id }} />
    )
  ...
  it('calls onDismiss when dismiss button clicked for expiring', async () => {
    let dismissed = false
    const { getByTestId } = await render(
      <CredentialExpiryWarning credentials={oneExpiring} onDismiss={() => { dismissed = true }} />
    )
  ...
  it('calls onDismiss when dismiss button clicked for expired', async () => {
    let dismissed = false
    const { getByTestId } = await render(
      <CredentialExpiryWarning credentials={oneExpired} onDismiss={() => { dismissed = true }} />
    )
  ...
  for (const creds of expiringSets) {
    ...
    it(`shows correct count in expiring alert for ${creds.length}`, async () => {
      ...
      if (creds.length === 1) {
        ...
        expect(getByTestId('expiring-alert').textContent).toContain(`${creds.length} credentials are expiring soon`)
      }
    })
  ...
  for (const creds of expiredSets) {
    ...
    it(`shows correct count in expired alert for ${creds.length}`, async () => {
      ...
      if (creds.length === 1) {
        ...
        expect(getByTestId('expired-alert').textContent).toContain(`${creds.length} credentials have expired`)
      }
    })
  ...
  it('mixed set rotate button works for expiring item', async () => {
    let rotatedId = ''
    const { getByTestId } = await render(
      <CredentialExpiryWarning credentials={mixed} onRotate={(id) => { rotatedId = id }} />
    )
    await fireEvent.click(getByTestId('rotate-expiring-c2'))
    expect(rotatedId).toBe('c2')
  })
  ...
  it('extra render check 1 - component renders correctly', async () => {
    const { getByTestId } = await render(
      <CredentialExpiryWarning credentials={oneExpiring} />
    )
    expect(getByTestId('expiry-warning')).toBeDefined()
  })
