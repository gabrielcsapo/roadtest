...
export function CredentialTable({
  ...
  const allSelected = credentials.length > 0 && credentials.every((c) => selectedIds.includes(c.id))

  if (loading) {
    return (
      <div data-testid="credential-table-loading" style={{ display: 'flex', justifyContent: 'center', padding: '48px' }}>
        <Spinner />
      </div>
    )
  }
        ...
        <thead>
          <tr style={{ borderBottom: '2px solid #e5e7eb', background: '#f9fafb' }}>
            ...
            <th
              ...
              style={{ padding: '12px 16px', textAlign: 'left', cursor: 'pointer', fontSize: '13px', fontWeight: 600, color: '#374151' }}
          ...
          {credentials.map((cred) => {
            ...
            return (
              ...
              >
                ...
                <td style={{ padding: '12px 16px' }}>
                  <div data-testid={`cred-name-${cred.id}`} style={{ fontWeight: 500, color: '#111827' }}>
                    {cred.name}
                  </div>
                </td>
                <td style={{ padding: '12px 16px' }}>
                ...
                <td style={{ padding: '12px 16px' }}>
                  <CredentialStatusBadge status={cred.status} expiresAt={cred.expiresAt} showDaysRemaining={false} />
                </td>
                <td
