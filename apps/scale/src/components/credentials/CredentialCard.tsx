...
export function CredentialCard({
  ...
  const handleRotate = (e: React.MouseEvent) => { e.stopPropagation(); onRotate?.(credential) }
  const handleDelete = (e: React.MouseEvent) => { e.stopPropagation(); onDelete?.(credential) }
  const handleEdit = (e: React.MouseEvent) => { e.stopPropagation(); onEdit?.(credential) }
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
        <CredentialStatusBadge status={credential.status} expiresAt={credential.expiresAt} showDaysRemaining={true} />
      </div>
      ...
      {(onRotate || onDelete || onEdit) && (
        ...
        >
          {onEdit && (
            <Button data-testid="credential-edit-btn" variant="ghost" size="sm" onClick={handleEdit}>
              Edit
            </Button>
          )}
          {onRotate && (
            <Button data-testid="credential-rotate-btn" variant="secondary" size="sm" onClick={handleRotate}>
              Rotate
            </Button>
          )}
          {onDelete && (
            <Button data-testid="credential-delete-btn" variant="danger" size="sm" onClick={handleDelete}>
              Delete
            </Button>
          )}
        </div>
