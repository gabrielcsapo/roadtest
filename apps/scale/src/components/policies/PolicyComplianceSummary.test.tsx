...
describe('PolicyComplianceSummary', () => {
  ...
  it('hides loading state when loading is false', async () => {
    const { queryByTestId } = await render(<PolicyComplianceSummary policies={policies} loading={false} />)
  ...
  allStatuses.map((status) =>
    it(`calls onFilterByStatus with ${status} when that card is clicked`, async () => {
      ...
      const { getByTestId } = await render(
        <PolicyComplianceSummary policies={policies} onFilterByStatus={(s) => { clicked = s }} />
      )
  ...
  it('handles all policies being non-compliant', async () => {
    const allNonCompliant = policies.map((p) => ({ ...p, status: 'non-compliant' as ComplianceStatus }))
  ...
  it('counts all-in-progress correctly', async () => {
    const allInProgress = policies.map((p) => ({ ...p, status: 'in-progress' as ComplianceStatus }))
  ...
  allStatuses.map((status) =>
    it(`clicking ${status} card passes the right status`, async () => {
      ...
      const { getByTestId } = await render(
        <PolicyComplianceSummary policies={policies} onFilterByStatus={(s) => { received = s }} />
      )
      await fireEvent.click(getByTestId(`status-card-${status}`))
