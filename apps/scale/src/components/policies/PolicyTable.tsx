...
export function PolicyTable({ policies, loading, onSort, onEdit, selectedIds = [], onSelect }: PolicyTableProps) {
  ...
  const allSelected = filteredPolicies.length > 0 && filteredPolicies.every((p) => selectedIds.includes(p.id))

  if (loading) {
    return (
      <div data-testid="policy-table-loading" style={{ display: 'flex', justifyContent: 'center', padding: '40px' }}>
  ...
  return (
    <div data-testid="policy-table-container">
      <div data-testid="policy-table-filters" style={{ display: 'flex', gap: '12px', marginBottom: '16px' }}>
        <Select
      ...
      {filteredPolicies.length === 0 ? (
        <EmptyState data-testid="policy-table-empty" title="No policies found" description="Try adjusting your filters." />
      ) : (
      ...
      <div data-testid="policy-table-footer" style={{ marginTop: '8px', fontSize: '0.875rem', color: '#6b7280' }}>
