...
export function PolicyCard({ policy, onClick, onEdit, onAssign }: PolicyCardProps) {
  ...
  return (
    ...
    >
      <div
        data-testid="policy-card-header"
      ...
      >
        <div>
          ...
          <p
            ...
            style={{ margin: "4px 0", color: "#6b7280", fontSize: "0.875rem" }}
          >
            {policy.description}
          </p>
        </div>
        <Badge data-testid="policy-status-badge" color={statusColors[policy.status] || "gray"}>
          {statusLabels[policy.status] || policy.status}
        </Badge>
      </div>

      <div
        data-testid="policy-frameworks"
        style={{ display: "flex", gap: "4px", flexWrap: "wrap", margin: "8px 0" }}
      >
        {policy.frameworks.map((fw) => (
          <PolicyFrameworkBadge
            key={fw}
            framework={fw}
            size="sm"
            data-testid={`framework-badge-${fw}`}
          />
        ))}
        {policy.frameworks.length === 0 && (
          <span data-testid="no-frameworks" style={{ color: "#9ca3af", fontSize: "0.75rem" }}>
            No frameworks
          </span>
        )}
      </div>
      ...
      {(onEdit || onAssign) && (
        ...
        >
          {onEdit && (
            <Button data-testid="edit-button" variant="secondary" size="sm" onClick={handleEdit}>
              Edit
            </Button>
          )}
          {onAssign && (
