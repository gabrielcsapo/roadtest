...
export function IssueCard({ issue, onClick, onAssign, onResolve, onDismiss }: IssueCardProps) {
  ...
  return (
    ...
    >
      <div data-testid="issue-card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
        <div style={{ flex: 1 }}>
          ...
          <h3 data-testid="issue-title" style={{ margin: 0, fontSize: '0.9375rem', fontWeight: 600 }}>
            {issue.title}
          </h3>
          <p data-testid="issue-description" style={{ margin: '4px 0 0', color: '#6b7280', fontSize: '0.875rem' }}>
            {issue.description}
          </p>
        </div>
      </div>

      <div data-testid="issue-meta" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '12px' }}>
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          {issue.assignee ? (
            <div data-testid="issue-assignee" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <Avatar
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          {issue.dueDate ? (
            <span
              ...
              style={{ fontSize: '0.75rem', color: overdue ? '#ef4444' : '#6b7280', fontWeight: overdue ? 600 : 400 }}
            >
      ...
      {(onAssign || onResolve || onDismiss) && (
        <div data-testid="issue-actions" style={{ display: 'flex', gap: '8px', marginTop: '12px', borderTop: '1px solid #e5e7eb', paddingTop: '12px' }}>
          {onAssign && (
            <Button data-testid="assign-button" variant="secondary" size="sm" onClick={handleAssign}>
              Assign
            </Button>
          )}
          {onResolve && issue.status !== 'resolved' && (
            <Button data-testid="resolve-button" variant="primary" size="sm" onClick={handleResolve}>
              Resolve
            </Button>
          )}
          {onDismiss && (
            <Button data-testid="dismiss-button" variant="secondary" size="sm" onClick={handleDismiss}>
              Dismiss
            </Button>
          )}
        </div>
