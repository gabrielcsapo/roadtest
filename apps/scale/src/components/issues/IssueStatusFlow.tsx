  ...
  return (
    <div data-testid="issue-status-flow" data-current-status={issue.status}>
      ...
      {availableTransitions.length > 0 && (
        <div data-testid="transition-actions" style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          <span style={{ fontSize: '0.75rem', color: '#6b7280', width: '100%', marginBottom: '4px' }}>
            Transition to:
          </span>
      ...
      {loading && (
        <div data-testid="status-flow-loading" style={{ marginTop: '8px', fontSize: '0.75rem', color: '#6b7280' }}>
          Updating status...
        </div>
      )}

      <div data-testid="status-flow-history" style={{ marginTop: '16px', fontSize: '0.75rem', color: '#9ca3af' }}>
