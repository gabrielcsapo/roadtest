...
describe('IssueStatusFlow', () => {
  ...
  allIssues.forEach((issue) =>
    it(`renders status flow for ${issue.status} issue`, async () => {
      const { getByTestId } = await render(<IssueStatusFlow issue={issue} onTransition={() => {}} />)
  ...
  it('shows Open label for open status', async () => {
    const { getByTestId } = await render(<IssueStatusFlow issue={openIssue} onTransition={() => {}} />)
  ...
  it('shows In Progress label for in-progress status', async () => {
    const { getByTestId } = await render(<IssueStatusFlow issue={inProgressIssue} onTransition={() => {}} />)
  ...
  it('shows Resolved label for resolved status', async () => {
    const { getByTestId } = await render(<IssueStatusFlow issue={resolvedIssue} onTransition={() => {}} />)
    expect(getByTestId('current-status-badge').textContent).toContain('Resolved')
  })

  it("shows Won't Fix label for wont-fix status", async () => {
    const { getByTestId } = await render(<IssueStatusFlow issue={wontFixIssue} onTransition={() => {}} />)
  ...
  allIssues.forEach((issue) =>
    it(`has correct data-current-status for ${issue.status}`, async () => {
      const { getByTestId } = await render(<IssueStatusFlow issue={issue} onTransition={() => {}} />)
      expect(getByTestId('issue-status-flow').getAttribute('data-current-status')).toBe(issue.status)
    })
  )
  ...
  it('does not show resolved transition from open status', async () => {
    const { queryByTestId } = await render(<IssueStatusFlow issue={openIssue} onTransition={() => {}} />)
  ...
  it('does not show wont-fix transition from in-progress', async () => {
    const { queryByTestId } = await render(<IssueStatusFlow issue={inProgressIssue} onTransition={() => {}} />)
  ...
  it('does not show in-progress transition from resolved', async () => {
    const { queryByTestId } = await render(<IssueStatusFlow issue={resolvedIssue} onTransition={() => {}} />)
  ...
  it('does not show resolved transition from wont-fix', async () => {
    const { queryByTestId } = await render(<IssueStatusFlow issue={wontFixIssue} onTransition={() => {}} />)
    expect(queryByTestId('transition-to-resolved')).toBeNull()
  })
  ...
  it('calls onTransition with in-progress when in-progress button clicked from open', async () => {
    ...
    const { getByTestId } = await render(
      <IssueStatusFlow issue={openIssue} onTransition={(s) => { transitioned = s }} />
    )
  ...
  it('calls onTransition with resolved when resolved button clicked from in-progress', async () => {
    let transitioned = ''
    const { getByTestId } = await render(
      <IssueStatusFlow issue={inProgressIssue} onTransition={(s) => { transitioned = s }} />
    )
  ...
  it('calls onTransition with open when open button clicked from resolved', async () => {
    let transitioned = ''
    const { getByTestId } = await render(
      <IssueStatusFlow issue={resolvedIssue} onTransition={(s) => { transitioned = s }} />
    )
    await fireEvent.click(getByTestId('transition-to-open'))
    expect(transitioned).toBe('open')
  })
  ...
  it('calls onTransition with open when open button clicked from wont-fix', async () => {
    let transitioned = ''
    const { getByTestId } = await render(
      <IssueStatusFlow issue={wontFixIssue} onTransition={(s) => { transitioned = s }} />
    )
    await fireEvent.click(getByTestId('transition-to-open'))
    expect(transitioned).toBe('open')
  })
  ...
  it('shows loading indicator when loading is true', async () => {
    const { getByTestId } = await render(<IssueStatusFlow issue={openIssue} onTransition={() => {}} loading />)
    expect(getByTestId('status-flow-loading')).toBeDefined()
  })

  it('hides loading indicator when loading is false', async () => {
    const { queryByTestId } = await render(<IssueStatusFlow issue={openIssue} onTransition={() => {}} loading={false} />)
  ...
  it('enables transition buttons when not loading', async () => {
    const { getByTestId } = await render(<IssueStatusFlow issue={openIssue} onTransition={() => {}} loading={false} />)
    const btn = getByTestId('transition-to-in-progress')
  ...
  it('matches snapshot for open status', async () => {
    const { container } = await render(<IssueStatusFlow issue={openIssue} onTransition={() => {}} />)
    await snapshot('issue-status-flow-open')
  })

  it('matches snapshot for in-progress status', async () => {
    const { container } = await render(<IssueStatusFlow issue={inProgressIssue} onTransition={() => {}} />)
    await snapshot('issue-status-flow-in-progress')
  })

  it('matches snapshot for resolved status', async () => {
    const { container } = await render(<IssueStatusFlow issue={resolvedIssue} onTransition={() => {}} />)
    await snapshot('issue-status-flow-resolved')
  })

  it('matches snapshot for wont-fix status', async () => {
    const { container } = await render(<IssueStatusFlow issue={wontFixIssue} onTransition={() => {}} />)
