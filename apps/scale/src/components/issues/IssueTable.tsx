...
export function IssueTable({ issues, loading, onSort, onResolve, onDismiss, selectedIds = [], onSelect }: IssueTableProps) {
  ...
  const allSelected = filteredIssues.length > 0 && filteredIssues.every((i) => selectedIds.includes(i.id))

  if (loading) {
    return (
      <div data-testid="issue-table-loading" style={{ display: 'flex', justifyContent: 'center', padding: '40px' }}>
  ...
  return (
    <div data-testid="issue-table-container">
      <div data-testid="issue-table-filters" style={{ display: 'flex', gap: '12px', marginBottom: '16px' }}>
        <Select
          ...
          options={[
            ...
            ...allSeverities.map((s) => ({ value: s, label: s.charAt(0).toUpperCase() + s.slice(1) })),
          ]}
        />
        <Select
      ...
      {filteredIssues.length === 0 ? (
        <EmptyState data-testid="issue-table-empty" title="No issues found" description="Try adjusting your filters." />
      ) : (
        <Table data-testid="issue-table">
          <thead>
            <tr data-testid="issue-table-header">
              ...
              <th data-testid="sort-title" onClick={() => handleSort('title')} style={{ cursor: 'pointer' }}>
                Title {sortField === 'title' && (sortDirection === 'asc' ? '↑' : '↓')}
              </th>
              <th data-testid="sort-severity" onClick={() => handleSort('severity')} style={{ cursor: 'pointer' }}>
            ...
            {filteredIssues.map((issue) => {
              ...
              return (
                ...
                >
                  ...
                  <td data-testid={`issue-assignee-${issue.id}`}>{issue.assignee?.name ?? 'Unassigned'}</td>
                  <td
      ...
      <div data-testid="issue-table-footer" style={{ marginTop: '8px', fontSize: '0.875rem', color: '#6b7280' }}>
