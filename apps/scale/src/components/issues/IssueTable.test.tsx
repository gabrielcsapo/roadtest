...
describe('IssueTable', () => {
  ...
  it('calls onSort when title header is clicked', async () => {
    let sortField = ''
    const { getByTestId } = await render(<IssueTable issues={issues} onSort={(f) => { sortField = f }} />)
  ...
  it('toggles sort direction asc to desc', async () => {
    let dir = ''
    const { getByTestId } = await render(<IssueTable issues={issues} onSort={(_, d) => { dir = d }} />)
    await fireEvent.click(getByTestId('sort-title'))
    await fireEvent.click(getByTestId('sort-title'))
  ...
  it('shows select-all checkbox when onSelect provided', async () => {
    const { getByTestId } = await render(<IssueTable issues={issues} onSelect={() => {}} selectedIds={[]} />)
  ...
  it('calls onSelect with all ids when select-all is clicked', async () => {
    ...
    const { getByTestId } = await render(
      <IssueTable issues={issues} onSelect={(ids) => { selected = ids }} selectedIds={[]} />
    )
  ...
  it('deselects all when select-all is unchecked', async () => {
    ...
    const { getByTestId } = await render(
      <IssueTable issues={issues} onSelect={(ids) => { selected = ids }} selectedIds={['i1', 'i2', 'i3', 'i4', 'i5']} />
    )
    await fireEvent.click(getByTestId('select-all-checkbox'))
  ...
  it('does not show resolve button for resolved issues', async () => {
    const { queryByTestId } = await render(<IssueTable issues={[issues[2]]} onResolve={() => {}} />)
    expect(queryByTestId('resolve-button-i3')).toBeNull()
  })

  it('calls onResolve with correct issue', async () => {
    let resolved: Issue | null = null
    const { getByTestId } = await render(<IssueTable issues={[issues[0]]} onResolve={(i) => { resolved = i }} />)
  ...
  it('calls onDismiss with correct issue', async () => {
    ...
    const { getByTestId } = await render(<IssueTable issues={[issues[0]]} onDismiss={(i) => { dismissed = i }} />)
  ...
  it('marks non-overdue rows with data-overdue false', async () => {
    const { getByTestId } = await render(<IssueTable issues={[{ ...mockIssue, id: 'future', dueDate: '2099-01-01' }]} />)
