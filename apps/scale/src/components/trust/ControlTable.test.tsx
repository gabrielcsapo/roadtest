...
describe('ControlTable', () => {
  ...
  allFrameworks.map((fw) =>
    it(`calls onFrameworkFilter with ${fw} when tab is clicked`, async () => {
      ...
      const { getByTestId } = await render(
        <ControlTable controls={controls} onFrameworkFilter={(f) => { filtered = f }} />
      )
  ...
  it('calls onFrameworkFilter with undefined when All tab is clicked', async () => {
    ...
    const { getByTestId } = await render(
      <ControlTable controls={controls} onFrameworkFilter={(f) => { filtered = f ?? null }} />
    )
  ...
  it('highlights active framework tab based on frameworkFilter prop', async () => {
    const { getByTestId } = await render(<ControlTable controls={controls} frameworkFilter="SOC2" />)
  ...
  it('calls onSort when name header is clicked', async () => {
    let sortField = ''
    const { getByTestId } = await render(<ControlTable controls={controls} onSort={(f) => { sortField = f }} />)
  ...
  it('toggles sort direction on second click', async () => {
    let sortDir = ''
    const { getByTestId } = await render(<ControlTable controls={controls} onSort={(_, d) => { sortDir = d }} />)
    await fireEvent.click(getByTestId('sort-name'))
    await fireEvent.click(getByTestId('sort-name'))
  ...
  controls.map((c) =>
    it(`calls onEdit with correct control for ${c.id}`, async () => {
      ...
      const { getByTestId } = await render(<ControlTable controls={controls} onEdit={(ctrl) => { edited = ctrl }} />)
