...
export function TrustCenterScore({
  controls,
  showBreakdown = false,
  framework,
  loading,
}: TrustCenterScoreProps) {
  if (loading) {
    return (
      <div
  ...
  return (
    <div data-testid="trust-score-container" data-score={score}>
      ...
      <div
        data-testid="trust-score-summary"
        style={{
          textAlign: "center",
          marginBottom: "16px",
          color: "#6b7280",
      ...
      >
        {filteredControls.filter((c) => c.status === "compliant").length} of{" "}
        {filteredControls.length} controls compliant
