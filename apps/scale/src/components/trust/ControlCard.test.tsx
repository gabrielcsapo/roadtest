...
describe('ControlCard', () => {
  ...
  it('shows Compliant label', async () => {
    const { getByTestId } = await render(<ControlCard control={{ ...mockControl, status: 'compliant' }} />)
    expect(getByTestId('control-status-badge').textContent).toContain('Compliant')
  })

  it('shows Non-Compliant label', async () => {
    const { getByTestId } = await render(<ControlCard control={{ ...mockControl, status: 'non-compliant' }} />)
    expect(getByTestId('control-status-badge').textContent).toContain('Non-Compliant')
  })

  it('shows In Progress label', async () => {
    const { getByTestId } = await render(<ControlCard control={{ ...mockControl, status: 'in-progress' }} />)
    expect(getByTestId('control-status-badge').textContent).toContain('In Progress')
  })

  it('shows Not Applicable label', async () => {
    const { getByTestId } = await render(<ControlCard control={{ ...mockControl, status: 'not-applicable' }} />)
  ...
  it('shows evidence count 1 with singular label', async () => {
    const { getByTestId } = await render(<ControlCard control={{ ...mockControl, evidence: ['file.pdf'] }} />)
    expect(getByTestId('evidence-count').textContent).toContain('1 file')
  })

  it('shows evidence count 3', async () => {
    const { getByTestId } = await render(<ControlCard control={{ ...mockControl, evidence: ['a.pdf', 'b.png', 'c.doc'] }} />)
  ...
  it('shows upcoming due date', async () => {
    const { getByTestId } = await render(<ControlCard control={{ ...mockControl, dueDate: '2099-06-30' }} />)
    expect(getByTestId('control-due-date').textContent).toContain('Due:')
  })

  it('shows overdue label when due date is in the past', async () => {
    const { getByTestId } = await render(<ControlCard control={{ ...mockControl, dueDate: '2020-01-01' }} />)
  ...
  it('shows no-due-date message when dueDate is not set', async () => {
    const { getByTestId } = await render(<ControlCard control={{ ...mockControl, dueDate: undefined }} />)
    expect(getByTestId('no-due-date')).toBeDefined()
  })

  it('does not show due date element when dueDate is undefined', async () => {
    const { queryByTestId } = await render(<ControlCard control={{ ...mockControl, dueDate: undefined }} />)
  ...
  it('shows unassigned when owner is not provided', async () => {
    const { getByTestId } = await render(<ControlCard control={{ ...mockControl, owner: undefined }} />)
    expect(getByTestId('no-owner')).toBeDefined()
  })

  it('does not show owner section when no owner', async () => {
    const { queryByTestId } = await render(<ControlCard control={{ ...mockControl, owner: undefined }} />)
  ...
  it('calls onClick when card is clicked', async () => {
    let clicked = false
    const { getByTestId } = await render(<ControlCard control={mockControl} onClick={() => { clicked = true }} />)
  ...
  it('passes correct control to onClick', async () => {
    ...
    const { getByTestId } = await render(<ControlCard control={controls[2]} onClick={(c) => { received = c }} />)
    await fireEvent.click(getByTestId('control-card'))
  ...
  it('calls onEdit when edit button is clicked', async () => {
    ...
    const { getByTestId } = await render(<ControlCard control={mockControl} onEdit={(c) => { edited = c }} />)
  ...
  it('shows add evidence button when onAddEvidence provided', async () => {
    const { getByTestId } = await render(<ControlCard control={mockControl} onAddEvidence={() => {}} />)
  ...
  it('calls onAddEvidence when button is clicked', async () => {
    ...
    const { getByTestId } = await render(<ControlCard control={mockControl} onAddEvidence={(c) => { evidenceAdded = c }} />)
