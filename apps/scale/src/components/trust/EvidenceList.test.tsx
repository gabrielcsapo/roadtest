...
describe('EvidenceList', () => {
  ...
  it('calls onUpload when upload button is clicked', async () => {
    let uploaded = false
    const { getByTestId } = await render(<EvidenceList evidence={[]} onUpload={() => { uploaded = true }} />)
  ...
  it('does not show upload button in readonly mode', async () => {
    const { queryByTestId } = await render(<EvidenceList evidence={[]} onUpload={() => {}} readonly />)
    expect(queryByTestId('upload-button')).toBeNull()
  })

  // Delete action
  it('shows delete button when onDelete is provided', async () => {
    const { getByTestId } = await render(<EvidenceList evidence={['file.pdf']} onDelete={() => {}} />)
    expect(getByTestId('delete-button-file.pdf')).toBeDefined()
  })

  it('calls onDelete with correct filename', async () => {
    let deleted = ''
    const { getByTestId } = await render(<EvidenceList evidence={['file.pdf']} onDelete={(f) => { deleted = f }} />)
  ...
  it('does not show delete button in readonly mode', async () => {
    const { queryByTestId } = await render(<EvidenceList evidence={['file.pdf']} onDelete={() => {}} readonly />)
    expect(queryByTestId('delete-button-file.pdf')).toBeNull()
  })
  ...
  sampleFiles.map((file) =>
    it(`can delete file ${file}`, async () => {
      ...
      const { getByTestId } = await render(
        <EvidenceList evidence={sampleFiles} onDelete={(f) => { deleted = f }} />
      )
  ...
  it('shows preview button when onPreview is provided', async () => {
    const { getByTestId } = await render(<EvidenceList evidence={['file.pdf']} onPreview={() => {}} />)
    expect(getByTestId('preview-button-file.pdf')).toBeDefined()
  })

  it('calls onPreview with correct filename', async () => {
    let previewed = ''
    const { getByTestId } = await render(<EvidenceList evidence={['file.pdf']} onPreview={(f) => { previewed = f }} />)
  ...
  it('shows preview button in readonly mode', async () => {
    const { getByTestId } = await render(<EvidenceList evidence={['file.pdf']} onPreview={() => {}} readonly />)
    expect(getByTestId('preview-button-file.pdf')).toBeDefined()
  })
  ...
  it('matches snapshot with files', async () => {
    const { container } = await render(<EvidenceList evidence={sampleFiles} onUpload={() => {}} onDelete={() => {}} />)
