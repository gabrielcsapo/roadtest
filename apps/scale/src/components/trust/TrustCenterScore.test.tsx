...
describe('TrustCenterScore', () => {
  ...
  Array.from({ length: 11 }, (_, i) => i * 10).map((pct) =>
    it(`shows score ${pct} correctly`, async () => {
      ...
      const testControls = [
        ...Array.from({ length: compliant }, (_, i) => ({ ...mockControl, id: `comp${i}`, status: 'compliant' as ComplianceStatus })),
        ...Array.from({ length: nonCompliant }, (_, i) => ({ ...mockControl, id: `nc${i}`, status: 'non-compliant' as ComplianceStatus })),
      ]
  ...
  it('shows Needs Attention label for score < 60', async () => {
    const allNonCompliant = controls.map((c) => ({ ...c, status: 'non-compliant' as ComplianceStatus }))
  ...
  it('shows Fair label for score >= 60 and < 80', async () => {
    const mixed = [
      ...Array.from({ length: 6 }, (_, i) => ({ ...mockControl, id: `c${i}`, status: 'compliant' as ComplianceStatus })),
      ...Array.from({ length: 4 }, (_, i) => ({ ...mockControl, id: `n${i}`, status: 'non-compliant' as ComplianceStatus })),
    ]
  ...
  allFrameworks.map((fw) =>
    it(`filters controls for framework ${fw}`, async () => {
      const fwControls = [...controls, { ...mockControl, id: 'extra', framework: fw, status: 'compliant' as ComplianceStatus }]
      const { getByTestId } = await render(<TrustCenterScore controls={fwControls} framework={fw} />)
      expect(getByTestId('trust-score-container')).toBeDefined()
    })
  )
  ...
  it('shows 100% for SOC2 when all SOC2 controls are compliant', async () => {
    const soc2Controls = [
      { ...mockControl, id: 's1', framework: 'SOC2' as Framework, status: 'compliant' as ComplianceStatus },
      { ...mockControl, id: 's2', framework: 'SOC2' as Framework, status: 'compliant' as ComplianceStatus },
    ]
    const { getByTestId } = await render(<TrustCenterScore controls={soc2Controls} framework="SOC2" />)
  ...
  it('shows 0% when framework filter matches no compliant controls', async () => {
    const hipaaControls = [{ ...mockControl, id: 'h1', framework: 'HIPAA' as Framework, status: 'non-compliant' as ComplianceStatus }]
    const { getByTestId } = await render(<TrustCenterScore controls={hipaaControls} framework="HIPAA" />)
    expect(getByTestId('trust-score-value').textContent).toBe('0')
  })

  // Breakdown
  it('does not show breakdown when showBreakdown is false', async () => {
    const { queryByTestId } = await render(<TrustCenterScore controls={controls} showBreakdown={false} />)
  ...
  it('shows 0% when all controls are in-progress', async () => {
    const allInProgress = controls.map((c) => ({ ...c, status: 'in-progress' as ComplianceStatus }))
  ...
  it('handles single compliant control', async () => {
    const { getByTestId } = await render(<TrustCenterScore controls={[{ ...mockControl, status: 'compliant' }]} />)
    expect(getByTestId('trust-score-value').textContent).toBe('100')
  })

  it('handles single non-compliant control', async () => {
    const { getByTestId } = await render(<TrustCenterScore controls={[{ ...mockControl, status: 'non-compliant' }]} />)
    expect(getByTestId('trust-score-value').textContent).toBe('0')
  })
})
