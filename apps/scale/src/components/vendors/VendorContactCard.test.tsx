...
const contacts = [
  ...
  { name: 'Carol White', email: 'carol@umbrella.com', role: 'Compliance Officer', phone: undefined },
...
describe('VendorContactCard', () => {
  ...
  it('renders contact card', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" />)
  ...
  it('snapshot with all props', async () => {
    await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" role="CISO" phone="+1-555-0100" onEmail={() => {}} onCopy={() => {}} />)
    await snapshot('vendor-contact-card-full')
  })

  it('renders with only required props', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Test User" email="test@example.com" />)
    expect(getByTestId('vendor-contact-card')).toBeDefined()
  })

  // Role display (5)
  it('renders role badge when role provided', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" role="CISO" />)
  ...
  it('hides role badge when role not provided', async () => {
    const { queryByTestId } = await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" />)
    expect(queryByTestId('contact-role')).toBeNull()
  })

  it('renders Security Manager role', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Bob Jones" email="bob@globex.com" role="Security Manager" />)
    expect(getByTestId('contact-role').textContent).toBe('Security Manager')
  })

  it('renders Compliance Officer role', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Carol White" email="carol@umbrella.com" role="Compliance Officer" />)
    expect(getByTestId('contact-role').textContent).toBe('Compliance Officer')
  })

  // Phone display (5)
  it('renders phone row when phone provided', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" phone="+1-555-0100" />)
  ...
  it('renders different phone number', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Dave Brown" email="dave@initech.com" phone="+1-555-0400" />)
    expect(getByTestId('contact-phone-value').textContent).toBe('+1-555-0400')
  })

  // Email callback (10)
  it('shows send email button when onEmail provided', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" onEmail={() => {}} />)
  ...
  it('calls onEmail with email when send email clicked', async () => {
    let received = ''
    const { getByTestId } = await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" onEmail={e => { received = e }} />)
  ...
  it('fires onEmail for different email addresses', async () => {
    for (const c of contacts.slice(0, 3)) {
      let received = ''
      const { getByTestId } = await render(<VendorContactCard name={c.name} email={c.email} onEmail={e => { received = e }} />)
      await fireEvent.click(getByTestId('send-email-button'))
  ...
  it('fires onEmail multiple times', async () => {
    let count = 0
    const { getByTestId } = await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" onEmail={() => { count++ }} />)
    await fireEvent.click(getByTestId('send-email-button'))
    await fireEvent.click(getByTestId('send-email-button'))
    expect(count).toBe(2)
  })
  ...
  it('shows copy email button when onCopy provided', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" onCopy={() => {}} />)
  ...
  it('calls onCopy with email when copy email clicked', async () => {
    let received = ''
    const { getByTestId } = await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" onCopy={v => { received = v }} />)
    await fireEvent.click(getByTestId('copy-email-button'))
    expect(received).toBe('alice@acme.com')
  })

  it('shows copy phone button when phone and onCopy provided', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" phone="+1-555-0100" onCopy={() => {}} />)
    expect(getByTestId('copy-phone-button')).toBeDefined()
  })

  it('hides copy phone button when no phone', async () => {
    const { queryByTestId } = await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" onCopy={() => {}} />)
    expect(queryByTestId('copy-phone-button')).toBeNull()
  })

  it('calls onCopy with phone when copy phone clicked', async () => {
    let received = ''
    const { getByTestId } = await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" phone="+1-555-0100" onCopy={v => { received = v }} />)
  ...
  it('hides copy phone when onCopy not provided', async () => {
    const { queryByTestId } = await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" phone="+1-555-0100" />)
    expect(queryByTestId('copy-phone-button')).toBeNull()
  })

  it('fires onCopy multiple times', async () => {
    let count = 0
    const { getByTestId } = await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" onCopy={() => { count++ }} />)
    await fireEvent.click(getByTestId('copy-email-button'))
    await fireEvent.click(getByTestId('copy-email-button'))
    expect(count).toBe(2)
  })
  ...
  it('snapshot with copy buttons', async () => {
    await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" phone="+1-555-0100" onCopy={() => {}} />)
  ...
  for (const contact of contacts) {
    it(`renders contact: ${contact.name}`, async () => {
      const { getByTestId } = await render(<VendorContactCard name={contact.name} email={contact.email} role={contact.role} phone={contact.phone} />)
  ...
  it('renders with long name', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Very Long Contact Name Here" email="long@example.com" />)
    expect(getByTestId('contact-name')).toBeDefined()
  })

  it('renders with long email', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice Smith" email="verylongemailaddress@verylongdomainname.com" />)
    expect(getByTestId('contact-email-value')).toBeDefined()
  })

  it('renders without phone but with all button handlers', async () => {
    const { queryByTestId } = await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" onEmail={() => {}} onCopy={() => {}} />)
    expect(queryByTestId('contact-phone-row')).toBeNull()
  })

  it('renders with all props provided', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" role="CISO" phone="+1-555-0100" onEmail={() => {}} onCopy={() => {}} />)
    expect(getByTestId('vendor-contact-card')).toBeDefined()
  })
  ...
  it('renders correct email for different contacts', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Bob Jones" email="bob@globex.com" />)
    expect(getByTestId('contact-email-value').textContent).toBe('bob@globex.com')
  })

  it('role shows DPO role', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Test" email="t@t.com" role="DPO" />)
    expect(getByTestId('contact-role').textContent).toBe('DPO')
  })

  it('renders correctly with empty phone string handling', async () => {
    const { queryByTestId } = await render(<VendorContactCard name="Alice" email="alice@test.com" phone={undefined} />)
    expect(queryByTestId('contact-phone-row')).toBeNull()
  })
  ...
  it('snapshot with phone only', async () => {
    await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" phone="+1-555-0100" />)
  ...
  it('copy email passes correct value when multiple emails', async () => {
    ...
    for (const email of emails) {
      let received = ''
      const { getByTestId } = await render(<VendorContactCard name="Test" email={email} onCopy={v => { received = v }} />)
      await fireEvent.click(getByTestId('copy-email-button'))
  ...
  it('renders correctly when only name and email provided', async () => {
    const { getByTestId, queryByTestId } = await render(<VendorContactCard name="Test" email="test@test.com" />)
    expect(getByTestId('vendor-contact-card')).toBeDefined()
    expect(queryByTestId('contact-role')).toBeNull()
    expect(queryByTestId('contact-phone-row')).toBeNull()
    expect(queryByTestId('send-email-button')).toBeNull()
  ...
  it('contact card has correct data-testid attribute', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice" email="alice@test.com" />)
    expect(getByTestId('vendor-contact-card').getAttribute('data-testid')).toBe('vendor-contact-card')
  })
  ...
  it('contact name renders within header', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice Smith" email="alice@test.com" />)
    expect(getByTestId('contact-header')).toBeDefined()
    expect(getByTestId('contact-name')).toBeDefined()
  })
  ...
  it('email value has correct data-testid', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice" email="alice@test.com" />)
    expect(getByTestId('contact-email-value').getAttribute('data-testid')).toBe('contact-email-value')
  })

  it('phone row data-testid is present when phone given', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice" email="alice@test.com" phone="+1-800-0000" />)
  ...
  it('phone value data-testid is present when phone given', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice" email="alice@test.com" phone="+1-800-0000" />)
    expect(getByTestId('contact-phone-value').getAttribute('data-testid')).toBe('contact-phone-value')
  })

  it('renders with IT Manager role', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Test" email="t@t.com" role="IT Manager" />)
    expect(getByTestId('contact-role').textContent).toBe('IT Manager')
  })

  it('renders with VP of Security role', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Test" email="t@t.com" role="VP of Security" />)
    expect(getByTestId('contact-role').textContent).toBe('VP of Security')
  })

  it('renders with CTO role', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Test" email="t@t.com" role="CTO" />)
    expect(getByTestId('contact-role').textContent).toBe('CTO')
  })

  it('renders with CEO role', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Test" email="t@t.com" role="CEO" />)
    expect(getByTestId('contact-role').textContent).toBe('CEO')
  })

  it('sends correct email for contact Bob', async () => {
    let received = ''
    const { getByTestId } = await render(<VendorContactCard name="Bob" email="bob@globex.com" onEmail={e => { received = e }} />)
    await fireEvent.click(getByTestId('send-email-button'))
  ...
  it('sends correct email for contact Carol', async () => {
    let received = ''
    const { getByTestId } = await render(<VendorContactCard name="Carol" email="carol@umbrella.com" onEmail={e => { received = e }} />)
    await fireEvent.click(getByTestId('send-email-button'))
  ...
  it('sends correct email for contact Dave', async () => {
    let received = ''
    const { getByTestId } = await render(<VendorContactCard name="Dave" email="dave@initech.com" onEmail={e => { received = e }} />)
    await fireEvent.click(getByTestId('send-email-button'))
  ...
  it('sends correct email for contact Eve', async () => {
    let received = ''
    const { getByTestId } = await render(<VendorContactCard name="Eve" email="eve@massive.com" onEmail={e => { received = e }} />)
    await fireEvent.click(getByTestId('send-email-button'))
  ...
  it('copies correct email for contact Bob', async () => {
    let received = ''
    const { getByTestId } = await render(<VendorContactCard name="Bob" email="bob@globex.com" onCopy={v => { received = v }} />)
    await fireEvent.click(getByTestId('copy-email-button'))
    expect(received).toBe('bob@globex.com')
  })
  ...
  it('copies correct email for contact Carol', async () => {
    let received = ''
    const { getByTestId } = await render(<VendorContactCard name="Carol" email="carol@umbrella.com" onCopy={v => { received = v }} />)
    await fireEvent.click(getByTestId('copy-email-button'))
    expect(received).toBe('carol@umbrella.com')
  })
  ...
  it('copies correct phone for different number', async () => {
    let received = ''
    const { getByTestId } = await render(<VendorContactCard name="Bob" email="bob@globex.com" phone="+1-555-0200" onCopy={v => { received = v }} />)
    await fireEvent.click(getByTestId('copy-phone-button'))
  ...
  it('copies correct phone for Dave', async () => {
    let received = ''
    const { getByTestId } = await render(<VendorContactCard name="Dave" email="dave@initech.com" phone="+1-555-0400" onCopy={v => { received = v }} />)
    await fireEvent.click(getByTestId('copy-phone-button'))
  ...
  it('send email fires once on single click', async () => {
    let count = 0
    const { getByTestId } = await render(<VendorContactCard name="Alice" email="alice@test.com" onEmail={() => { count++ }} />)
    await fireEvent.click(getByTestId('send-email-button'))
    expect(count).toBe(1)
  })
  ...
  it('copy email fires once on single click', async () => {
    let count = 0
    const { getByTestId } = await render(<VendorContactCard name="Alice" email="alice@test.com" onCopy={() => { count++ }} />)
    await fireEvent.click(getByTestId('copy-email-button'))
    expect(count).toBe(1)
  })
  ...
  it('copy phone fires once on single click', async () => {
    let count = 0
    const { getByTestId } = await render(<VendorContactCard name="Alice" email="alice@test.com" phone="+1-555-0100" onCopy={() => { count++ }} />)
    await fireEvent.click(getByTestId('copy-phone-button'))
    expect(count).toBe(1)
  })
  ...
  it('snapshot contact with email button only', async () => {
    await render(<VendorContactCard name="Alice Smith" email="alice@acme.com" onEmail={() => {}} />)
  ...
  it('snapshot contact Dave with phone', async () => {
    await render(<VendorContactCard name="Dave Brown" email="dave@initech.com" phone="+1-555-0400" role={undefined} />)
    await snapshot('vendor-contact-card-dave')
  })

  it('snapshot contact with role and no phone', async () => {
    await render(<VendorContactCard name="Bob Jones" email="bob@globex.com" role="Security Manager" />)
  ...
  it('email label data-testid is contact-email-label', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice" email="alice@test.com" />)
    expect(getByTestId('contact-email-label').getAttribute('data-testid')).toBe('contact-email-label')
  })

  it('phone label data-testid is contact-phone-label when phone given', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice" email="alice@test.com" phone="+1-555-0100" />)
    expect(getByTestId('contact-phone-label').getAttribute('data-testid')).toBe('contact-phone-label')
  })

  it('send email button data-testid is send-email-button', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice" email="alice@test.com" onEmail={() => {}} />)
  ...
  it('copy email button data-testid is copy-email-button', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice" email="alice@test.com" onCopy={() => {}} />)
  ...
  it('copy phone button data-testid is copy-phone-button', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice" email="alice@test.com" phone="+1-555-0100" onCopy={() => {}} />)
  ...
  it('renders role badge data-testid is contact-role', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice" email="alice@test.com" role="CISO" />)
  ...
  it('both send email and copy buttons present when both handlers provided', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice" email="alice@test.com" onEmail={() => {}} onCopy={() => {}} />)
    expect(getByTestId('send-email-button')).toBeDefined()
    expect(getByTestId('copy-email-button')).toBeDefined()
  })

  it('both send email and copy phone buttons present with phone', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice" email="alice@test.com" phone="+1-555-0100" onEmail={() => {}} onCopy={() => {}} />)
    expect(getByTestId('send-email-button')).toBeDefined()
    expect(getByTestId('copy-phone-button')).toBeDefined()
  })

  it('role, phone row, and email row all visible when all props provided', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Alice" email="alice@test.com" role="CISO" phone="+1-555-0100" />)
    expect(getByTestId('contact-role')).toBeDefined()
    expect(getByTestId('contact-phone-row')).toBeDefined()
    expect(getByTestId('contact-email-row')).toBeDefined()
  })
  ...
  it('renders name Carol White correctly', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Carol White" email="carol@umbrella.com" />)
    expect(getByTestId('contact-name').textContent).toBe('Carol White')
  })

  it('renders name Dave Brown correctly', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Dave Brown" email="dave@initech.com" />)
    expect(getByTestId('contact-name').textContent).toBe('Dave Brown')
  })

  it('renders name Eve Davis correctly', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Eve Davis" email="eve@massive.com" />)
  ...
  it('renders email carol@umbrella.com correctly', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Carol" email="carol@umbrella.com" />)
  ...
  it('renders email dave@initech.com correctly', async () => {
    const { getByTestId } = await render(<VendorContactCard name="Dave" email="dave@initech.com" />)
