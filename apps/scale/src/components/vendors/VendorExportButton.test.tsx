...
describe('VendorExportButton', () => {
  ...
  it('renders export button container', async () => {
    const { getByTestId } = await render(<VendorExportButton vendors={vendors} onExport={() => {}} />)
  ...
  it('dropdown is hidden by default', async () => {
    const { queryByTestId } = await render(<VendorExportButton vendors={vendors} onExport={() => {}} />)
  ...
  it('clicking trigger again closes dropdown', async () => {
    const { getByTestId, queryByTestId } = await render(<VendorExportButton vendors={vendors} onExport={() => {}} />)
    await fireEvent.click(getByTestId('export-trigger-button'))
    await fireEvent.click(getByTestId('export-trigger-button'))
    expect(queryByTestId('export-dropdown')).toBeNull()
  })
  ...
  for (const format of formats) {
    it(`calls onExport with ${format} when ${format} option selected`, async () => {
      let received = ''
      const { getByTestId } = await render(<VendorExportButton vendors={vendors} onExport={f => { received = f }} />)
      await fireEvent.click(getByTestId('export-trigger-button'))
  ...
  it('fires onExport once per selection', async () => {
    let count = 0
    const { getByTestId } = await render(<VendorExportButton vendors={vendors} onExport={() => { count++ }} />)
    await fireEvent.click(getByTestId('export-trigger-button'))
    await fireEvent.click(getByTestId('export-option-csv'))
    expect(count).toBe(1)
  })
  ...
  it('does not call onExport without selecting option', async () => {
    let exported = false
    const { getByTestId } = await render(<VendorExportButton vendors={vendors} onExport={() => { exported = true }} />)
    await fireEvent.click(getByTestId('export-trigger-button'))
  ...
  it('trigger button is disabled when disabled=true', async () => {
    const { getByTestId } = await render(<VendorExportButton vendors={vendors} onExport={() => {}} disabled />)
    expect(getByTestId('export-trigger-button').disabled).toBeTruthy()
  })

  it('trigger button is disabled when loading=true', async () => {
    const { getByTestId } = await render(<VendorExportButton vendors={vendors} onExport={() => {}} loading />)
    expect(getByTestId('export-trigger-button').disabled).toBeTruthy()
  })
  ...
  it('dropdown does not open when disabled', async () => {
    const { getByTestId, queryByTestId } = await render(<VendorExportButton vendors={vendors} onExport={() => {}} disabled />)
  ...
  it('shows count of 1 vendor', async () => {
    const { getByTestId } = await render(<VendorExportButton vendors={[mockVendor]} onExport={() => {}} />)
  ...
  it('shows count of 20 vendors', async () => {
    ...
    const { getByTestId } = await render(<VendorExportButton vendors={manyVendors} onExport={() => {}} />)
  ...
  it('shows count of 100 vendors', async () => {
    ...
    const { getByTestId } = await render(<VendorExportButton vendors={bigList} onExport={() => {}} />)
    expect(getByTestId('export-trigger-button').textContent).toContain('100')
  })

  it('renders correctly with 2 vendors', async () => {
    const { getByTestId } = await render(<VendorExportButton vendors={vendors.slice(0, 2)} onExport={() => {}} />)
  ...
  it('renders correctly with 10 vendors', async () => {
    ...
    const { getByTestId } = await render(<VendorExportButton vendors={tenVendors} onExport={() => {}} />)
  ...
  it('trigger button disabled with 0 vendors even if not explicitly disabled', async () => {
    const { getByTestId } = await render(<VendorExportButton vendors={[]} onExport={() => {}} disabled={false} />)
    expect(getByTestId('export-trigger-button').disabled).toBeTruthy()
  })
  ...
  it('renders with disabled and loading both true', async () => {
    const { getByTestId } = await render(<VendorExportButton vendors={vendors} onExport={() => {}} disabled loading />)
    expect(getByTestId('export-trigger-button').disabled).toBeTruthy()
  })
  ...
  it('exports csv successfully', async () => {
    let format = ''
    const { getByTestId } = await render(<VendorExportButton vendors={vendors} onExport={f => { format = f }} />)
    await fireEvent.click(getByTestId('export-trigger-button'))
    await fireEvent.click(getByTestId('export-option-csv'))
  ...
  it('renders with disabled=false explicitly', async () => {
    const { getByTestId } = await render(<VendorExportButton vendors={vendors} onExport={() => {}} disabled={false} />)
    expect(getByTestId('export-trigger-button').disabled).toBe(false)
  })

  it('renders with loading=false explicitly', async () => {
    const { queryByTestId } = await render(<VendorExportButton vendors={vendors} onExport={() => {}} loading={false} />)
    expect(queryByTestId('export-spinner')).toBeNull()
  })
  ...
  it('vendor-export-button data-testid is vendor-export-button', async () => {
    const { getByTestId } = await render(<VendorExportButton vendors={vendors} onExport={() => {}} />)
    expect(getByTestId('vendor-export-button').getAttribute('data-testid')).toBe('vendor-export-button')
  })

  it('export-trigger-button data-testid is export-trigger-button', async () => {
    const { getByTestId } = await render(<VendorExportButton vendors={vendors} onExport={() => {}} />)
    expect(getByTestId('export-trigger-button').getAttribute('data-testid')).toBe('export-trigger-button')
  })
  ...
  it('export-option-json data-testid is export-option-json', async () => {
    const { getByTestId } = await render(<VendorExportButton vendors={vendors} onExport={() => {}} />)
    await fireEvent.click(getByTestId('export-trigger-button'))
    expect(getByTestId('export-option-json').getAttribute('data-testid')).toBe('export-option-json')
  })
  ...
  it('dropdown does not open when loading', async () => {
    const { queryByTestId } = await render(<VendorExportButton vendors={vendors} onExport={() => {}} loading />)
    expect(queryByTestId('export-dropdown')).toBeNull()
  })
  ...
  it('trigger button not disabled with loading=false', async () => {
    const { getByTestId } = await render(<VendorExportButton vendors={vendors} onExport={() => {}} loading={false} />)
    expect(getByTestId('export-trigger-button').disabled).toBe(false)
  })
  ...
  it('empty-hint hidden when vendors present', async () => {
    const { queryByTestId } = await render(<VendorExportButton vendors={[mockVendor]} onExport={() => {}} />)
    expect(queryByTestId('export-empty-hint')).toBeNull()
  })
  ...
  it('renders container element', async () => {
    const { container } = await render(<VendorExportButton vendors={vendors} onExport={() => {}} />)
  ...
  it('trigger button shows (3) for 3 vendors', async () => {
    const { getByTestId } = await render(<VendorExportButton vendors={vendors.slice(0, 3)} onExport={() => {}} />)
    expect(getByTestId('export-trigger-button').textContent).toContain('(3)')
  })

  it('trigger button shows (4) for 4 vendors', async () => {
    const { getByTestId } = await render(<VendorExportButton vendors={vendors.slice(0, 4)} onExport={() => {}} />)
    expect(getByTestId('export-trigger-button').textContent).toContain('(4)')
  })

  it('dropdown has exactly 3 options when open', async () => {
    const { getByTestId, container } = await render(<VendorExportButton vendors={vendors} onExport={() => {}} />)
    await fireEvent.click(getByTestId('export-trigger-button'))
