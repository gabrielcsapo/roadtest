...
describe("VendorComplianceStatus", () => {
  ...
  it("renders compliance status card", async () => {
    const { getByTestId } = await render(
      <VendorComplianceStatus vendor={mockVendor} frameworks={sampleFrameworks} />,
    );
  ...
  it("hides subtitle in compact mode", async () => {
    const { queryByTestId } = await render(
      <VendorComplianceStatus vendor={mockVendor} frameworks={sampleFrameworks} compact />,
    );
  ...
  it("shows 100% when all compliant", async () => {
    const allCompliant = sampleFrameworks.map((f) => ({
      ...f,
      status: "compliant" as ComplianceStatus,
    }));
    const { getByTestId } = await render(
      <VendorComplianceStatus vendor={mockVendor} frameworks={allCompliant} />,
    );
    expect(getByTestId("compliance-score").textContent).toContain("100%");
  });

  it("shows 0% when none compliant", async () => {
    const noneCompliant = sampleFrameworks.map((f) => ({
      ...f,
      status: "non-compliant" as ComplianceStatus,
    }));
    const { getByTestId } = await render(
      <VendorComplianceStatus vendor={mockVendor} frameworks={noneCompliant} />,
    );
  ...
  it("shows 0% for empty frameworks", async () => {
    const { getByTestId } = await render(
      <VendorComplianceStatus vendor={mockVendor} frameworks={[]} />,
    );
    expect(getByTestId("compliance-score").textContent).toContain("0%");
  });
  ...
  it("shows 80% for 4 of 5 compliant", async () => {
    ...
    const { getByTestId } = await render(
      <VendorComplianceStatus vendor={mockVendor} frameworks={fourCompliant} />,
    );
  ...
  it("shows 50% for half compliant", async () => {
    ...
    const { getByTestId } = await render(
      <VendorComplianceStatus vendor={mockVendor} frameworks={half} />,
    );
  ...
  it("only compliant status counts toward score", async () => {
    ...
    const { getByTestId } = await render(
      <VendorComplianceStatus vendor={mockVendor} frameworks={mixed} />,
    );
  ...
  it("renders framework name", async () => {
    const { getByTestId } = await render(
      <VendorComplianceStatus
        vendor={mockVendor}
        frameworks={[{ name: "SOC 2", status: "compliant" }]}
      />,
    );
  ...
  for (const status of complianceStatuses) {
    it(`renders ${status} framework status`, async () => {
      const { getByTestId } = await render(
        <VendorComplianceStatus vendor={mockVendor} frameworks={[{ name: "Test", status }]} />,
      );
  ...
  for (const vendor of vendors) {
    it(`renders compliance for vendor: ${vendor.name}`, async () => {
      const { getByTestId } = await render(
        <VendorComplianceStatus vendor={vendor} frameworks={sampleFrameworks} />,
      );
  ...
  it("renders with 10 frameworks", async () => {
    const tenFrameworks = Array.from({ length: 10 }, (_, i) => ({
      name: `FW${i}`,
      status: "compliant" as ComplianceStatus,
    }));
    const { getByTestId } = await render(
      <VendorComplianceStatus vendor={mockVendor} frameworks={tenFrameworks} />,
    );
  ...
  it("renders non-compliant status badge for HIPAA", async () => {
    const { getByTestId } = await render(
      <VendorComplianceStatus
        vendor={mockVendor}
        frameworks={[{ name: "HIPAA", status: "non-compliant" }]}
      />,
    );
  ...
  it("renders in-progress status badge for GDPR", async () => {
    const { getByTestId } = await render(
      <VendorComplianceStatus
        vendor={mockVendor}
        frameworks={[{ name: "GDPR", status: "in-progress" }]}
      />,
    );
  ...
  it("renders not-applicable status badge for PCI DSS", async () => {
    const { getByTestId } = await render(
      <VendorComplianceStatus
        vendor={mockVendor}
        frameworks={[{ name: "PCI DSS", status: "not-applicable" }]}
      />,
    );
  ...
  it("renders Globex Inc compliance", async () => {
    const { getByTestId } = await render(
      <VendorComplianceStatus vendor={vendors[1]} frameworks={sampleFrameworks} />,
    );
  ...
  it("renders Umbrella Corp compliance", async () => {
    const { getByTestId } = await render(
      <VendorComplianceStatus vendor={vendors[2]} frameworks={sampleFrameworks} />,
    );
  ...
  it("renders Initech compliance", async () => {
    const { getByTestId } = await render(
      <VendorComplianceStatus vendor={vendors[3]} frameworks={sampleFrameworks} />,
    );
  ...
  it("renders Massive Dynamic compliance", async () => {
    const { getByTestId } = await render(
      <VendorComplianceStatus vendor={vendors[4]} frameworks={sampleFrameworks} />,
    );
  ...
  it("progress bar shows 0 for 0% compliance", async () => {
    const { getByTestId } = await render(
      <VendorComplianceStatus
        vendor={mockVendor}
        frameworks={[{ name: "X", status: "non-compliant" }]}
      />,
    );
    expect(getByTestId("compliance-progress")).toBeDefined();
  });
  ...
  it("score badge shows 100% for compliant only", async () => {
    const { getByTestId } = await render(
      <VendorComplianceStatus
        vendor={mockVendor}
        frameworks={[{ name: "X", status: "compliant" }]}
      />,
    );
    expect(getByTestId("compliance-score").textContent).toContain("100%");
  });
  ...
  it("renders 3 frameworks correctly", async () => {
    ...
    const { getByTestId } = await render(
      <VendorComplianceStatus vendor={mockVendor} frameworks={three} />,
    );
  ...
  it("renders 2 frameworks correctly", async () => {
    ...
    const { getByTestId } = await render(
      <VendorComplianceStatus vendor={mockVendor} frameworks={two} />,
    );
  ...
  it("renders correctly with not-applicable only", async () => {
    const naOnly = sampleFrameworks.map((f) => ({
      ...f,
      status: "not-applicable" as ComplianceStatus,
    }));
    const { getByTestId } = await render(
      <VendorComplianceStatus vendor={mockVendor} frameworks={naOnly} />,
    );
    expect(getByTestId("compliance-score").textContent).toContain("0%");
  });

  it("renders correctly with in-progress only", async () => {
    const inProgress = sampleFrameworks.map((f) => ({
      ...f,
      status: "in-progress" as ComplianceStatus,
    }));
    const { getByTestId } = await render(
      <VendorComplianceStatus vendor={mockVendor} frameworks={inProgress} />,
    );
    expect(getByTestId("compliance-score").textContent).toContain("0%");
  });
  ...
  it("snapshot with 3 frameworks", async () => {
    await render(
      <VendorComplianceStatus vendor={mockVendor} frameworks={sampleFrameworks.slice(0, 3)} />,
    );
  ...
  it("snapshot compact with Umbrella Corp", async () => {
    await render(
      <VendorComplianceStatus vendor={vendors[2]} frameworks={sampleFrameworks} compact />,
    );
  ...
  it("vendor-compliance-status data-testid is vendor-compliance-status", async () => {
    ...
    expect(getByTestId("vendor-compliance-status").getAttribute("data-testid")).toBe(
  ...
  it("compliance-progress data-testid is compliance-progress", async () => {
    ...
    expect(getByTestId("compliance-progress").getAttribute("data-testid")).toBe(
      "compliance-progress",
    );
  });
  ...
  it("compliance-summary data-testid is compliance-summary", async () => {
    ...
    expect(getByTestId("compliance-summary").getAttribute("data-testid")).toBe(
      "compliance-summary",
    );
  });
  ...
  it("compliance-score-badge data-testid is compliance-score-badge", async () => {
    ...
    expect(getByTestId("compliance-score-badge").getAttribute("data-testid")).toBe(
  ...
  it("compliance-subtitle data-testid is compliance-subtitle", async () => {
    ...
    expect(getByTestId("compliance-subtitle").getAttribute("data-testid")).toBe(
      "compliance-subtitle",
    );
  });
  ...
  it("compliance-compact-badges data-testid is correct in compact mode", async () => {
    ...
    expect(getByTestId("compliance-compact-badges").getAttribute("data-testid")).toBe(
      "compliance-compact-badges",
    );
  });
  ...
  it("summary shows 1 of 2 when 1 compliant", async () => {
    ...
    const { getByTestId } = await render(
      <VendorComplianceStatus vendor={mockVendor} frameworks={partial} />,
    );
  ...
  it("score is 25% with 1 compliant of 4", async () => {
    ...
    const { getByTestId } = await render(
      <VendorComplianceStatus vendor={mockVendor} frameworks={fws} />,
    );
  ...
  it("title shows Acme Corp", async () => {
    const { getByTestId } = await render(
      <VendorComplianceStatus vendor={vendors[0]} frameworks={sampleFrameworks} />,
    );
    expect(getByTestId("compliance-title").textContent).toContain("Acme Corp");
  });
  ...
  it("compliance-frameworks data-testid is compliance-frameworks", async () => {
    ...
    expect(getByTestId("compliance-frameworks").getAttribute("data-testid")).toBe(
