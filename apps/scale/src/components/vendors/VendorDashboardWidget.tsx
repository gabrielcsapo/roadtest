...
export function VendorDashboardWidget({
  vendors,
  onClick,
  loading = false,
}: VendorDashboardWidgetProps) {
  if (loading) {
    return (
  ...
  const riskCounts = RISK_ORDER.reduce<Record<Risk, number>>(
    (acc, r) => {
  ...
  return (
    ...
    >
      ...
      >
        <h3 data-testid="widget-title" style={{ margin: 0 }}>
          Vendors
        </h3>
      ...
      >
        <div data-testid="stat-total" style={{ textAlign: "center" }}>
          <div data-testid="stat-total-count" style={{ fontSize: "28px", fontWeight: "bold" }}>
            {total}
          </div>
      ...
      <div data-testid="widget-risk-breakdown" style={{ marginBottom: "16px" }}>
        ...
        {RISK_ORDER.map((risk) => {
          ...
          return (
            <div
              key={risk}
              data-testid={`widget-risk-row-${risk}`}
              style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}
            >
              <span
                data-testid={`widget-risk-label-${risk}`}
                style={{ width: "60px", fontSize: "12px", textTransform: "capitalize" }}
              >
                {risk}
              </span>
              <Progress
                data-testid={`widget-risk-progress-${risk}`}
                value={pct}
                color={RISK_COLORS[risk]}
                style={{ flex: 1 }}
              />
              <span
                data-testid={`widget-risk-count-${risk}`}
                style={{ fontSize: "12px", color: "#6b7280", width: "30px", textAlign: "right" }}
              >
                {count}
              </span>
            </div>
      ...
      {topRiskVendors.length > 0 && (
        <div data-testid="widget-top-risk-vendors">
          ...
          {topRiskVendors.map((vendor) => (
            ...
            >
              <span data-testid={`widget-vendor-name-${vendor.id}`} style={{ fontSize: "13px" }}>
                {vendor.name}
              </span>
              <div style={{ display: "flex", gap: "4px" }}>
                <RiskBadge
                  risk={vendor.riskLevel}
                  data-testid={`widget-vendor-risk-${vendor.id}`}
                />
                <StatusBadge
                  status={vendor.status}
                  data-testid={`widget-vendor-status-${vendor.id}`}
                />
              </div>
            </div>
          ))}
      ...
      {topRiskVendors.length === 0 && (
        <div
          data-testid="widget-no-risk-vendors"
          style={{ fontSize: "13px", color: "#6b7280", textAlign: "center", padding: "12px 0" }}
        >
