...
describe('VendorDashboardWidget', () => {
  ...
  it('loading=false shows normal content', async () => {
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} loading={false} />)
    expect(getByTestId('widget-stats')).toBeDefined()
  })
  ...
  it('calls onClick when widget clicked', async () => {
    let clicked = false
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} onClick={() => { clicked = true }} />)
  ...
  it('calls onClick without vendor when widget clicked', async () => {
    ...
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} onClick={v => { received = v }} />)
    await fireEvent.click(getByTestId('vendor-dashboard-widget'))
  ...
  it('fires onClick multiple times', async () => {
    let count = 0
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} onClick={() => { count++ }} />)
    await fireEvent.click(getByTestId('vendor-dashboard-widget'))
    await fireEvent.click(getByTestId('vendor-dashboard-widget'))
    expect(count).toBe(2)
  })
  ...
  it('vendor row click does not propagate to widget', async () => {
    let widgetClicks = 0
    let vendorClicks = 0
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} onClick={v => { if (v) vendorClicks++; else widgetClicks++ }} />)
    await fireEvent.click(getByTestId('widget-vendor-3'))
  ...
  it('can click each top risk vendor', async () => {
    ...
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} onClick={v => { if (v) clicked.push(v.id) }} />)
    await fireEvent.click(getByTestId('widget-vendor-3'))
  ...
  it('widget has pointer cursor when onClick provided', async () => {
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} onClick={() => {}} />)
    expect(getByTestId('vendor-dashboard-widget')).toBeDefined()
  })
  ...
  it('renders with 20 vendors', async () => {
    const manyVendors = Array.from({ length: 20 }, (_, i) => ({ ...mockVendor, id: String(i), riskLevel: riskLevels[i % 4] }))
  ...
  it('renders correctly with all optional props', async () => {
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} onClick={() => {}} loading={false} />)
    expect(getByTestId('vendor-dashboard-widget')).toBeDefined()
  })
  ...
  it('shows only up to 3 top risk vendors even with many', async () => {
    const manyHighRisk = Array.from({ length: 10 }, (_, i) => ({ ...mockVendor, id: String(i), riskLevel: 'critical' as Risk }))
  ...
  it('widget container data-testid is vendor-dashboard-widget', async () => {
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} />)
    expect(getByTestId('vendor-dashboard-widget').getAttribute('data-testid')).toBe('vendor-dashboard-widget')
  })
  ...
  it('widget-total-badge data-testid is widget-total-badge', async () => {
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} />)
    expect(getByTestId('widget-total-badge').getAttribute('data-testid')).toBe('widget-total-badge')
  })
  ...
  it('widget-risk-breakdown data-testid is widget-risk-breakdown', async () => {
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} />)
    expect(getByTestId('widget-risk-breakdown').getAttribute('data-testid')).toBe('widget-risk-breakdown')
  })
  ...
  it('stat-active-count shows 1 when 1 active vendor', async () => {
    const { getByTestId } = await render(<VendorDashboardWidget vendors={[{ ...mockVendor, status: 'active' }]} />)
  ...
  it('stat-critical-count shows 2 when 2 critical vendors', async () => {
    const twoCritical = vendors.map((v, i) => ({ ...v, riskLevel: i < 2 ? 'critical' as Risk : 'low' as Risk }))
  ...
  it('widget-risk-row-low data-testid is correct', async () => {
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} />)
    expect(getByTestId('widget-risk-row-low').getAttribute('data-testid')).toBe('widget-risk-row-low')
  })

  it('widget-risk-row-medium data-testid is correct', async () => {
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} />)
    expect(getByTestId('widget-risk-row-medium').getAttribute('data-testid')).toBe('widget-risk-row-medium')
  })

  it('widget-risk-row-high data-testid is correct', async () => {
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} />)
    expect(getByTestId('widget-risk-row-high').getAttribute('data-testid')).toBe('widget-risk-row-high')
  })

  it('widget-risk-row-critical data-testid is correct', async () => {
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} />)
    expect(getByTestId('widget-risk-row-critical').getAttribute('data-testid')).toBe('widget-risk-row-critical')
  })
  ...
  it('widget-risk-progress-low data-testid is correct', async () => {
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} />)
    expect(getByTestId('widget-risk-progress-low').getAttribute('data-testid')).toBe('widget-risk-progress-low')
  })

  it('widget-risk-progress-medium data-testid is correct', async () => {
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} />)
    expect(getByTestId('widget-risk-progress-medium').getAttribute('data-testid')).toBe('widget-risk-progress-medium')
  })

  it('widget-risk-progress-high data-testid is correct', async () => {
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} />)
    expect(getByTestId('widget-risk-progress-high').getAttribute('data-testid')).toBe('widget-risk-progress-high')
  })

  it('widget-risk-progress-critical data-testid is correct', async () => {
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} />)
    expect(getByTestId('widget-risk-progress-critical').getAttribute('data-testid')).toBe('widget-risk-progress-critical')
  })
  ...
  it('widget-top-risk-vendors data-testid is correct', async () => {
    const { getByTestId } = await render(<VendorDashboardWidget vendors={vendors} />)
    expect(getByTestId('widget-top-risk-vendors').getAttribute('data-testid')).toBe('widget-top-risk-vendors')
  })

  it('widget-no-risk-vendors data-testid is correct for all low', async () => {
    const allLow = vendors.map(v => ({ ...v, riskLevel: 'low' as Risk }))
    const { getByTestId } = await render(<VendorDashboardWidget vendors={allLow} />)
    expect(getByTestId('widget-no-risk-vendors').getAttribute('data-testid')).toBe('widget-no-risk-vendors')
  })

  it('renders widget with 10 vendors and shows count 10', async () => {
    const tenVendors = Array.from({ length: 10 }, (_, i) => ({ ...mockVendor, id: String(i), riskLevel: riskLevels[i % 4] }))
  ...
  it('renders widget with 4 critical vendors', async () => {
    const fourCritical = vendors.map((v, i) => ({ ...v, riskLevel: i < 4 ? 'critical' as Risk : 'low' as Risk }))
