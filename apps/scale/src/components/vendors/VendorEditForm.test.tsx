...
describe('VendorEditForm', () => {
  ...
  it('renders form container', async () => {
    const { getByTestId } = await render(<VendorEditForm onSubmit={() => {}} onCancel={() => {}} />)
  ...
  it('renders Edit Vendor title when vendor prop provided', async () => {
    const { getByTestId } = await render(<VendorEditForm vendor={mockVendor} onSubmit={() => {}} onCancel={() => {}} />)
  ...
  it('updates email field on change', async () => {
    const { getByTestId } = await render(<VendorEditForm onSubmit={() => {}} onCancel={() => {}} />)
    await fireEvent.change(getByTestId('input-contact-email'), { target: { value: 'test@example.com' } })
  ...
  it('updates description on change', async () => {
    const { getByTestId } = await render(<VendorEditForm onSubmit={() => {}} onCancel={() => {}} />)
    await fireEvent.change(getByTestId('input-description'), { target: { value: 'A new description' } })
  ...
  it('calls onCancel when cancel button clicked', async () => {
    let cancelled = false
    const { getByTestId } = await render(<VendorEditForm onSubmit={() => {}} onCancel={() => { cancelled = true }} />)
  ...
  it('does not call onSubmit when validation fails', async () => {
    let submitted = false
    const { getByTestId } = await render(<VendorEditForm onSubmit={() => { submitted = true }} onCancel={() => {}} />)
    await fireEvent.click(getByTestId('submit-button'))
  ...
  it('calls onSubmit when form is valid', async () => {
    let submitted = false
    const { getByTestId } = await render(<VendorEditForm vendor={mockVendor} onSubmit={() => { submitted = true }} onCancel={() => {}} />)
    await fireEvent.click(getByTestId('submit-button'))
  ...
  it('passes correct data to onSubmit', async () => {
    ...
    const { getByTestId } = await render(<VendorEditForm vendor={mockVendor} onSubmit={v => { data = v }} onCancel={() => {}} />)
    await fireEvent.click(getByTestId('submit-button'))
  ...
  it('hides error alert when form is valid', async () => {
    const { queryByTestId } = await render(<VendorEditForm vendor={mockVendor} onSubmit={() => {}} onCancel={() => {}} />)
    expect(queryByTestId('form-errors')).toBeNull()
  })

  // Loading state (10)
  it('disables submit button when loading', async () => {
    const { getByTestId } = await render(<VendorEditForm onSubmit={() => {}} onCancel={() => {}} loading />)
  ...
  it('loading snapshot', async () => {
    await render(<VendorEditForm vendor={mockVendor} onSubmit={() => {}} onCancel={() => {}} loading />)
  ...
  for (const risk of riskLevels) {
    it(`pre-populates ${risk} risk level`, async () => {
      const { getByTestId } = await render(<VendorEditForm vendor={{ ...mockVendor, riskLevel: risk }} onSubmit={() => {}} onCancel={() => {}} />)
  ...
  for (const status of statuses) {
    it(`pre-populates ${status} status`, async () => {
      const { getByTestId } = await render(<VendorEditForm vendor={{ ...mockVendor, status }} onSubmit={() => {}} onCancel={() => {}} />)
  ...
  it('renders with vendor without description', async () => {
    const { getByTestId } = await render(<VendorEditForm vendor={{ ...mockVendor, description: undefined }} onSubmit={() => {}} onCancel={() => {}} />)
    expect(getByTestId('input-description').value).toBe('')
  })

  it('renders with vendor with empty tags', async () => {
    const { getByTestId } = await render(<VendorEditForm vendor={{ ...mockVendor, tags: [] }} onSubmit={() => {}} onCancel={() => {}} />)
  ...
  it('invalid email shows error message', async () => {
    ...
    await fireEvent.change(getByTestId('input-name'), { target: { value: 'Test' } })
    await fireEvent.change(getByTestId('input-website'), { target: { value: 'https://test.com' } })
    await fireEvent.change(getByTestId('input-contact-email'), { target: { value: 'not-an-email' } })
  ...
  it('input-contact-email data-testid is input-contact-email', async () => {
    const { getByTestId } = await render(<VendorEditForm onSubmit={() => {}} onCancel={() => {}} />)
    expect(getByTestId('input-contact-email').getAttribute('data-testid')).toBe('input-contact-email')
  })
  ...
  it('pre-populates website for editing', async () => {
    const { getByTestId } = await render(<VendorEditForm vendor={{ ...mockVendor, website: 'https://globex.com' }} onSubmit={() => {}} onCancel={() => {}} />)
    expect(getByTestId('input-website').value).toBe('https://globex.com')
  })

  it('pre-populates name for Globex', async () => {
    const { getByTestId } = await render(<VendorEditForm vendor={{ ...mockVendor, name: 'Globex Inc' }} onSubmit={() => {}} onCancel={() => {}} />)
    expect(getByTestId('input-name').value).toBe('Globex Inc')
  })

  it('pre-populates email for different contact', async () => {
    const { getByTestId } = await render(<VendorEditForm vendor={{ ...mockVendor, contactEmail: 'bob@globex.com' }} onSubmit={() => {}} onCancel={() => {}} />)
  ...
  it('calls onCancel once on single click', async () => {
    let count = 0
    const { getByTestId } = await render(<VendorEditForm onSubmit={() => {}} onCancel={() => { count++ }} />)
    await fireEvent.click(getByTestId('cancel-button'))
    expect(count).toBe(1)
  })
  ...
  it('no error shown before submit attempt', async () => {
    const { queryByTestId } = await render(<VendorEditForm onSubmit={() => {}} onCancel={() => {}} />)
    expect(queryByTestId('form-errors')).toBeNull()
  })
