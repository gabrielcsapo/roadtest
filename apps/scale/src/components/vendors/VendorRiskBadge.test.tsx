...
describe('VendorRiskBadge', () => {
  ...
  it('shows risk badge for low risk', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'low' }} />)
    expect(getByTestId('risk-badge-core')).toBeDefined()
  })

  it('shows risk badge for critical risk', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'critical' }} />)
    expect(getByTestId('risk-badge-core')).toBeDefined()
  })
  ...
  for (const risk of riskLevels) {
    it(`renders ${risk} risk badge correctly`, async () => {
      const { getByTestId } = await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: risk }} />)
      expect(getByTestId('risk-badge-core')).toBeDefined()
    })
  }
  ...
  for (const size of sizes) {
    ...
    it(`renders with trend and size ${size}`, async () => {
      const { getByTestId } = await render(<VendorRiskBadge vendor={mockVendor} size={size} showTrend trend="up" />)
  ...
  for (const trend of trends) {
    it(`renders ${trend} trend icon when showTrend=true`, async () => {
      const { getByTestId } = await render(<VendorRiskBadge vendor={mockVendor} showTrend trend={trend} />)
      expect(getByTestId('risk-trend')).toBeDefined()
    })
    ...
    it(`hides trend when showTrend=false for ${trend}`, async () => {
      const { queryByTestId } = await render(<VendorRiskBadge vendor={mockVendor} showTrend={false} trend={trend} />)
      expect(queryByTestId('risk-trend')).toBeNull()
    })
  }
  ...
  it('shows assessment date when showDate=true and date provided', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={mockVendor} showDate assessmentDate="2024-01-15" />)
    expect(getByTestId('risk-assessment-date')).toBeDefined()
  })

  it('hides assessment date when showDate=false', async () => {
    const { queryByTestId } = await render(<VendorRiskBadge vendor={mockVendor} showDate={false} assessmentDate="2024-01-15" />)
    expect(queryByTestId('risk-assessment-date')).toBeNull()
  })
  ...
  it('shows correct assessment date text', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={mockVendor} showDate assessmentDate="2024-03-20" />)
    expect(getByTestId('risk-assessment-date').textContent).toContain('2024-03-20')
  })

  it('shows different assessment date', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={mockVendor} showDate assessmentDate="2023-12-01" />)
  ...
  it('snapshot with trend and date', async () => {
    await render(<VendorRiskBadge vendor={mockVendor} showTrend showDate trend="up" assessmentDate="2024-01-15" />)
    await snapshot('vendor-risk-badge-trend-and-date')
  })

  it('renders with trend and date for all risk levels', async () => {
    for (const risk of riskLevels) {
      const { getByTestId } = await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: risk }} showTrend showDate trend="stable" assessmentDate="2024-01-01" />)
      expect(getByTestId('risk-assessment-date')).toBeDefined()
    }
  })

  it('renders with all props', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={mockVendor} showTrend showDate trend="up" assessmentDate="2024-01-15" size="lg" />)
    expect(getByTestId('vendor-risk-badge')).toBeDefined()
    expect(getByTestId('risk-trend')).toBeDefined()
    expect(getByTestId('risk-assessment-date')).toBeDefined()
  })
  ...
  it('renders correctly for all vendors with all props', async () => {
    for (const v of vendors) {
      const { getByTestId } = await render(<VendorRiskBadge vendor={v} showTrend showDate trend="down" assessmentDate="2024-06-01" size="sm" />)
      expect(getByTestId('vendor-risk-badge')).toBeDefined()
    }
  })
  ...
  it('up trend shows up arrow', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={mockVendor} showTrend trend="up" />)
    expect(getByTestId('risk-trend').textContent).toContain('↑')
  })

  it('down trend shows down arrow', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={mockVendor} showTrend trend="down" />)
    expect(getByTestId('risk-trend').textContent).toContain('↓')
  })

  it('stable trend shows right arrow', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={mockVendor} showTrend trend="stable" />)
    expect(getByTestId('risk-trend').textContent).toContain('→')
  })

  it('snapshot critical with up trend', async () => {
    await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'critical' }} showTrend trend="up" />)
    await snapshot('vendor-risk-badge-critical-up')
  })

  it('snapshot low with down trend', async () => {
    await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'low' }} showTrend trend="down" />)
  ...
  it('renders low risk with sm size', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'low' }} size="sm" />)
    expect(getByTestId('vendor-risk-badge').getAttribute('data-size')).toBe('sm')
  })

  it('renders medium risk with md size', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'medium' }} size="md" />)
    expect(getByTestId('vendor-risk-badge').getAttribute('data-size')).toBe('md')
  })

  it('renders high risk with lg size', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'high' }} size="lg" />)
    expect(getByTestId('vendor-risk-badge').getAttribute('data-size')).toBe('lg')
  })

  it('renders critical risk with up trend and sm size', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'critical' }} showTrend trend="up" size="sm" />)
    expect(getByTestId('risk-trend').getAttribute('data-trend')).toBe('up')
  })

  it('renders low risk with down trend and lg size', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'low' }} showTrend trend="down" size="lg" />)
  ...
  it('renders medium risk with stable trend and md size', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'medium' }} showTrend trend="stable" size="md" />)
    expect(getByTestId('risk-trend').getAttribute('data-trend')).toBe('stable')
  })

  it('renders high risk with date and sm size', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'high' }} showDate assessmentDate="2024-01-01" size="sm" />)
  ...
  it('renders critical risk with date and lg size', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'critical' }} showDate assessmentDate="2024-06-15" size="lg" />)
  ...
  it('renders low risk with all features at sm', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'low' }} showTrend showDate trend="stable" assessmentDate="2024-01-01" size="sm" />)
    expect(getByTestId('vendor-risk-badge').getAttribute('data-size')).toBe('sm')
  })

  it('renders medium risk with all features at lg', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'medium' }} showTrend showDate trend="up" assessmentDate="2024-02-01" size="lg" />)
    expect(getByTestId('vendor-risk-badge').getAttribute('data-size')).toBe('lg')
  })
  ...
  it('snapshot medium risk lg with stable trend', async () => {
    await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'medium' }} size="lg" showTrend trend="stable" />)
    await snapshot('vendor-risk-badge-medium-lg-stable')
  })

  it('snapshot high risk md with date', async () => {
    await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'high' }} size="md" showDate assessmentDate="2024-03-15" />)
    await snapshot('vendor-risk-badge-high-md-date')
  })

  it('snapshot critical risk sm with up trend and date', async () => {
    await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'critical' }} size="sm" showTrend trend="up" showDate assessmentDate="2024-01-15" />)
  ...
  it('renders each risk with each trend', async () => {
    for (const risk of riskLevels) {
      for (const trend of trends) {
        const { getByTestId } = await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: risk }} showTrend trend={trend} />)
        expect(getByTestId('risk-trend').getAttribute('data-trend')).toBe(trend)
      }
    }
  })
  ...
  it('assessment date is hidden when showDate false regardless of date value', async () => {
    const { queryByTestId } = await render(<VendorRiskBadge vendor={mockVendor} showDate={false} assessmentDate="2024-01-01" />)
    expect(queryByTestId('risk-assessment-date')).toBeNull()
  })
  ...
  it('renders assessment date 2023-01-01 correctly', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={mockVendor} showDate assessmentDate="2023-01-01" />)
  ...
  it('renders assessment date 2025-12-31 correctly', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={mockVendor} showDate assessmentDate="2025-12-31" />)
  ...
  it('renders vendor Acme Corp with all options', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={vendors[0]} showTrend showDate trend="up" assessmentDate="2024-01-15" size="md" />)
    expect(getByTestId('risk-badge-core')).toBeDefined()
  })

  it('renders vendor Globex Inc with all options', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={vendors[1]} showTrend showDate trend="down" assessmentDate="2024-02-01" size="lg" />)
    expect(getByTestId('risk-assessment-date')).toBeDefined()
  })
  ...
  it('renders vendor Initech with trend only', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={vendors[3]} showTrend trend="stable" />)
    expect(getByTestId('risk-trend')).toBeDefined()
  })

  it('renders vendor Massive Dynamic with date only', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={vendors[4]} showDate assessmentDate="2024-05-01" />)
    expect(getByTestId('risk-assessment-date')).toBeDefined()
  })
  ...
  it('risk-assessment-date data-testid is risk-assessment-date when shown', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={mockVendor} showDate assessmentDate="2024-01-15" />)
    expect(getByTestId('risk-assessment-date').getAttribute('data-testid')).toBe('risk-assessment-date')
  })
  ...
  it('renders medium risk badge core text contains medium', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'medium' }} />)
  ...
  it('renders high risk badge core text contains high', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'high' }} />)
  ...
  it('snapshot with date and trend', async () => {
    await render(<VendorRiskBadge vendor={mockVendor} showTrend trend="stable" showDate assessmentDate="2024-06-01" />)
  ...
  it('renders all 5 vendors with showDate and date', async () => {
    for (const vendor of vendors) {
      const { getByTestId } = await render(<VendorRiskBadge vendor={vendor} showDate assessmentDate="2024-01-01" />)
      expect(getByTestId('risk-assessment-date')).toBeDefined()
    }
  })
  ...
  it('renders all vendors with all trends without error', async () => {
    for (const vendor of vendors) {
      for (const trend of trends) {
        const { getByTestId } = await render(<VendorRiskBadge vendor={vendor} showTrend trend={trend} />)
        expect(getByTestId('vendor-risk-badge')).toBeDefined()
      }
    }
  })
  ...
  it('snapshot low risk with down trend lg', async () => {
    await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'low' }} showTrend trend="down" size="lg" />)
    await snapshot('vendor-risk-badge-low-down-lg')
  })

  it('snapshot critical with all options', async () => {
    await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'critical' }} showTrend trend="up" showDate assessmentDate="2024-12-01" size="lg" />)
    await snapshot('vendor-risk-badge-critical-all-lg')
  })

  it('renders all trends for critical risk', async () => {
    for (const trend of trends) {
      const { getByTestId } = await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: 'critical' }} showTrend trend={trend} />)
      expect(getByTestId('risk-trend').getAttribute('data-trend')).toBe(trend)
    }
  })
  ...
  it('renders all sizes for each risk level', async () => {
    for (const risk of riskLevels) {
      for (const size of sizes) {
        const { getByTestId } = await render(<VendorRiskBadge vendor={{ ...mockVendor, riskLevel: risk }} size={size} />)
        expect(getByTestId('vendor-risk-badge').getAttribute('data-size')).toBe(size)
      }
    }
  })
  ...
  it('renders assessment date 2024-03-01 correctly', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={mockVendor} showDate assessmentDate="2024-03-01" />)
  ...
  it('renders assessment date 2024-07-04 correctly', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={mockVendor} showDate assessmentDate="2024-07-04" />)
  ...
  it('renders assessment date 2024-12-31 correctly', async () => {
    const { getByTestId } = await render(<VendorRiskBadge vendor={mockVendor} showDate assessmentDate="2024-12-31" />)
