...
describe('VendorTable', () => {
  ...
  it('calls onSort when name column clicked', async () => {
    ...
    const { getByTestId } = await render(<VendorTable vendors={vendors} onSort={() => { sortCalled = true }} />)
  ...
  it('calls onSort with correct field when name clicked', async () => {
    let sortField = ''
    const { getByTestId } = await render(<VendorTable vendors={vendors} onSort={s => { sortField = s.field }} />)
    await fireEvent.click(getByTestId('col-name'))
  ...
  it('calls onSort with asc direction on first click', async () => {
    let sortDir = ''
    const { getByTestId } = await render(<VendorTable vendors={vendors} onSort={s => { sortDir = s.direction }} />)
    await fireEvent.click(getByTestId('col-name'))
  ...
  it('marks multiple rows as selected', async () => {
    const { container } = await render(<VendorTable vendors={vendors} selectedIds={['1', '2', '3']} />)
  ...
  it('calls onSelect when row clicked', async () => {
    ...
    const { getByTestId } = await render(<VendorTable vendors={[mockVendor]} onSelect={v => { selected = v }} />)
  ...
  it('edit button calls onEdit', async () => {
    ...
    const { getByTestId } = await render(<VendorTable vendors={[mockVendor]} onEdit={v => { edited = v }} />)
  ...
  it('delete button calls onDelete', async () => {
    ...
    const { getByTestId } = await render(<VendorTable vendors={[mockVendor]} onDelete={v => { deleted = v }} />)
  ...
  for (const risk of riskLevels) {
    it(`renders ${risk} risk row correctly`, async () => {
      const { getByTestId } = await render(<VendorTable vendors={[{ ...mockVendor, riskLevel: risk }]} />)
      expect(getByTestId('risk-1')).toBeDefined()
    })
  }
  ...
  it('renders all action buttons when both handlers provided', async () => {
    const { container } = await render(<VendorTable vendors={vendors} onEdit={() => {}} onDelete={() => {}} />)
  ...
  it('vendor-table-container data-testid is vendor-table-container', async () => {
    const { getByTestId } = await render(<VendorTable vendors={vendors} />)
    expect(getByTestId('vendor-table-container').getAttribute('data-testid')).toBe('vendor-table-container')
  })
  ...
  it('vendor-table-header data-testid is vendor-table-header', async () => {
    const { getByTestId } = await render(<VendorTable vendors={vendors} />)
    expect(getByTestId('vendor-table-header').getAttribute('data-testid')).toBe('vendor-table-header')
  })
  ...
  it('calling onSelect with vendor 2 returns correct vendor', async () => {
    let selected: Vendor | null = null
    const { getByTestId } = await render(<VendorTable vendors={vendors} onSelect={v => { selected = v }} />)
  ...
  it('onEdit with vendor 2 returns vendor 2', async () => {
    let edited: Vendor | null = null
    const { getByTestId } = await render(<VendorTable vendors={vendors} onEdit={v => { edited = v }} />)
  ...
  it('onDelete with vendor 2 returns vendor 2', async () => {
    let deleted: Vendor | null = null
    const { getByTestId } = await render(<VendorTable vendors={vendors} onDelete={v => { deleted = v }} />)
