...
describe("VendorReviewSchedule", () => {
  ...
  it("shows correct vendor last review date for different vendors", async () => {
    ...
    const { getByTestId } = await render(
      <VendorReviewSchedule vendor={{ ...mockVendor, lastReviewDate: date }} />,
    );
  ...
  it("shows custom 90 day interval", async () => {
    const { getByTestId } = await render(
      <VendorReviewSchedule vendor={mockVendor} reviewIntervalDays={90} />,
    );
  ...
  it("shows custom 180 day interval", async () => {
    const { getByTestId } = await render(
      <VendorReviewSchedule vendor={mockVendor} reviewIntervalDays={180} />,
    );
  ...
  it("shows custom 30 day interval", async () => {
    const { getByTestId } = await render(
      <VendorReviewSchedule vendor={mockVendor} reviewIntervalDays={30} />,
    );
  ...
  it("next review date is interval days after last review", async () => {
    ...
    const { getByTestId } = await render(
      <VendorReviewSchedule
        vendor={{ ...mockVendor, lastReviewDate: lastReview }}
  ...
  it("shows overdue alert when review is overdue", async () => {
    const { getByTestId } = await render(
      <VendorReviewSchedule vendor={{ ...mockVendor, lastReviewDate: pastDate }} />,
    );
  ...
  it("due soon alert shown when < 30 days away", async () => {
    const { getByTestId } = await render(
      <VendorReviewSchedule vendor={{ ...mockVendor, lastReviewDate: soonDate }} />,
    );
  ...
  it("shows schedule review button when handler provided", async () => {
    const { getByTestId } = await render(
      <VendorReviewSchedule vendor={mockVendor} onScheduleReview={() => {}} />,
    );
  ...
  for (const risk of riskLevels) {
    it(`renders schedule for ${risk} risk vendor`, async () => {
      const { getByTestId } = await render(
        <VendorReviewSchedule vendor={{ ...mockVendor, riskLevel: risk }} />,
      );
      expect(getByTestId("vendor-review-schedule")).toBeDefined();
    });
  }
  ...
  for (const status of statuses) {
    it(`renders schedule for ${status} vendor`, async () => {
      const { getByTestId } = await render(
        <VendorReviewSchedule vendor={{ ...mockVendor, status }} />,
      );
      expect(getByTestId("vendor-review-schedule")).toBeDefined();
    });
  }
  ...
  it("renders with reviewIntervalDays=1", async () => {
    const { getByTestId } = await render(
      <VendorReviewSchedule vendor={mockVendor} reviewIntervalDays={1} />,
    );
  ...
  it("renders with very old review date", async () => {
    const { getByTestId } = await render(
      <VendorReviewSchedule vendor={{ ...mockVendor, lastReviewDate: "2010-01-01" }} />,
    );
    expect(getByTestId("overdue-alert")).toBeDefined();
  });
  ...
  it("renders correctly with interval of 7 days", async () => {
    const { getByTestId } = await render(
      <VendorReviewSchedule vendor={mockVendor} reviewIntervalDays={7} />,
    );
  ...
  it("renders correctly with interval of 730 days", async () => {
    const { getByTestId } = await render(
      <VendorReviewSchedule vendor={mockVendor} reviewIntervalDays={730} />,
    );
  ...
  it("shows different vendor names correctly", async () => {
    const { getByTestId } = await render(
      <VendorReviewSchedule vendor={{ ...mockVendor, lastReviewDate: "2024-03-01" }} />,
    );
  ...
  it("schedule review button text matches for each vendor", async () => {
    for (const vendor of vendors) {
      const { getByTestId } = await render(
        <VendorReviewSchedule vendor={vendor} onScheduleReview={() => {}} />,
      );
      expect(getByTestId("schedule-review-button").textContent).toContain("Schedule Review");
    }
  });
  ...
  it("shows last-review-date for 2024-01-15", async () => {
    const { getByTestId } = await render(
      <VendorReviewSchedule vendor={{ ...mockVendor, lastReviewDate: "2024-01-15" }} />,
    );
  ...
  it("shows last-review-date for 2023-09-01", async () => {
    const { getByTestId } = await render(
      <VendorReviewSchedule vendor={{ ...mockVendor, lastReviewDate: "2023-09-01" }} />,
    );
  ...
  it("shows last-review-date for 2022-06-15", async () => {
    const { getByTestId } = await render(
      <VendorReviewSchedule vendor={{ ...mockVendor, lastReviewDate: "2022-06-15" }} />,
    );
  ...
  it("overdue alert contains the days count", async () => {
    const { getByTestId } = await render(
      <VendorReviewSchedule vendor={{ ...mockVendor, lastReviewDate: "2020-01-01" }} />,
    );
  ...
  it("next review section shows date for 90 day interval", async () => {
    const { getByTestId } = await render(
      <VendorReviewSchedule
        vendor={{ ...mockVendor, lastReviewDate: "2024-01-01" }}
        reviewIntervalDays={90}
      />,
    );
  ...
  it("renders correctly for all 5 vendors without button", async () => {
    for (const vendor of vendors) {
      const { queryByTestId } = await render(
        <VendorReviewSchedule vendor={vendor} reviewIntervalDays={365} />,
      );
      expect(queryByTestId("schedule-review-button")).toBeNull();
    }
  });
  ...
  it("vendor-review-schedule data-testid is vendor-review-schedule", async () => {
    const { getByTestId } = await render(<VendorReviewSchedule vendor={mockVendor} />);
    expect(getByTestId("vendor-review-schedule").getAttribute("data-testid")).toBe(
  ...
  it("review-schedule-title data-testid is review-schedule-title", async () => {
    const { getByTestId } = await render(<VendorReviewSchedule vendor={mockVendor} />);
    expect(getByTestId("review-schedule-title").getAttribute("data-testid")).toBe(
  ...
  it("last-review-section data-testid is last-review-section", async () => {
    const { getByTestId } = await render(<VendorReviewSchedule vendor={mockVendor} />);
    expect(getByTestId("last-review-section").getAttribute("data-testid")).toBe(
      "last-review-section",
    );
  });
  ...
  it("next-review-section data-testid is next-review-section", async () => {
    const { getByTestId } = await render(<VendorReviewSchedule vendor={mockVendor} />);
    expect(getByTestId("next-review-section").getAttribute("data-testid")).toBe(
      "next-review-section",
    );
  });
  ...
  it("review-status-badge data-testid is review-status-badge", async () => {
    const { getByTestId } = await render(<VendorReviewSchedule vendor={mockVendor} />);
    expect(getByTestId("review-status-badge").getAttribute("data-testid")).toBe(
      "review-status-badge",
    );
  });
  ...
  it("schedule-review-button data-testid is schedule-review-button", async () => {
    ...
    expect(getByTestId("schedule-review-button").getAttribute("data-testid")).toBe(
  ...
  it("review-interval-section data-testid is review-interval-section", async () => {
    ...
    expect(getByTestId("review-interval-section").getAttribute("data-testid")).toBe(
      "review-interval-section",
    );
  });
  ...
  it("snapshot for all vendors with schedule button", async () => {
    for (const vendor of vendors) {
      ...
      await snapshot(
        `vendor-review-schedule-${vendor.name.toLowerCase().replace(/\s+/g, "-")}-btn`,
      );
    }
  });
});
