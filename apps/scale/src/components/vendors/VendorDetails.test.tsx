...
describe('VendorDetails', () => {
  ...
  it('hides description when not present', async () => {
    const { queryByTestId } = await render(<VendorDetails vendor={{ ...mockVendor, description: undefined }} />)
  ...
  it('calls onEdit with vendor when edit clicked', async () => {
    ...
    const { getByTestId } = await render(<VendorDetails vendor={mockVendor} onEdit={v => { edited = v }} />)
  ...
  it('shows archive button when onArchive provided', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={mockVendor} onArchive={() => {}} />)
  ...
  it('calls onArchive with vendor when archive clicked', async () => {
    ...
    const { getByTestId } = await render(<VendorDetails vendor={mockVendor} onArchive={v => { archived = v }} />)
  ...
  it('shows both buttons when both handlers provided', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={mockVendor} onEdit={() => {}} onArchive={() => {}} />)
    expect(getByTestId('details-edit-button')).toBeDefined()
    expect(getByTestId('details-archive-button')).toBeDefined()
  })
  ...
  for (const risk of riskLevels) {
    it(`renders ${risk} risk correctly`, async () => {
      const { getByTestId } = await render(<VendorDetails vendor={{ ...mockVendor, riskLevel: risk }} />)
      expect(getByTestId('details-risk-badge')).toBeDefined()
    })
  }
  ...
  it('renders without description', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={{ ...mockVendor, description: undefined }} />)
    expect(getByTestId('vendor-details')).toBeDefined()
  })
  ...
  it('renders vendor with many tags', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={{ ...mockVendor, tags: ['a', 'b', 'c', 'd', 'e'] }} />)
    expect(getByTestId('overview-tags')).toBeDefined()
  })

  it('renders vendor with special chars in name', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={{ ...mockVendor, name: 'Acme & Co.' }} />)
  ...
  it('renders correctly with different contact email', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={{ ...mockVendor, contactEmail: 'ciso@corp.com' }} />)
  ...
  it('renders correctly with different review date', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={{ ...mockVendor, lastReviewDate: '2023-12-01' }} />)
  ...
  it('renders correctly with different website', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={{ ...mockVendor, website: 'https://newsite.com' }} />)
  ...
  it('overview website link has correct href', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={{ ...mockVendor, website: 'https://acme.com' }} />)
    const link = getByTestId('overview-website').querySelector('a')
    expect(link?.getAttribute('href')).toBe('https://acme.com')
  })
  ...
  it('edit button calls onEdit for Acme Corp', async () => {
    let edited: Vendor | null = null
    const { getByTestId } = await render(<VendorDetails vendor={vendors[0]} onEdit={v => { edited = v }} />)
    await fireEvent.click(getByTestId('details-edit-button'))
  ...
  it('archive button calls onArchive for Globex Inc', async () => {
    let archived: Vendor | null = null
    const { getByTestId } = await render(<VendorDetails vendor={vendors[1]} onArchive={v => { archived = v }} />)
    await fireEvent.click(getByTestId('details-archive-button'))
  ...
  it('vendor-details-name data-testid is vendor-details-name', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={mockVendor} />)
    expect(getByTestId('vendor-details-name').getAttribute('data-testid')).toBe('vendor-details-name')
  })

  it('vendor-details-content data-testid is vendor-details-content', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={mockVendor} />)
    expect(getByTestId('vendor-details-content').getAttribute('data-testid')).toBe('vendor-details-content')
  })

  it('vendor-details-tabs data-testid is vendor-details-tabs', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={mockVendor} />)
    expect(getByTestId('vendor-details-tabs').getAttribute('data-testid')).toBe('vendor-details-tabs')
  })
  ...
  it('overview-review-date data-testid is overview-review-date', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={mockVendor} />)
    expect(getByTestId('overview-review-date').getAttribute('data-testid')).toBe('overview-review-date')
  })

  it('details-category-badge data-testid is details-category-badge', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={mockVendor} />)
    expect(getByTestId('details-category-badge').getAttribute('data-testid')).toBe('details-category-badge')
  })

  it('details-edit-button data-testid is details-edit-button', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={mockVendor} onEdit={() => {}} />)
    expect(getByTestId('details-edit-button').getAttribute('data-testid')).toBe('details-edit-button')
  })

  it('details-archive-button data-testid is details-archive-button', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={mockVendor} onArchive={() => {}} />)
    expect(getByTestId('details-archive-button').getAttribute('data-testid')).toBe('details-archive-button')
  })

  it('edit fires for Umbrella Corp', async () => {
    let edited: Vendor | null = null
    const { getByTestId } = await render(<VendorDetails vendor={vendors[2]} onEdit={v => { edited = v }} />)
    await fireEvent.click(getByTestId('details-edit-button'))
  ...
  it('archive fires for Initech', async () => {
    let archived: Vendor | null = null
    const { getByTestId } = await render(<VendorDetails vendor={vendors[3]} onArchive={v => { archived = v }} />)
    await fireEvent.click(getByTestId('details-archive-button'))
  ...
  it('edit fires once on single click', async () => {
    let count = 0
    const { getByTestId } = await render(<VendorDetails vendor={mockVendor} onEdit={() => { count++ }} />)
    await fireEvent.click(getByTestId('details-edit-button'))
    expect(count).toBe(1)
  })
  ...
  it('archive fires once on single click', async () => {
    let count = 0
    const { getByTestId } = await render(<VendorDetails vendor={mockVendor} onArchive={() => { count++ }} />)
    await fireEvent.click(getByTestId('details-archive-button'))
    expect(count).toBe(1)
  })
  ...
  it('vendor-details-header data-testid is vendor-details-header', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={mockVendor} />)
    expect(getByTestId('vendor-details-header').getAttribute('data-testid')).toBe('vendor-details-header')
  })

  it('vendor-details-actions data-testid is vendor-details-actions when actions provided', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={mockVendor} onEdit={() => {}} />)
    expect(getByTestId('vendor-details-actions').getAttribute('data-testid')).toBe('vendor-details-actions')
  })
  ...
  it('vendor-details-loading data-testid is vendor-details-loading', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={mockVendor} loading />)
    expect(getByTestId('vendor-details-loading').getAttribute('data-testid')).toBe('vendor-details-loading')
  })

  it('overview-description contains vendor description text', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={mockVendor} />)
    expect(getByTestId('overview-description').textContent).toContain('A cloud infrastructure provider.')
  })

  it('overview-tag-cloud data-testid includes tag', async () => {
    const { getByTestId } = await render(<VendorDetails vendor={mockVendor} />)
    expect(getByTestId('overview-tag-cloud').getAttribute('data-testid')).toBe('overview-tag-cloud')
  })
