...
describe('VendorBulkActions', () => {
  ...
  it('shows archive button when onArchive provided', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onArchive={() => {}} />)
  ...
  it('calls onArchive when archive clicked', async () => {
    let archived = false
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onArchive={() => { archived = true }} />)
  ...
  it('archive button is disabled when loading', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onArchive={() => {}} loading />)
  ...
  it('fires onArchive multiple times', async () => {
    let count = 0
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onArchive={() => { count++ }} />)
    await fireEvent.click(getByTestId('bulk-archive-button'))
    await fireEvent.click(getByTestId('bulk-archive-button'))
    expect(count).toBe(2)
  })
  ...
  it('archive button not disabled when loading=false', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onArchive={() => {}} loading={false} />)
    expect(getByTestId('bulk-archive-button').disabled).toBe(false)
  })

  it('does not call onArchive when loading and button disabled', async () => {
    let archived = false
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onArchive={() => { archived = true }} loading />)
  ...
  it('shows delete button when onDelete provided', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onDelete={() => {}} />)
  ...
  it('calls onDelete when delete clicked', async () => {
    let deleted = false
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onDelete={() => { deleted = true }} />)
    await fireEvent.click(getByTestId('bulk-delete-button'))
    expect(deleted).toBeTruthy()
  })

  it('delete button is disabled when loading', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onDelete={() => {}} loading />)
  ...
  it('fires onDelete multiple times', async () => {
    let count = 0
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onDelete={() => { count++ }} />)
    await fireEvent.click(getByTestId('bulk-delete-button'))
    await fireEvent.click(getByTestId('bulk-delete-button'))
    expect(count).toBe(2)
  })
  ...
  it('archive and delete buttons both visible', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onArchive={() => {}} onDelete={() => {}} />)
    expect(getByTestId('bulk-archive-button')).toBeDefined()
    expect(getByTestId('bulk-delete-button')).toBeDefined()
  })
  ...
  it('does not call onDelete when loading and button disabled', async () => {
    let deleted = false
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onDelete={() => { deleted = true }} loading />)
  ...
  it('shows risk change section when onChangeRisk provided', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onChangeRisk={() => {}} />)
  ...
  it('calls onChangeRisk with selected risk', async () => {
    ...
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onChangeRisk={r => { received = r }} />)
  ...
  it('shows export button when onExport provided', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onExport={() => {}} />)
  ...
  it('calls onExport when export clicked', async () => {
    let exported = false
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onExport={() => { exported = true }} />)
    await fireEvent.click(getByTestId('bulk-export-button'))
    expect(exported).toBeTruthy()
  })

  it('export button is disabled when loading', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onExport={() => {}} loading />)
  ...
  it('snapshot loading state', async () => {
    await render(<VendorBulkActions selectedCount={3} onArchive={() => {}} onDelete={() => {}} loading />)
    await snapshot('vendor-bulk-actions-loading')
  })

  it('snapshot all actions', async () => {
    await render(<VendorBulkActions selectedCount={5} onArchive={() => {}} onDelete={() => {}} onChangeRisk={() => {}} onExport={() => {}} />)
    await snapshot('vendor-bulk-actions-all')
  })

  it('risk select disabled when loading', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onChangeRisk={() => {}} loading />)
  ...
  it('renders correctly with only archive', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={5} onArchive={() => {}} />)
    expect(getByTestId('bulk-archive-button')).toBeDefined()
    expect(queryByTestId => queryByTestId('bulk-delete-button')).toBeDefined()
  })

  it('renders correctly with only delete', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={5} onDelete={() => {}} />)
    expect(getByTestId('bulk-delete-button')).toBeDefined()
  })

  it('renders correctly with only risk change', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={5} onChangeRisk={() => {}} />)
    expect(getByTestId('bulk-risk-change')).toBeDefined()
  })

  it('renders correctly with only export', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={5} onExport={() => {}} />)
    expect(getByTestId('bulk-export-button')).toBeDefined()
  })

  it('archive and risk change both visible', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onArchive={() => {}} onChangeRisk={() => {}} />)
    expect(getByTestId('bulk-archive-button')).toBeDefined()
    expect(getByTestId('bulk-risk-change')).toBeDefined()
  })

  it('delete and export both visible', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onDelete={() => {}} onExport={() => {}} />)
    expect(getByTestId('bulk-delete-button')).toBeDefined()
    expect(getByTestId('bulk-export-button')).toBeDefined()
  })
  ...
  it('export fires and count is 1', async () => {
    let count = 0
    const { getByTestId } = await render(<VendorBulkActions selectedCount={3} onExport={() => { count++ }} />)
    await fireEvent.click(getByTestId('bulk-export-button'))
    expect(count).toBe(1)
  })
  ...
  it('snapshot with 10 selected and all actions', async () => {
    await render(<VendorBulkActions selectedCount={10} onArchive={() => {}} onDelete={() => {}} onChangeRisk={() => {}} onExport={() => {}} />)
    await snapshot('vendor-bulk-actions-10-selected-all')
  })

  it('archive button present when selectedCount is 1', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={1} onArchive={() => {}} />)
    expect(getByTestId('bulk-archive-button')).toBeDefined()
  })

  it('delete button present when selectedCount is 100', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={100} onDelete={() => {}} />)
    expect(getByTestId('bulk-delete-button')).toBeDefined()
  })

  it('export button present when selectedCount is 50', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={50} onExport={() => {}} />)
    expect(getByTestId('bulk-export-button')).toBeDefined()
  })

  it('risk select present when selectedCount is 25', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={25} onChangeRisk={() => {}} />)
    expect(getByTestId('bulk-risk-select')).toBeDefined()
  })
  ...
  it('vendor-bulk-actions data-testid is vendor-bulk-actions', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={5} />)
    expect(getByTestId('vendor-bulk-actions').getAttribute('data-testid')).toBe('vendor-bulk-actions')
  })

  it('selected-count-badge data-testid is selected-count-badge', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={5} />)
    expect(getByTestId('selected-count-badge').getAttribute('data-testid')).toBe('selected-count-badge')
  })

  it('bulk-action-buttons data-testid is bulk-action-buttons', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={5} />)
    expect(getByTestId('bulk-action-buttons').getAttribute('data-testid')).toBe('bulk-action-buttons')
  })

  it('bulk-archive-button data-testid is bulk-archive-button', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={5} onArchive={() => {}} />)
    expect(getByTestId('bulk-archive-button').getAttribute('data-testid')).toBe('bulk-archive-button')
  })

  it('bulk-delete-button data-testid is bulk-delete-button', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={5} onDelete={() => {}} />)
    expect(getByTestId('bulk-delete-button').getAttribute('data-testid')).toBe('bulk-delete-button')
  })

  it('bulk-export-button data-testid is bulk-export-button', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={5} onExport={() => {}} />)
    expect(getByTestId('bulk-export-button').getAttribute('data-testid')).toBe('bulk-export-button')
  })
  ...
  it('bulk-risk-apply-button data-testid is bulk-risk-apply-button', async () => {
    const { getByTestId } = await render(<VendorBulkActions selectedCount={5} onChangeRisk={() => {}} />)
    expect(getByTestId('bulk-risk-apply-button').getAttribute('data-testid')).toBe('bulk-risk-apply-button')
  })
