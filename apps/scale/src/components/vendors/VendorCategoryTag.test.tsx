...
describe('VendorCategoryTag', () => {
  ...
  it('calls onClick when tag clicked', async () => {
    let clicked = false
    const { getByTestId } = await render(<VendorCategoryTag category="Security" onClick={() => { clicked = true }} />)
  ...
  it('passes category to onClick', async () => {
    let received = ''
    const { getByTestId } = await render(<VendorCategoryTag category="Security" onClick={c => { received = c }} />)
    await fireEvent.click(getByTestId('vendor-category-tag'))
  ...
  it('fires onClick for each category', async () => {
    for (const cat of categories) {
      let received = ''
      const { getByTestId } = await render(<VendorCategoryTag category={cat} onClick={c => { received = c }} />)
      await fireEvent.click(getByTestId('vendor-category-tag'))
  ...
  it('fires onClick multiple times', async () => {
    let count = 0
    const { getByTestId } = await render(<VendorCategoryTag category="Security" onClick={() => { count++ }} />)
    await fireEvent.click(getByTestId('vendor-category-tag'))
    await fireEvent.click(getByTestId('vendor-category-tag'))
    expect(count).toBe(2)
  })
  ...
  it('calls onRemove when remove button clicked', async () => {
    let removed = false
    const { getByTestId } = await render(<VendorCategoryTag category="Security" onRemove={() => { removed = true }} />)
    await fireEvent.click(getByTestId('category-tag-remove'))
  ...
  it('passes category to onRemove', async () => {
    let received = ''
    const { getByTestId } = await render(<VendorCategoryTag category="Security" onRemove={c => { received = c }} />)
    await fireEvent.click(getByTestId('category-tag-remove'))
    expect(received).toBe('Security')
  })

  it('shows remove button when onRemove provided', async () => {
    const { getByTestId } = await render(<VendorCategoryTag category="Security" onRemove={() => {}} />)
  ...
  it('snapshot with all features', async () => {
    await render(<VendorCategoryTag category="Security" color="#dbeafe" onClick={() => {}} onRemove={() => {}} size="sm" />)
  ...
  it('renders with long category name', async () => {
    const { getByTestId } = await render(<VendorCategoryTag category="Very Long Category Name Here" />)
  ...
  it('renders with both onClick and onRemove', async () => {
    const { getByTestId } = await render(<VendorCategoryTag category="Security" onClick={() => {}} onRemove={() => {}} />)
    expect(getByTestId('category-tag-remove')).toBeDefined()
  })
  ...
  it('renders with size sm and remove button', async () => {
    const { getByTestId } = await render(<VendorCategoryTag category="Security" size="sm" onRemove={() => {}} />)
    expect(getByTestId('category-tag-remove')).toBeDefined()
  })
  ...
  it('fires onRemove for each category', async () => {
    for (const cat of categories) {
      let received = ''
      const { getByTestId } = await render(<VendorCategoryTag category={cat} onRemove={c => { received = c }} />)
      await fireEvent.click(getByTestId('category-tag-remove'))
      expect(received).toBe(cat)
    }
  })
  ...
  it('size sm tag with all props renders correctly', async () => {
    const { getByTestId } = await render(<VendorCategoryTag category="Security" size="sm" color="#dbeafe" onClick={() => {}} onRemove={() => {}} />)
    expect(getByTestId('vendor-category-tag').getAttribute('data-size')).toBe('sm')
  })

  it('size md tag with all props renders correctly', async () => {
    const { getByTestId } = await render(<VendorCategoryTag category="Security" size="md" color="#fee2e2" onClick={() => {}} onRemove={() => {}} />)
    expect(getByTestId('vendor-category-tag').getAttribute('data-size')).toBe('md')
  })

  it('clicking tag fires onClick even when onRemove provided', async () => {
    let tagClicked = false
    const { getByTestId } = await render(<VendorCategoryTag category="Security" onClick={() => { tagClicked = true }} onRemove={() => {}} />)
  ...
  it('data-category uses exact prop value', async () => {
    const { getByTestId } = await render(<VendorCategoryTag category="Cloud Infrastructure" />)
    expect(getByTestId('vendor-category-tag').getAttribute('data-category')).toBe('Cloud Infrastructure')
  })
  ...
  it('renders correctly with green color', async () => {
    const { getByTestId } = await render(<VendorCategoryTag category="Compliant" color="#dcfce7" />)
    expect(getByTestId('vendor-category-tag')).toBeDefined()
  })

  it('renders correctly with purple color', async () => {
    const { getByTestId } = await render(<VendorCategoryTag category="Enterprise" color="#f3e8ff" />)
    expect(getByTestId('vendor-category-tag')).toBeDefined()
  })

  it('renders exactly 5 categories test', async () => {
    for (const cat of categories) {
      const { getByTestId } = await render(<VendorCategoryTag category={cat} size="sm" onRemove={() => {}} onClick={() => {}} />)
      expect(getByTestId('category-tag-label').textContent).toBe(cat)
    }
  })
  ...
  it('vendor-category-tag data-testid is vendor-category-tag', async () => {
    const { getByTestId } = await render(<VendorCategoryTag category="Security" />)
    expect(getByTestId('vendor-category-tag').getAttribute('data-testid')).toBe('vendor-category-tag')
  })

  it('category-tag-label data-testid is category-tag-label', async () => {
    const { getByTestId } = await render(<VendorCategoryTag category="Security" />)
    expect(getByTestId('category-tag-label').getAttribute('data-testid')).toBe('category-tag-label')
  })

  it('category-tag-remove data-testid is category-tag-remove', async () => {
    const { getByTestId } = await render(<VendorCategoryTag category="Security" onRemove={() => {}} />)
    expect(getByTestId('category-tag-remove').getAttribute('data-testid')).toBe('category-tag-remove')
  })
  ...
  it('fires onRemove once on single click', async () => {
    let count = 0
    const { getByTestId } = await render(<VendorCategoryTag category="Security" onRemove={() => { count++ }} />)
    await fireEvent.click(getByTestId('category-tag-remove'))
    expect(count).toBe(1)
  })
  ...
  it('snapshot with all options sm', async () => {
    await render(<VendorCategoryTag category="Security" size="sm" color="#dbeafe" onClick={() => {}} onRemove={() => {}} />)
    await snapshot('vendor-category-tag-all-sm')
  })

  it('snapshot with all options md', async () => {
    await render(<VendorCategoryTag category="Security" size="md" color="#fee2e2" onClick={() => {}} onRemove={() => {}} />)
