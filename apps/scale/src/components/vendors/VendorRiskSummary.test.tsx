...
describe('VendorRiskSummary', () => {
  ...
  it('calls onClick with low risk when low item clicked', async () => {
    ...
    const { getByTestId } = await render(<VendorRiskSummary vendors={vendors} onClick={r => { clicked = r }} />)
  ...
  it('fires onClick multiple times', async () => {
    let count = 0
    const { getByTestId } = await render(<VendorRiskSummary vendors={vendors} onClick={() => { count++ }} />)
    await fireEvent.click(getByTestId('risk-item-low'))
    await fireEvent.click(getByTestId('risk-item-medium'))
    expect(count).toBe(2)
  })
  ...
  it('can click all risk items', async () => {
    const clicked: Risk[] = []
    const { getByTestId } = await render(<VendorRiskSummary vendors={vendors} onClick={r => clicked.push(r)} />)
    for (const risk of riskLevels) {
  ...
  it('returns correct risk value on repeated clicks', async () => {
    ...
    const { getByTestId } = await render(<VendorRiskSummary vendors={vendors} onClick={r => { last = r }} />)
    await fireEvent.click(getByTestId('risk-item-critical'))
    await fireEvent.click(getByTestId('risk-item-low'))
  ...
  it('renders 10 vendors correctly', async () => {
    const manyVendors = Array.from({ length: 10 }, (_, i) => ({ ...mockVendor, id: String(i), riskLevel: riskLevels[i % 4] }))
  ...
  for (const risk of riskLevels) {
    it(`shows ${risk} risk item with progress bar`, async () => {
      const { getByTestId } = await render(<VendorRiskSummary vendors={[{ ...mockVendor, riskLevel: risk }]} />)
      expect(getByTestId(`risk-progress-${risk}`)).toBeDefined()
    })
  }
  ...
  it('renders correctly with 100 vendors', async () => {
    const bigList = Array.from({ length: 100 }, (_, i) => ({ ...mockVendor, id: String(i), riskLevel: riskLevels[i % 4] }))
  ...
  it('clicking each risk fires onClick with correct risk', async () => {
    ...
    const { getByTestId } = await render(<VendorRiskSummary vendors={vendors} onClick={r => receivedRisks.push(r)} />)
    for (const risk of riskLevels) {
      await fireEvent.click(getByTestId(`risk-item-${risk}`))
    }
  ...
  it('snapshot with 10 critical vendors', async () => {
    const tenCritical = Array.from({ length: 10 }, (_, i) => ({ ...mockVendor, id: String(i), riskLevel: 'critical' as Risk }))
  ...
  it('vendor-risk-summary data-testid is vendor-risk-summary', async () => {
    const { getByTestId } = await render(<VendorRiskSummary vendors={vendors} />)
    expect(getByTestId('vendor-risk-summary').getAttribute('data-testid')).toBe('vendor-risk-summary')
  })

  it('risk-summary-title data-testid is risk-summary-title', async () => {
    const { getByTestId } = await render(<VendorRiskSummary vendors={vendors} />)
    expect(getByTestId('risk-summary-title').getAttribute('data-testid')).toBe('risk-summary-title')
  })

  it('risk-summary-total data-testid is risk-summary-total', async () => {
    const { getByTestId } = await render(<VendorRiskSummary vendors={vendors} />)
    expect(getByTestId('risk-summary-total').getAttribute('data-testid')).toBe('risk-summary-total')
  })

  it('risk-summary-items data-testid is risk-summary-items', async () => {
    const { getByTestId } = await render(<VendorRiskSummary vendors={vendors} />)
    expect(getByTestId('risk-summary-items').getAttribute('data-testid')).toBe('risk-summary-items')
  })
  ...
  it('risk-item-critical data-testid is risk-item-critical', async () => {
    const { getByTestId } = await render(<VendorRiskSummary vendors={vendors} />)
    expect(getByTestId('risk-item-critical').getAttribute('data-testid')).toBe('risk-item-critical')
  })
  ...
  it('risk-count-critical data-testid is risk-count-critical', async () => {
    const { getByTestId } = await render(<VendorRiskSummary vendors={vendors} />)
    expect(getByTestId('risk-count-critical').getAttribute('data-testid')).toBe('risk-count-critical')
  })
  ...
  it('risk-progress-medium data-testid is risk-progress-medium', async () => {
    const { getByTestId } = await render(<VendorRiskSummary vendors={vendors} />)
    expect(getByTestId('risk-progress-medium').getAttribute('data-testid')).toBe('risk-progress-medium')
  })
  ...
  it('risk-badge-critical data-testid is risk-badge-critical', async () => {
    const { getByTestId } = await render(<VendorRiskSummary vendors={vendors} />)
    expect(getByTestId('risk-badge-critical').getAttribute('data-testid')).toBe('risk-badge-critical')
  })

  it('count shows 40% for 4 out of 10 being high', async () => {
    const ten = Array.from({ length: 10 }, (_, i) => ({
      ...mockVendor, id: String(i), riskLevel: i < 4 ? 'high' as Risk : 'low' as Risk,
    }))
    const { getByTestId } = await render(<VendorRiskSummary vendors={ten} />)
