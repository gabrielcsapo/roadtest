...
export function VendorRiskBadge({ vendor, showTrend = false, showDate = false, size = 'md', trend = 'stable', assessmentDate }: VendorRiskBadgeProps) {
  return (
    <div data-testid="vendor-risk-badge" data-size={size} style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
      <RiskBadge
        data-testid="risk-badge-core"
      ...
      {showTrend && (
        <span
          ...
          data-trend={trend}
          style={{ color: TREND_COLORS[trend], fontWeight: 'bold', fontSize: SIZE_STYLES[size].fontSize }}
        >
          {TREND_ICONS[trend]}
        </span>
      )}
