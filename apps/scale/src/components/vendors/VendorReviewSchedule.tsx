...
export function VendorReviewSchedule({ vendor, reviewIntervalDays = 365, onScheduleReview }: VendorReviewScheduleProps) {
  ...
  return (
    <Card data-testid="vendor-review-schedule">
      <h3 data-testid="review-schedule-title" style={{ margin: '0 0 16px 0' }}>Review Schedule</h3>
      ...
      <div data-testid="review-info" style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '14px' }}>
        <div data-testid="last-review-section">
          ...
          <span data-testid="days-since-review" style={{ marginLeft: '8px', color: '#9ca3af', fontSize: '12px' }}>
        ...
        <div data-testid="next-review-section">
          ...
          <span data-testid="days-until-review" style={{ marginLeft: '8px' }}>
            <Badge data-testid="review-status-badge" style={{ backgroundColor: isOverdue ? '#ef4444' : isDueSoon ? '#f59e0b' : '#22c55e', color: 'white' }}>
              {isOverdue ? `${Math.abs(daysUntilNextReview)}d overdue` : isDueSoon ? `${daysUntilNextReview}d` : 'On schedule'}
            </Badge>
          </span>
        </div>
