...
describe('VendorCard', () => {
  ...
  for (const risk of riskLevels) {
    it(`shows ${risk} risk badge`, async () => {
      const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, riskLevel: risk }} />)
  ...
  it('calls onClick when card is clicked', async () => {
    let clicked = false
    const { getByTestId } = await render(<VendorCard vendor={mockVendor} onClick={() => { clicked = true }} />)
  ...
  it('passes vendor to onClick', async () => {
    ...
    const { getByTestId } = await render(<VendorCard vendor={mockVendor} onClick={v => { received = v }} />)
    await fireEvent.click(getByTestId('vendor-card'))
  ...
  it('edit button calls onEdit with vendor', async () => {
    ...
    const { getByTestId } = await render(<VendorCard vendor={mockVendor} onEdit={v => { edited = v }} />)
  ...
  it('delete button calls onDelete with vendor', async () => {
    ...
    const { getByTestId } = await render(<VendorCard vendor={mockVendor} onDelete={v => { deleted = v }} />)
  ...
  it('shows both buttons when both handlers provided', async () => {
    const { getByTestId } = await render(<VendorCard vendor={mockVendor} onEdit={() => {}} onDelete={() => {}} />)
    expect(getByTestId('edit-button')).toBeDefined()
    expect(getByTestId('delete-button')).toBeDefined()
  })
  ...
  it('renders tags from vendor', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, tags: ['cloud', 'saas'] }} />)
  ...
  it('renders single tag', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, tags: ['fintech'] }} />)
  ...
  it('renders security tag', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, tags: ['security'] }} />)
    expect(getByTestId('tag-security')).toBeDefined()
  })

  it('renders compliance tag', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, tags: ['compliance'] }} />)
    expect(getByTestId('tag-compliance')).toBeDefined()
  })

  it('renders hipaa tag', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, tags: ['hipaa'] }} />)
    expect(getByTestId('tag-hipaa')).toBeDefined()
  })

  it('renders iso27001 tag', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, tags: ['iso27001'] }} />)
  ...
  it('displays last review date', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, lastReviewDate: '2024-01-15' }} />)
    expect(getByTestId('vendor-review-date').textContent).toContain('2024-01-15')
  })

  it('displays different review date', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, lastReviewDate: '2023-06-30' }} />)
  ...
  it('renders without description', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, description: undefined }} />)
    expect(getByTestId('vendor-card')).toBeDefined()
  })
  ...
  it('renders vendor with long name', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, name: 'A Very Long Vendor Name That Might Overflow' }} />)
    expect(getByTestId('vendor-name')).toBeDefined()
  })

  it('renders vendor with long description', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, description: 'This is a very long description that goes on and on and on.' }} />)
    expect(getByTestId('vendor-description')).toBeDefined()
  })

  it('renders vendor with no tags and no description', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, tags: [], description: undefined }} />)
    expect(getByTestId('vendor-name')).toBeDefined()
  })

  it('renders vendor with special characters in name', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, name: 'Vendor & Co. <Test>' }} />)
    expect(getByTestId('vendor-name')).toBeDefined()
  })

  it('renders vendor with different category', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, category: 'Payment Processing' }} />)
  ...
  it('fires onClick multiple times', async () => {
    let count = 0
    const { getByTestId } = await render(<VendorCard vendor={mockVendor} onClick={() => { count++ }} />)
    await fireEvent.click(getByTestId('vendor-card'))
    await fireEvent.click(getByTestId('vendor-card'))
    await fireEvent.click(getByTestId('vendor-card'))
    expect(count).toBe(3)
  ...
  it('can toggle selected state', async () => {
    const { getByTestId, container } = await render(<VendorCard vendor={mockVendor} selected={false} />)
    expect(getByTestId('vendor-card').getAttribute('data-selected')).toBe('false')
  })
  ...
  it('renders correct contact email', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, contactEmail: 'test@example.com' }} />)
    expect(getByTestId('vendor-contact').textContent).toContain('test@example.com')
  })

  it('renders correct website href', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, website: 'https://example.com' }} />)
    expect(getByTestId('vendor-website').querySelector('a')?.getAttribute('href')).toBe('https://example.com')
  })
  ...
  it('vendor with http website renders correctly', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, website: 'http://legacy.com' }} />)
  ...
  it('vendor-card-header data-testid is vendor-card-header', async () => {
    const { getByTestId } = await render(<VendorCard vendor={mockVendor} />)
    expect(getByTestId('vendor-card-header').getAttribute('data-testid')).toBe('vendor-card-header')
  })
  ...
  it('vendor-review-date data-testid is vendor-review-date', async () => {
    const { getByTestId } = await render(<VendorCard vendor={mockVendor} />)
    expect(getByTestId('vendor-review-date').getAttribute('data-testid')).toBe('vendor-review-date')
  })
  ...
  it('snapshot card with all actions', async () => {
    await render(<VendorCard vendor={mockVendor} onEdit={() => {}} onDelete={() => {}} onClick={() => {}} />)
  ...
  it('fires onEdit once on single click', async () => {
    let count = 0
    const { getByTestId } = await render(<VendorCard vendor={mockVendor} onEdit={() => { count++ }} />)
    await fireEvent.click(getByTestId('edit-button'))
    expect(count).toBe(1)
  })
  ...
  it('fires onDelete once on single click', async () => {
    let count = 0
    const { getByTestId } = await render(<VendorCard vendor={mockVendor} onDelete={() => { count++ }} />)
    await fireEvent.click(getByTestId('delete-button'))
    expect(count).toBe(1)
  })
  ...
  it('fires onClick for each vendor in the list', async () => {
    for (const v of vendors) {
      let received: Vendor | null = null
      const { getByTestId } = await render(<VendorCard vendor={v} onClick={vendor => { received = vendor }} />)
      await fireEvent.click(getByTestId('vendor-card'))
  ...
  it('vendor-tags data-testid is vendor-tags', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, tags: ['cloud'] }} />)
  ...
  it('vendor-description data-testid is vendor-description', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, description: 'Test description' }} />)
    expect(getByTestId('vendor-description').getAttribute('data-testid')).toBe('vendor-description')
  })

  it('renders vendor description text', async () => {
    const { getByTestId } = await render(<VendorCard vendor={{ ...mockVendor, description: 'My description' }} />)
