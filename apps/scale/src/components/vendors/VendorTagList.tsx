...
export function VendorTagList({ tags, maxVisible = 5, onTagClick, onAddTag, onRemoveTag, editable = false }: VendorTagListProps) {
  ...
  return (
    <div data-testid="vendor-tag-list" style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', alignItems: 'center' }}>
      {visibleTags.map(tag => (
        <span key={tag} data-testid={`tag-item-${tag}`} style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
          <Badge
      ...
      {editable && adding && (
        <div data-testid="add-tag-input-container" style={{ display: 'inline-flex', gap: '4px' }}>
          <Input
            ...
            autoFocus
          />
          <Button data-testid="confirm-add-tag" size="sm" onClick={handleAddTag}>Add</Button>
          <Button data-testid="cancel-add-tag" size="sm" variant="secondary" onClick={() => { setAdding(false); setNewTag('') }}>Cancel</Button>
        </div>
      )}
    </div>
  )
