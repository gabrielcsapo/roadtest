...
export function VendorRiskSummary({ vendors, onClick }: VendorRiskSummaryProps) {
  ...
  const counts = RISK_ORDER.reduce<Record<Risk, number>>((acc, risk) => {
  ...
  return (
    <Card data-testid="vendor-risk-summary">
      <h3 data-testid="risk-summary-title" style={{ margin: '0 0 16px 0' }}>Vendor Risk Summary</h3>
      <div data-testid="risk-summary-total" style={{ marginBottom: '16px', color: '#6b7280', fontSize: '14px' }}>
        Total vendors: <strong>{total}</strong>
      </div>
      <div data-testid="risk-summary-items" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {RISK_ORDER.map(risk => {
          ...
          return (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                ...
                <span data-testid={`risk-count-${risk}`}>{count} ({pct}%)</span>
              </div>
              <Progress
                value={pct}
