...
const cleanSummary = {
  ...
  policies: { total: 10, byStatus: { compliant: 10, 'non-compliant': 0, 'in-progress': 0, 'not-applicable': 0 } },
...
const emptySummary = {
  ...
  policies: { total: 0, byStatus: { compliant: 0, 'non-compliant': 0, 'in-progress': 0, 'not-applicable': 0 } },
  issues: { total: 0, bySeverity: { low: 0, medium: 0, high: 0, critical: 0 } },
}
...
describe('ComplianceOverview', () => {
  ...
  it('calls onVendorsClick when vendors card is clicked', async () => {
    let clicked = false
    const { getByTestId } = await render(<ComplianceOverview summary={defaultSummary} onVendorsClick={() => { clicked = true }} />)
  ...
  it('calls onPersonnelClick when personnel card is clicked', async () => {
    let clicked = false
    const { getByTestId } = await render(<ComplianceOverview summary={defaultSummary} onPersonnelClick={() => { clicked = true }} />)
  ...
  it('calls onCredentialsClick when credentials card is clicked', async () => {
    let clicked = false
    const { getByTestId } = await render(<ComplianceOverview summary={defaultSummary} onCredentialsClick={() => { clicked = true }} />)
  ...
  it('calls onPoliciesClick when policies card is clicked', async () => {
    let clicked = false
    const { getByTestId } = await render(<ComplianceOverview summary={defaultSummary} onPoliciesClick={() => { clicked = true }} />)
  ...
  it('calls onIssuesClick when issues card is clicked', async () => {
    let clicked = false
    const { getByTestId } = await render(<ComplianceOverview summary={defaultSummary} onIssuesClick={() => { clicked = true }} />)
