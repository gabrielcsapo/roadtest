...
describe('FrameworkComplianceGrid', () => {
  ...
  it('shows spinner component when loading', async () => {
    const { getByTestId } = await render(<FrameworkComplianceGrid frameworks={frameworkEntries} loading />)
    expect(getByTestId('framework-grid-spinner')).toBeDefined()
  })

  it('does not show container when loading', async () => {
    const { queryByTestId } = await render(<FrameworkComplianceGrid frameworks={frameworkEntries} loading />)
    expect(queryByTestId('framework-grid-container')).toBeNull()
  })

  it('hides loading when not loading', async () => {
    const { queryByTestId } = await render(<FrameworkComplianceGrid frameworks={frameworkEntries} />)
  ...
  allFrameworks.map((fw) =>
    it(`shows green color for ${fw} at high compliance`, async () => {
      const { getByTestId } = await render(<FrameworkComplianceGrid frameworks={highComplianceFrameworks} />)
  ...
  allFrameworks.map((fw) =>
    it(`shows red color indicator for ${fw} at low compliance`, async () => {
      const { getByTestId } = await render(<FrameworkComplianceGrid frameworks={lowComplianceFrameworks} />)
  ...
  allFrameworks.map((fw) =>
    it(`shows yellow color indicator for ${fw} at mid compliance`, async () => {
      const { getByTestId } = await render(<FrameworkComplianceGrid frameworks={midComplianceFrameworks} />)
  ...
  allFrameworks.map((fw) =>
    it(`calls onSelect with ${fw} when card is clicked`, async () => {
      ...
      const { getByTestId } = await render(
        <FrameworkComplianceGrid frameworks={frameworkEntries} onSelect={(f) => { selected = f }} />
      )
  ...
  allFrameworks.map((fw, i) =>
    it(`data-percentage attribute is set correctly for ${fw}`, async () => {
      const { getByTestId } = await render(<FrameworkComplianceGrid frameworks={frameworkEntries} />)
      const expectedPct = Math.round(((8 - i) / 10) * 100)
      expect(getByTestId(`framework-card-${fw}`).getAttribute('data-percentage')).toBe(String(expectedPct))
    })
  )
  ...
  ] as Array<{ compliant: number; total: number; expected: string }>).map(({ compliant, total, expected }) =>
