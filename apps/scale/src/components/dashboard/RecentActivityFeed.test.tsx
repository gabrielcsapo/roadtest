...
const logs: AuditLog[] = [
  ...
  { ...mockLog, id: 'l4', action: 'user.role_changed', target: 'Bob Smith', targetType: 'user', actor: bobActor },
...
describe('RecentActivityFeed', () => {
  ...
  it('calls onViewAll when view all button is clicked', async () => {
    let clicked = false
    const { getByTestId } = await render(<RecentActivityFeed logs={logs} onViewAll={() => { clicked = true }} />)
  ...
  it('shows icon for vendor.created action', async () => {
    const { getByTestId } = await render(<RecentActivityFeed logs={[{ ...mockLog, id: 'vc', action: 'vendor.created' }]} />)
    expect(getByTestId('activity-icon-vc')).toBeDefined()
  })

  it('shows icon for policy.updated action', async () => {
    const { getByTestId } = await render(<RecentActivityFeed logs={[{ ...mockLog, id: 'pu', action: 'policy.updated' }]} />)
    expect(getByTestId('activity-icon-pu')).toBeDefined()
  })

  it('shows icon for credential.rotated action', async () => {
    const { getByTestId } = await render(<RecentActivityFeed logs={[{ ...mockLog, id: 'cr', action: 'credential.rotated' }]} />)
    expect(getByTestId('activity-icon-cr')).toBeDefined()
  })

  it('shows icon for unknown action type', async () => {
    const { getByTestId } = await render(<RecentActivityFeed logs={[{ ...mockLog, id: 'ua', action: 'something.unknown' }]} />)
