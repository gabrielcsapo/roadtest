...
export function ComplianceOverview({
  ...
  if (loading) {
    return (
      <div data-testid="compliance-overview-loading" style={{ display: 'flex', justifyContent: 'center', padding: '40px' }}>
    ...
    >
      ...
      >
        <div data-testid="vendors-section">
          ...
          <div data-testid="vendors-total" style={{ fontSize: '1.75rem', fontWeight: 700, color: '#111827' }}>
            {summary.vendors.total}
          </div>
          <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', marginTop: '4px' }}>
            {(Object.keys(summary.vendors.byRisk) as Risk[]).map((risk) => (
              summary.vendors.byRisk[risk] > 0 && (
                <Badge key={risk} data-testid={`vendors-risk-${risk}`} color={risk === 'critical' ? 'red' : risk === 'high' ? 'orange' : risk === 'medium' ? 'yellow' : 'green'} size="sm">
      ...
      >
        <div data-testid="personnel-section">
          <div style={{ fontSize: '0.75rem', color: '#6b7280', marginBottom: '4px' }}>Personnel</div>
          <div data-testid="personnel-total" style={{ fontSize: '1.75rem', fontWeight: 700, color: '#111827' }}>
            {summary.personnel.total}
          </div>
          <div style={{ marginTop: '4px', fontSize: '0.75rem' }}>
            {summary.personnel.offboarding > 0 && (
              <span data-testid="personnel-offboarding" style={{ color: '#f59e0b', marginRight: '8px' }}>
      ...
      >
        <div data-testid="credentials-section">
          <div style={{ fontSize: '0.75rem', color: '#6b7280', marginBottom: '4px' }}>Credentials</div>
          <div data-testid="credentials-total" style={{ fontSize: '1.75rem', fontWeight: 700, color: expiredCredentials > 0 ? '#ef4444' : '#111827' }}>
            {summary.credentials.total}
          </div>
          <div style={{ marginTop: '4px', fontSize: '0.75rem' }}>
            {summary.credentials.expired > 0 && (
              <span data-testid="credentials-expired" style={{ color: '#ef4444', marginRight: '8px' }}>
      ...
      >
        <div data-testid="policies-section">
          ...
          <div data-testid="policies-total" style={{ fontSize: '1.75rem', fontWeight: 700, color: nonCompliantPolicies > 0 ? '#ef4444' : '#111827' }}>
            {summary.policies.total}
          </div>
          <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', marginTop: '4px' }}>
            {Object.entries(summary.policies.byStatus).map(([status, count]) => (
              count > 0 && (
                <Badge key={status} data-testid={`policies-status-${status}`} color={status === 'compliant' ? 'green' : status === 'non-compliant' ? 'red' : status === 'in-progress' ? 'yellow' : 'gray'} size="sm">
                  {count}
                </Badge>
              )
            ))}
      ...
      >
        <div data-testid="issues-section">
          ...
          <div data-testid="issues-total" style={{ fontSize: '1.75rem', fontWeight: 700, color: criticalIssues > 0 ? '#ef4444' : '#111827' }}>
            {summary.issues.total}
          </div>
          <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', marginTop: '4px' }}>
            {(Object.keys(summary.issues.bySeverity) as Risk[]).map((severity) => (
              summary.issues.bySeverity[severity] > 0 && (
                <Badge key={severity} data-testid={`issues-severity-${severity}`} color={severity === 'critical' ? 'red' : severity === 'high' ? 'orange' : severity === 'medium' ? 'yellow' : 'green'} size="sm">
