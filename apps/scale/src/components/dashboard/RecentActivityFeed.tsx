  ...
  return (
    <div data-testid="activity-feed-container" data-item-count={displayedLogs.length}>
      ...
      >
        ...
        {onViewAll && (
          <Button data-testid="view-all-button" variant="ghost" size="sm" onClick={onViewAll}>
            View All
          </Button>
        )}
      </div>
      ...
      {displayedLogs.length === 0 ? (
        ...
        <ul data-testid="activity-feed-list" style={{ listStyle: "none", padding: 0, margin: 0 }}>
          {displayedLogs.map((log) => (
            ...
            >
              <span
                data-testid={`activity-icon-${log.id}`}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                >
                  <span
                    data-testid={`activity-actor-${log.id}`}
                  ...
                  <AuditActionBadge
                    action={log.action}
                    size="sm"
                    data-testid={`activity-action-badge-${log.id}`}
                  />
                </div>
                <div
                  data-testid={`activity-target-${log.id}`}
      ...
      {maxItems !== undefined && logs.length > maxItems && (
        <div
          data-testid="activity-feed-truncated"
          style={{ textAlign: "center", marginTop: "8px", fontSize: "0.875rem", color: "#6b7280" }}
        >
