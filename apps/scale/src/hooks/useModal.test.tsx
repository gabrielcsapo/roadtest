...
function ModalHarness({ initialOpen = false }: { initialOpen?: boolean }) {
  ...
  return (
    <div>
      ...
      <button data-testid="open-with-data" onClick={() => open({ id: "1", name: "Alice" })}>
        open with data
      </button>
      <button data-testid="open-with-data-2" onClick={() => open({ id: "2", name: "Bob" })}>
        open with data 2
      </button>
      ...
      function PayloadHarness() {
        ...
        return (
          <div>
            <span data-testid="isOpen">{String(isOpen)}</span>
            <span data-testid="data">{data ? JSON.stringify(data) : "null"}</span>
            <button data-testid="open" onClick={() => open(payload)}>
              open
            </button>
            <button data-testid="close" onClick={close}>
              close
