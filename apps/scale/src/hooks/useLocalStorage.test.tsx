...
function LocalStorageHarness<T>({ storageKey, initial }: { storageKey: string; initial: T }) {
  ...
  return (
    <div>
      ...
      <button data-testid="set-hello" onClick={() => setValue("hello" as unknown as T)}>
        set hello
      </button>
      <button data-testid="set-world" onClick={() => setValue("world" as unknown as T)}>
        set world
      </button>
      <button data-testid="set-number" onClick={() => setValue(42 as unknown as T)}>
        set 42
      </button>
      <button data-testid="set-null" onClick={() => setValue(null as unknown as T)}>
        set null
      </button>
      <button data-testid="remove" onClick={removeValue}>
        remove
      </button>
      <button
        data-testid="set-obj"
        onClick={() => setValue({ name: "Alice", age: 30 } as unknown as T)}
      >
        set obj
      </button>
      <button data-testid="set-arr" onClick={() => setValue([1, 2, 3] as unknown as T)}>
        set arr
      </button>
      <button data-testid="set-empty" onClick={() => setValue("" as unknown as T)}>
        set empty
      </button>
      <button data-testid="set-bool-true" onClick={() => setValue(true as unknown as T)}>
        set true
      </button>
      <button data-testid="set-bool-false" onClick={() => setValue(false as unknown as T)}>
        set false
      </button>
    </div>
  );
...
describe("useLocalStorage", () => {
    ...
    for (const val of initialValues) {
      it(`initializes with string value "${val}"`, async () => {
        const { getByTestId } = await render(
          <LocalStorageHarness storageKey={`test-str-${val}`} initial={val} />,
        );
  ...
  describe("initial state with numeric values", () => {
    ...
    for (const val of initialValues) {
      it(`initializes with numeric value ${val}`, async () => {
        const { getByTestId } = await render(
          <LocalStorageHarness storageKey={`test-num-${val}`} initial={val} />,
        );
        expect(getByTestId("value").textContent).toBe(JSON.stringify(val));
      });
    }
  ...
  describe("setting values", () => {
    it('sets value to "hello"', async () => {
      const { getByTestId } = await render(
        <LocalStorageHarness storageKey="test-set-1" initial="" />,
      );
    ...
    it('sets value to "world"', async () => {
      const { getByTestId } = await render(
        <LocalStorageHarness storageKey="test-set-2" initial="" />,
      );
    ...
    it("sets value to number 42", async () => {
      const { getByTestId } = await render(
        <LocalStorageHarness storageKey="test-set-3" initial={0} />,
      );
    ...
    it("sets value to null", async () => {
      const { getByTestId } = await render(
        <LocalStorageHarness storageKey="test-set-4" initial="default" />,
      );
    ...
    it("sets value to object", async () => {
      const { getByTestId } = await render(
        <LocalStorageHarness storageKey="test-set-5" initial={{}} />,
      );
    ...
    it("sets value to array", async () => {
      const { getByTestId } = await render(
        <LocalStorageHarness storageKey="test-set-6" initial={[]} />,
      );
    ...
    it("sets value to empty string", async () => {
      const { getByTestId } = await render(
        <LocalStorageHarness storageKey="test-set-7" initial="start" />,
      );
    ...
    it("sets value to boolean true", async () => {
      const { getByTestId } = await render(
        <LocalStorageHarness storageKey="test-set-8" initial={false} />,
      );
    ...
    it("sets value to boolean false", async () => {
      const { getByTestId } = await render(
        <LocalStorageHarness storageKey="test-set-9" initial={true} />,
      );
    ...
    it("can update value multiple times", async () => {
      const { getByTestId } = await render(
        <LocalStorageHarness storageKey="test-multi" initial="" />,
      );
      await fireEvent.click(getByTestId("set-hello"));
      expect(getByTestId("value").textContent).toBe('"hello"');
      await fireEvent.click(getByTestId("set-world"));
  ...
  describe("removing values", () => {
    it("removes value and reverts to initial (string)", async () => {
      const { getByTestId } = await render(
        <LocalStorageHarness storageKey="test-remove-1" initial="default" />,
      );
      await fireEvent.click(getByTestId("set-hello"));
    ...
    it("removes value and reverts to initial (empty string)", async () => {
      const { getByTestId } = await render(
        <LocalStorageHarness storageKey="test-remove-2" initial="" />,
      );
      await fireEvent.click(getByTestId("set-hello"));
      await fireEvent.click(getByTestId("remove"));
      expect(getByTestId("value").textContent).toBe('""');
    ...
    it("removes value and reverts to initial (number)", async () => {
      const { getByTestId } = await render(
        <LocalStorageHarness storageKey="test-remove-3" initial={0} />,
      );
      await fireEvent.click(getByTestId("set-number"));
      await fireEvent.click(getByTestId("remove"));
    ...
    it("calling remove on untouched state keeps initial", async () => {
      const { getByTestId } = await render(
        <LocalStorageHarness storageKey="test-remove-4" initial="initial" />,
      );
      await fireEvent.click(getByTestId("remove"));
  ...
  describe("sequence of operations", () => {
    ...
    for (const seq of sequences) {
      it(`sequence [${seq.ops.join(", ")}] ends with ${seq.final}`, async () => {
        const { getByTestId } = await render(
          <LocalStorageHarness storageKey={seq.key} initial="" />,
        );
  ...
  describe("different storage keys", () => {
    ...
    for (const k of keys) {
      it(`works with storage key "${k}"`, async () => {
        const { getByTestId } = await render(
          <LocalStorageHarness storageKey={k} initial="initial" />,
        );
        expect(getByTestId("value").textContent).toBe('"initial"');
        await fireEvent.click(getByTestId("set-hello"));
        expect(getByTestId("value").textContent).toBe('"hello"');
  ...
  describe("edge cases", () => {
    ...
    it("handles boolean initial value false", async () => {
      const { getByTestId } = await render(
        <LocalStorageHarness storageKey="edge-3" initial={false} />,
      );
      expect(getByTestId("value").textContent).toBe("false");
    });
    ...
    it("handles array initial value", async () => {
      const { getByTestId } = await render(
        <LocalStorageHarness storageKey="edge-4" initial={[]} />,
      );
      expect(getByTestId("value").textContent).toBe("[]");
    });
    ...
    it("handles object initial value", async () => {
      const { getByTestId } = await render(
        <LocalStorageHarness storageKey="edge-5" initial={{}} />,
      );
      expect(getByTestId("value").textContent).toBe("{}");
    });
  });
...
describe("useLocalStorage - type-specific initial values", () => {
  ...
  for (const c of typeCases) {
    it(`initial string "${c.initial}" shows as ${c.expected}`, async () => {
      ...
      const { getByTestId } = await render(
        <LocalStorageHarness storageKey={key} initial={c.initial} />,
      );
...
describe("useLocalStorage - additional initial value verification", () => {
  ...
  for (const val of extraValues) {
    it(`initial value "${val}" is displayed correctly`, async () => {
      const { getByTestId } = await render(
        <LocalStorageHarness storageKey={`extra-${val}`} initial={val} />,
      );
      expect(getByTestId("value").textContent).toBe(JSON.stringify(val));
    });
  }
