...
function PaginationHarness({ total, initialPage = 1, initialPageSize = 10 }: { total: number; initialPage?: number; initialPageSize?: number }) {
  const { page, pageSize, totalPages, goToPage, nextPage, prevPage, setPageSize, isFirstPage, isLastPage } = usePagination(total, initialPage, initialPageSize)
  return (
    <div>
      ...
      <button data-testid="go-1" onClick={() => goToPage(1)}>go 1</button>
      <button data-testid="go-2" onClick={() => goToPage(2)}>go 2</button>
      <button data-testid="go-5" onClick={() => goToPage(5)}>go 5</button>
      <button data-testid="go-last" onClick={() => goToPage(totalPages)}>go last</button>
      <button data-testid="go-999" onClick={() => goToPage(999)}>go 999</button>
      <button data-testid="size-5" onClick={() => setPageSize(5)}>size 5</button>
      <button data-testid="size-20" onClick={() => setPageSize(20)}>size 20</button>
      <button data-testid="size-50" onClick={() => setPageSize(50)}>size 50</button>
    </div>
  )
}
  ...
  describe('initial state with different page sizes', () => {
    ...
    for (const size of pageSizes) {
      it(`initializes with pageSize=${size}`, async () => {
        const { getByTestId } = await render(<PaginationHarness total={100} initialPageSize={size} />)
  ...
  describe('totalPages calculation', () => {
    ...
    for (const c of cases) {
      it(`total=${c.total}, size=${c.size} => ${c.expected} totalPages`, async () => {
        const { getByTestId } = await render(<PaginationHarness total={c.total} initialPageSize={c.size} />)
  ...
  describe('nextPage', () => {
    ...
    for (const startPage of nextCases) {
      it(`nextPage from page ${startPage} goes to ${startPage + 1}`, async () => {
        const { getByTestId } = await render(<PaginationHarness total={100} initialPage={startPage} />)
...
describe('usePagination - goToPage boundary clamping', () => {
  ...
  for (const c of clampCases) {
    it(`total=${c.total} pageSize=${c.pageSize} goTo ${c.goto} => page=${c.expected}`, async () => {
      const { getByTestId } = await render(<PaginationHarness total={c.total} initialPageSize={c.pageSize} />)
      if (c.goto === 999) {
        await fireEvent.click(getByTestId('go-999'))
      } else if (c.goto <= 0) {
        await fireEvent.click(getByTestId('go-1'))
...
describe('usePagination - page size changes', () => {
  ...
  for (const c of sizeCases) {
    it(`changing pageSize via ${c.to} sets pageSize to ${c.expected}`, async () => {
      const { getByTestId } = await render(<PaginationHarness total={100} initialPageSize={c.from} />)
