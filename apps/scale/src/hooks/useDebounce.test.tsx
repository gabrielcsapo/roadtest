...
function DebounceHarness({ initialValue = '', delay = 300 }: { initialValue?: string; delay?: number }) {
...
function NumericDebounceHarness({ initialValue = 0, delay = 300 }: { initialValue?: number; delay?: number }) {
  const [value, setValue] = useState(initialValue)
  const debouncedValue = useDebounce(value, delay)
  return (
    <div>
...
describe('useDebounce', () => {
  describe('initial state', () => {
    const initialValues = ['', 'hello', 'world', 'test', 'foo', 'bar', '12345', 'a', 'longer text here', 'special!@#']
  ...
  describe('input interactions', () => {
    const inputValues = ['a', 'ab', 'abc', 'hello', 'world', 'test1', 'test2', 'foo', 'bar', 'baz',
      'query1', 'query2', 'search', 'find', 'filter', 'abc123', 'xyz', 'new value', 'updated', 'final']
  ...
  describe('numeric increment interactions', () => {
    ...
    for (const n of increments) {
      it(`current value updates after ${n} increments`, async () => {
        const { getByTestId } = await render(<NumericDebounceHarness initialValue={0} delay={300} />)
        for (let i = 0; i < n; i++) {
...
describe('useDebounce - initial debounced matches initial current', () => {
  const matchCases = ['', 'hello', 'world', 'test', '123', 'abc', 'foo', 'bar', 'baz', 'query',
    'search', 'find', 'filter', 'data', 'value', 'entry', 'item', 'record', 'field', 'key']
...
describe('useDebounce - additional input verification cases', () => {
  const extraInputs = ['alpha', 'beta', 'gamma', 'delta', 'epsilon', 'zeta', 'eta', 'theta', 'iota', 'kappa',
