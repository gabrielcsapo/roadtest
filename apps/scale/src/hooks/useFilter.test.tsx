...
function FilterHarness({ initial = defaultFilters }: { initial?: TestFilters }) {
  const { filters, setFilter, toggleFilter, clearFilter, clearAllFilters, hasFilters } = useFilter<TestFilters>(initial)
  return (
    <div>
      ...
      <button data-testid="toggle-active" onClick={() => toggleFilter('status', 'active')}>toggle active</button>
      <button data-testid="toggle-inactive" onClick={() => toggleFilter('status', 'inactive')}>toggle inactive</button>
      <button data-testid="toggle-low" onClick={() => toggleFilter('risk', 'low')}>toggle low</button>
      <button data-testid="toggle-high" onClick={() => toggleFilter('risk', 'high')}>toggle high</button>
      <button data-testid="toggle-critical" onClick={() => toggleFilter('risk', 'critical')}>toggle critical</button>

      <button data-testid="set-search-hello" onClick={() => setFilter('search', 'hello')}>set search hello</button>
      <button data-testid="set-search-world" onClick={() => setFilter('search', 'world')}>set search world</button>
      <button data-testid="set-search-empty" onClick={() => setFilter('search', '')}>set search empty</button>
      <button data-testid="set-category-cloud" onClick={() => setFilter('category', 'cloud')}>set category cloud</button>
      <button data-testid="set-category-saas" onClick={() => setFilter('category', 'saas')}>set category saas</button>

      <button data-testid="clear-status" onClick={() => clearFilter('status')}>clear status</button>
      <button data-testid="clear-search" onClick={() => clearFilter('search')}>clear search</button>
      <button data-testid="clear-all" onClick={clearAllFilters}>clear all</button>
    </div>
  )
}
...
describe('useFilter', () => {
  ...
  describe('complex interaction sequences', () => {
    const sequences = [
      { name: 'toggle-active then clear-all', ops: ['toggle-active', 'clear-all'], expectHasFilters: false },
      { name: 'set-search-hello then clear-all', ops: ['set-search-hello', 'clear-all'], expectHasFilters: false },
      { name: 'toggle-active, toggle-low, clear-status', ops: ['toggle-active', 'toggle-low', 'clear-status'], expectHasFilters: true },
    ]
  ...
  describe('toggle idempotency', () => {
    const toggleCases = ['toggle-active', 'toggle-inactive', 'toggle-low', 'toggle-high', 'toggle-critical']
...
describe('useFilter - setFilter with various values', () => {
  const searchCases = ['hello', 'world', 'foo', 'bar', 'baz', 'test', 'query', 'search', 'find', 'filter', 'abc', 'xyz']
...
describe('useFilter - hasFilters with every combination', () => {
  const stateVariations = [
    { ops: ['toggle-active', 'toggle-low', 'set-search-hello', 'set-category-cloud'], expected: 'true' },
    { ops: ['toggle-active', 'toggle-low', 'set-search-hello', 'set-category-cloud', 'clear-all'], expected: 'false' },
