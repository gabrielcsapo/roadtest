...
function FormHarness({
  ...
  const {
    ...
    reset,
    handleSubmit,
  } = useForm<TestFormValues>(initialValues, withValidator ? validator : undefined);

  let submitResult = "";
  const onSubmit = handleSubmit((vals) => {
    submitResult = vals.name;
  });
  ...
  return (
    <div>
      ...
      <button data-testid="set-name-alice" onClick={() => setValue("name", "Alice")}>
        set name Alice
      </button>
      <button data-testid="set-name-bob" onClick={() => setValue("name", "Bob")}>
        set name Bob
      </button>
      <button data-testid="set-name-empty" onClick={() => setValue("name", "")}>
        set name empty
      </button>
      <button data-testid="set-email-valid" onClick={() => setValue("email", "user@example.com")}>
        set email valid
      </button>
      <button data-testid="set-email-invalid" onClick={() => setValue("email", "notanemail")}>
        set email invalid
      </button>
      <button data-testid="set-message" onClick={() => setValue("message", "Hello world")}>
        set message
      </button>
      <button data-testid="set-age-25" onClick={() => setValue("age", 25)}>
        set age 25
      </button>
      <button data-testid="set-agree-true" onClick={() => setValue("agree", true)}>
        set agree true
      </button>

      <button
        data-testid="set-values"
        onClick={() => setValues({ name: "Charlie", email: "charlie@example.com" })}
      >
        set values
      </button>

      <button data-testid="set-error-name" onClick={() => setError("name", "Custom error")}>
        set error name
      </button>
      <button data-testid="clear-error-name" onClick={() => clearError("name")}>
        clear error name
      </button>

      <button data-testid="touch-name" onClick={() => touch("name")}>
        touch name
      </button>
      <button data-testid="touch-email" onClick={() => touch("email")}>
        touch email
      </button>
      <button data-testid="touch-all" onClick={touchAll}>
        touch all
      </button>
    ...
    it(`setValue name="${name}" shows in name field`, async () => {
      function NameHarness() {
        ...
        return (
          <div>
            <span data-testid="name">{values.name}</span>
            <button data-testid="set" onClick={() => setValue("name", name)}>
              set
            </button>
          </div>
        );
...
describe("useForm - error management", () => {
  const errorCases = [
    ...
    {
      label: "set and clear name error",
      ops: ["set-error-name", "clear-error-name"],
      testid: "error-name",
      expected: "",
    },
    {
      label: "set and clear email error",
      ops: ["set-error-email", "clear-error-email"],
      testid: "error-email",
      expected: "",
    },
  ];
