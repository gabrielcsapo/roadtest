...
function SelectionHarness({ initialIds = [] as string[] }) {
  ...
  return (
    <div>
      ...
      <button data-testid="toggle-id1" onClick={() => toggle("id1")}>
        toggle id1
      </button>
      <button data-testid="toggle-id2" onClick={() => toggle("id2")}>
        toggle id2
      </button>
      <button data-testid="toggle-id3" onClick={() => toggle("id3")}>
        toggle id3
      </button>
      <button data-testid="toggle-id4" onClick={() => toggle("id4")}>
        toggle id4
      </button>
      <button data-testid="toggle-id5" onClick={() => toggle("id5")}>
        toggle id5
      </button>

      <button data-testid="select-id1" onClick={() => select("id1")}>
        select id1
      </button>
      <button data-testid="select-id2" onClick={() => select("id2")}>
        select id2
      </button>
      <button data-testid="select-id3" onClick={() => select("id3")}>
        select id3
      </button>

      <button data-testid="deselect-id1" onClick={() => deselect("id1")}>
        deselect id1
      </button>
      <button data-testid="deselect-id2" onClick={() => deselect("id2")}>
        deselect id2
      </button>

      <button data-testid="select-all" onClick={() => selectAll(ALL_IDS)}>
        select all
      </button>
      <button data-testid="deselect-all" onClick={deselectAll}>
        deselect all
      </button>
    </div>
  );
...
describe("useSelection - hasSelection states", () => {
  const hasCases = [
    ...
    {
      label: "select-all then deselect-all",
      ops: ["select-all", "deselect-all"],
      expected: "false",
    },
...
describe("useSelection - selectedCount with 5 pre-selected toggling all off", () => {
  ...
  it("toggle all 5 IDs off when pre-selected: count=0", async () => {
    const { getByTestId } = await render(
      <SelectionHarness initialIds={["id1", "id2", "id3", "id4", "id5"]} />,
    );
    for (const id of allIds) {
      await fireEvent.click(getByTestId(`toggle-${id}`));
    }
